(function(){
    ...
    var PV_DATA_BOX_ID = pvStr('data-data-box-id','box-data') || 'box-data';
...
function escapeHtml(s){
    ...
    function fitPoly2(xs, ys){
      try{
        ...
        for(var i=0;i<n;i++){
          var x=Number(xs[i]), y=Number(ys[i]);
          if(!isFinite(x) || !isFinite(y)) continue;
          var x2=x*x, x3=x2*x, x4=x2*x2;
          Sx+=x; Sx2+=x2; Sx3+=x3; Sx4+=x4;
          Sy+=y; Sxy+=x*y; Sx2y+=x2*y;
        }
        var A=[[Sx4,Sx3,Sx2],[Sx3,Sx2,Sx],[Sx2,Sx,n]], B=[Sx2y,Sxy,Sy];
        // Gaussian elimination 3x3
        for(var r=0;r<3;r++){
          var piv=r;
          for(var rr=r+1;rr<3;rr++) if(Math.abs(A[rr][r])>Math.abs(A[piv][r])) piv=rr;
          if(Math.abs(A[piv][r])<1e-12) return null;
          if(piv!==r){ var tA=A[r];A[r]=A[piv];A[piv]=tA; var tB=B[r];B[r]=B[piv];B[piv]=tB; }
          var div=A[r][r];
          for(var c=r;c<3;c++) A[r][c]/=div;
          B[r]/=div;
          for(var r2=0;r2<3;r2++) if(r2!==r){
            var f=A[r2][r];
            for(var c2=r;c2<3;c2++) A[r2][c2]-=f*A[r][c2];
            B[r2]-=f*B[r];
          }
        }
    ...
    function layoutBase(extra){
      ...
      var base = {
        ...
        font:{color: th.font},
        xaxis:{gridcolor: th.grid, zeroline:false, zerolinecolor: th.grid, automargin:true, ticklabeloverflow:'allow', title:{standoff:26}},
        yaxis:{gridcolor: th.grid, zeroline:false, zerolinecolor: th.grid, automargin:true, title:{standoff:16}}
      };
      if(extra){
    ...
    var VM_PALETTE_PRESETS = {
      ...
      oracle: '#312782,#f80000,#ff9f00,#00a6a6,#8c6bb1,#d95f02,#66a61e,#1b9e77,#7570b3,#e7298a',
      pastel: '#a1c9f4,#ffb482,#8de5a1,#ff9f9b,#d0bbff,#debb9b,#fab0e4,#cfcfcf,#fffea3,#b9f2f0',
      vibrant: '#3b82f6,#ef4444,#10b981,#f59e0b,#8b5cf6,#06b6d4,#f43f5e,#84cc16,#14b8a6,#f97316',
      colorblind: '#0072B2,#E69F00,#009E73,#CC79A7,#56B4E9,#D55E00,#F0E442,#999999',
      material: '#1e88e5,#43a047,#f4511e,#8e24aa,#00acc1,#fdd835,#6d4c41,#3949ab,#00897b,#c0ca33',
      set2: '#66c2a5,#fc8d62,#8da0cb,#e78ac3,#a6d854,#ffd92f,#e5c494,#b3b3b3'
    };
    function vmPalettePresetRaw(name){
      var k = String(name||'').toLowerCase();
      return VM_PALETTE_PRESETS[k] || '';
    }
    function vmPalettePresetNameFor(raw){
      var t = String(raw||'').replace(/\s+/g,'').toLowerCase();
      if(!t) return 'custom';
      for(var k in VM_PALETTE_PRESETS){
        if(!Object.prototype.hasOwnProperty.call(VM_PALETTE_PRESETS,k)) continue;
        if(String(VM_PALETTE_PRESETS[k]).replace(/\s+/g,'').toLowerCase()===t) return k;
      }
      return 'custom';
    }
    ...
    function vmGetPalette(v){
      ...
      var pal = [];
      if(opts.palettePreset && String(opts.palettePreset)!=='custom') pal = vmParsePalette(vmPalettePresetRaw(opts.palettePreset));
      if(!pal.length) pal = vmParsePalette(opts.palette);
      if(pal.length) return pal;
      if(!pal.length && STATE.globals && STATE.globals.palettePreset){ pal = vmParsePalette(vmPalettePresetRaw(STATE.globals.palettePreset)); }
      if(pal.length) return pal;
    ...
    function vmAddTargetsToLayout(layout, v, ctx, rows){
      ...
      var opts = v.options || {};
      var trg = vmResolveTargetRange(v, rows||[]);
      var minVal = trg.min;
      var maxVal = trg.max;
    ...
    function vmAddReferenceLinesToLayout(layout, v){
      if(!layout || !v || !isCartesianType(v.type)) return;
      var opts = v.options || {};
      if(opts.showRefLines !== true) return;
    ...
    function mappedAxisTitle(v, ctx, customTitle, fallbackKeys){
      if(customTitle!=null && String(customTitle).trim()!=='') return String(customTitle);
      var keys = Array.isArray(fallbackKeys) ? fallbackKeys : [fallbackKeys];
      for(var i=0;i<keys.length;i++){
        var k = firstKey(keys[i]);
        if(k) return vmColLabel(ctx, k);
      }
      return '';
    }
    function firstKey(vv){
      if(Array.isArray(vv)) return vv.length ? String(vv[0]||'') : '';
      if(vv && typeof vv==='object'){
        if(vv.colKey) return String(vv.colKey||'');
        if(vv.key) return String(vv.key||'');
      }
      return vv ? String(vv) : '';
    }
    function vmLegendLayoutFor(pos){
      ...
      if(pos==='bottom') return {orientation:'h', x:0, y:-0.20, xanchor:'left', yanchor:'top'};
      if(pos==='left') return {orientation:'v', x:-0.02, y:1, xanchor:'right', yanchor:'top'};
      if(pos==='right') return {orientation:'v', x:1.04, y:1, xanchor:'left', yanchor:'top'};
      return {}; // auto
    }
    function vmApplyRuntimeOptions(v, pel, ctx){
      try{
        ...
        if(opts){
          ...
          }else if(String(opts.legendPos||'auto')==='auto' && gd && Array.isArray(gd.data)){
            var tCount = gd.data.filter(function(t){ return !t || t.showlegend!==false; }).length;
            if(tCount>0 && tCount<=3){ updates.legend = objAssign({}, vmLegendLayoutFor('top')); }
          }
          if(opts.hoverMode) updates.hovermode = String(opts.hoverMode);
          var hasY2Axis = !!(gd.layout && gd.layout.yaxis2);
          if(opts.showLegend!==false && String(opts.legendPos||'auto')==='right') updates['margin.r'] = hasY2Axis ? 170 : 120;
          if(opts.showLegend!==false && String(opts.legendPos||'auto')==='left') updates['margin.l'] = hasY2Axis ? 90 : 70;
          if(opts.showLegend!==false && String(opts.legendPos||'auto')==='top') updates['margin.t'] = Math.max(44, Number((gd.layout&&gd.layout.margin&&gd.layout.margin.t)||0));
          if(opts.showLegend!==false && String(opts.legendPos||'auto')==='bottom') updates['margin.b'] = Math.max(56, Number((gd.layout&&gd.layout.margin&&gd.layout.margin.b)||0));
        }

        // axis titles default to mapped columns (if not set)
        var m = v && v.mapping ? v.mapping : {};
        var meas = v && v.measures ? v.measures : {};
        var xKey = firstKey(m.x || m.category || m.label || m.date || m.name || m.cols || m.col || m.row || '');
        var yKey = '';
        if(meas){
          if(meas.y && meas.y.colKey) yKey = String(meas.y.colKey||'');
          else {
            for(var mk in meas){
              if(!Object.prototype.hasOwnProperty.call(meas,mk)) continue;
              var mo = meas[mk];
              if(mo && mo.colKey){ yKey = String(mo.colKey||''); break; }
            }
          }
        }
        if(!yKey) yKey = firstKey(m.y || m.value || m.measure || m.target || m.values || '');
        // allow custom titles
        ...
        if(gd.layout && gd.layout.yaxis2){
          var y2Title = (opts && opts.y2Title) ? String(opts.y2Title) : '';
          if(!y2Title && meas){
            if(meas.y2 && meas.y2.colKey) y2Title = vmColLabel(ctx, meas.y2.colKey);
            else if(meas.line && meas.line.colKey) y2Title = vmColLabel(ctx, meas.line.colKey);
            else if(meas.right && meas.right.colKey) y2Title = vmColLabel(ctx, meas.right.colKey);
          }
          if(y2Title) updates['yaxis2.title.text'] = y2Title;
          updates['yaxis2.title.standoff'] = 22;
        }
        if(gd.layout && gd.layout.yaxis3){
          var y3Title = (opts && opts.y3Title) ? String(opts.y3Title) : '';
          if(y3Title) updates['yaxis3.title.text'] = y3Title;
        }
        if(gd.layout && gd.layout.yaxis4){
          var y4Title = (opts && opts.y4Title) ? String(opts.y4Title) : '';
          if(y4Title) updates['yaxis4.title.text'] = y4Title;
        }
        updates['xaxis.automargin'] = true;
        updates['yaxis.automargin'] = true;
        updates['xaxis.title.standoff'] = 26;
        updates['yaxis.title.standoff'] = 16;
        updates['xaxis.ticklabeloverflow'] = 'allow';
        var xAng = parseFloat(opts.xTickAngle);
        if(isFinite(xAng)) updates['xaxis.tickangle'] = xAng;
        var yAng = parseFloat(opts.yTickAngle);
        if(isFinite(yAng)) updates['yaxis.tickangle'] = yAng;
        try{
          var curB = Number(gd.layout && gd.layout.margin && gd.layout.margin.b);
          var reqB = (isFinite(xAng) && Math.abs(xAng)>=60) ? 140 : 96;
          if(!isFinite(curB) || curB < reqB) updates['margin.b'] = reqB;
          var curL = Number(gd.layout && gd.layout.margin && gd.layout.margin.l);
          var reqL = (isFinite(yAng) && Math.abs(yAng)>=45) ? 88 : 64;
          if(!isFinite(curL) || curL < reqL) updates['margin.l'] = reqL;
        }catch(_mb){}
        ...
        var yMinNum = (opts.yMin==null || opts.yMin==='') ? null : Number(opts.yMin);
        var yMaxNum = (opts.yMax==null || opts.yMax==='') ? null : Number(opts.yMax);
        if(yMode==='manual' && (isFinite(yMinNum) || isFinite(yMaxNum))){
          var yMin = isFinite(yMinNum) ? yMinNum : null;
          var yMax = isFinite(yMaxNum) ? yMaxNum : null;
          if(yMin!=null && yMax!=null && yMin>yMax){ var tmp=yMin; yMin=yMax; yMax=tmp; }
          if(yMin!=null && yMax!=null){ updates['yaxis.autorange']=false; updates['yaxis.range']=[yMin,yMax]; }
        }else if(yMode!=='manual'){
          updates['yaxis.autorange']=true;
        }
        // apply to y2/y3/y4 if present
        if(gd.layout && gd.layout.yaxis2){
          ...
          var y2MinNum = (opts.y2Min==null || opts.y2Min==='') ? null : Number(opts.y2Min);
          var y2MaxNum = (opts.y2Max==null || opts.y2Max==='') ? null : Number(opts.y2Max);
          if(yMode==='manual' && isFinite(y2MinNum) && isFinite(y2MaxNum)){
            var y2Min = y2MinNum, y2Max = y2MaxNum;
            if(y2Min>y2Max){ var t2=y2Min; y2Min=y2Max; y2Max=t2; }
            updates['yaxis2.autorange']=false; updates['yaxis2.range']=[y2Min,y2Max];
          }else if(yMode!=='manual'){
            updates['yaxis2.autorange']=true;
          }
        }
        if(gd.layout && gd.layout.yaxis3){
          var y3MinNum = (opts.y3Min==null || opts.y3Min==='') ? null : Number(opts.y3Min);
          var y3MaxNum = (opts.y3Max==null || opts.y3Max==='') ? null : Number(opts.y3Max);
          if(yMode==='manual' && isFinite(y3MinNum) && isFinite(y3MaxNum)){
            updates['yaxis3.autorange']=false; updates['yaxis3.range']=[Math.min(y3MinNum,y3MaxNum), Math.max(y3MinNum,y3MaxNum)];
          }else if(yMode!=='manual'){ updates['yaxis3.autorange']=true; }
        }
        if(gd.layout && gd.layout.yaxis4){
          var y4MinNum = (opts.y4Min==null || opts.y4Min==='') ? null : Number(opts.y4Min);
          var y4MaxNum = (opts.y4Max==null || opts.y4Max==='') ? null : Number(opts.y4Max);
          if(yMode==='manual' && isFinite(y4MinNum) && isFinite(y4MaxNum)){
            updates['yaxis4.autorange']=false; updates['yaxis4.range']=[Math.min(y4MinNum,y4MaxNum), Math.max(y4MinNum,y4MaxNum)];
          }else if(yMode!=='manual'){ updates['yaxis4.autorange']=true; }
        }
        ...
        if(opts && Object.prototype.hasOwnProperty.call(opts,'showLabels')){
          var labelOn = !!opts.showLabels;
          var textMode = labelOn ? 'auto' : 'none';
          try{ Plotly.restyle(gd, {texttemplate:textMode, textposition: (labelOn?'auto':'none')}); }catch(_rt){}
        }
      }catch(e){}
    }
...
// ---------------------------
    ...
    function normalizeDataKind(kind){
      var k = String(kind||'').toLowerCase();
      if(k==='number' || k==='numeric' || k==='decimal' || k==='float' || k==='integer' || k==='int') return 'number';
      if(k==='date' || k==='datetime' || k==='timestamp') return 'date';
      if(k==='boolean' || k==='bool') return 'boolean';
      return 'string';
    }

    function getNarrativeColumnsMeta(){
      var ids = ['box-columns','box_columns','columns-data','columns'];
      for(var i=0;i<ids.length;i++){
        ...
        else if(parsed && Array.isArray(parsed.columns)) list = parsed.columns;
        else if(parsed && Array.isArray(parsed.data)) list = parsed.data;
        if(!list.length) continue;
        var out = {};
        for(var j=0;j<list.length;j++){
          var c = list[j]||{};
          var key = String(c.key||c.name||c.column||c.id||'');
          if(!key) continue;
          var dec = c.decimals;
          if(dec==null && c.scale!=null) dec = c.scale;
          out[key] = { kind: normalizeDataKind(c.kind||c.type||c.dataType||c.datatype), decimals: (dec==null||dec==='') ? null : Number(dec) };
        }
    ...
    function getNarrativeRows(){
      ...
      var ids = [PV_DATA_BOX_ID,'box-data','box_data','vm-data','oas-data','data'];
      var el = null;
      for(var i=0;i<ids.length;i++){
    ...
    function inferColumns(rows, colMeta){
      var cols = [];
      ...
      if(colMeta && typeof colMeta==='object'){ Object.keys(colMeta).forEach(function(k){ keySet[k]=1; }); }
      var keys = Object.keys(keySet);
      for(var i=0;i<keys.length;i++){
        var k = keys[i];
        var kind = (colMeta && colMeta[k] && colMeta[k].kind) ? normalizeDataKind(colMeta[k].kind) : 'string';
        ...
        if((!colMeta || !colMeta[k] || !colMeta[k].kind) && nonNull>0 && numericCount/nonNull > 0.8) kind='number';
        var col = { key:k, label:k, kind:kind };
        if(colMeta && colMeta[k] && Number.isFinite(Number(colMeta[k].decimals))) col.decimals = Number(colMeta[k].decimals);
        cols.push(col);
      }
      return cols;
    }
    ...
    function colOptionsHtml(cols, selectedKey, allowBlank){
      var sig = (cols||[]).map(function(c){ return String(c&&c.key||''); }).join('|');
      var ck = sig + '|' + (selectedKey||'') + '|' + (allowBlank?'1':'0');
    ...
    // ---------------------------
    var Agg = {
      none: { label:'None (Raw)', needsNumeric:false },
    ...
    function aggregateRows(rows, transform, extraCols){
      ...
      try{
        var gState = (typeof STATE==='object' && STATE && STATE.globals) ? STATE.globals : null;
        var cf = gState ? gState.crossFilter : null;
        if(gState && gState.crossFilterEnabled && cf && cf.col){
          out = out.filter(function(r){ return asString(r[cf.col])===asString(cf.val); });
        }
      }catch(_){ }
      return out;
    }
    ...
    function mappingFieldMulti(label, keyName, cols, selectedKeys, allowBlank, note){
      ...
      var selectedSet = {};
      for(var i=0; i<selectedKeys.length; i++) selectedSet[selectedKeys[i]] = true;
      var btnLabel = selectedKeys.length ? (selectedKeys.length + ' selected') : 'Select values';
      var html = '';
      html += '<div class="lbl">'+escapeHtml(label)+'</div>';
      html += '<div class="map-multi-cell">';
      html += '<div class="vm-msel" data-map-multi-wrap="'+escapeHtml(keyName)+'">';
      html += '<button type="button" class="btn vm-msel-btn" data-map-multi-btn="'+escapeHtml(keyName)+'">'+escapeHtml(btnLabel)+'</button>';
      html += '<div class="vm-msel-menu" data-map-multi-menu="'+escapeHtml(keyName)+'" style="display:none">';
      html += '<input class="search" data-map-multi-search="'+escapeHtml(keyName)+'" placeholder="Search..." />';
      if(allowBlank){
        html += '<label class="vm-msel-item" data-map-multi-label="(none)">';
        html += '<input type="checkbox" data-map-multi="'+escapeHtml(keyName)+'" value="" '+(selectedSet['']?'checked':'')+'> <span>(none)</span>';
        html += '</label>';
      }
      for(var j=0; j<cols.length; j++){
        var c = cols[j] || {};
        var ck = String(c.key||'');
        var isChecked = selectedSet[ck] ? 'checked' : '';
        var lbl = String(c.label||ck);
        html += '<label class="vm-msel-item" data-map-multi-label="'+escapeHtml(lbl.toLowerCase())+'">';
        html += '<input type="checkbox" data-map-multi="'+escapeHtml(keyName)+'" value="'+escapeHtml(ck)+'" '+isChecked+'> <span>'+escapeHtml(lbl)+'</span>';
        html += '</label>';
      }
      html += '</div></div>';
    ...
    function measureField(label, keyName, cols, measure){
      ...
      var clrMode = (measure && measure.colorMode) ? String(measure.colorMode) : 'palette';
      if(clr && clrMode!=='custom') clrMode = 'custom';
      var ncols = (cols||[]).filter(function(c){ return c && c.kind==='number'; });
      if(!ncols.length) ncols = cols || [];
      return ''
        + '<div class="lbl">'+escapeHtml(label)+'</div>'
        + '<div class="row">'
        + '<select data-measure="'+escapeHtml(keyName)+'" data-part="col" style="min-width:170px">'
        + colOptionsHtml(ncols, colKey, true)
        + '</select>'
        + '<select data-measure="'+escapeHtml(keyName)+'" data-part="agg" style="min-width:112px">'
        + aggOptionsHtml(agg)
        + '</select>'
        + '<input data-measure="'+escapeHtml(keyName)+'" data-part="dec" type="number" min="" max="10" step="1" value="'+escapeHtml(String(dec))+'" placeholder="Dec" title="Decimal places (blank = auto)" style="width:46px" />'
        + '<select data-measure="'+escapeHtml(keyName)+'" data-part="colorMode" title="Color source" style="min-width:76px">'
        + '  <option value="palette" '+(clrMode==='palette'?'selected':'')+'>Palette</option>'
        + '  <option value="custom" '+(clrMode==='custom'?'selected':'')+'>Custom</option>'
        + '</select>'
        + '<input data-measure="'+escapeHtml(keyName)+'" data-part="color" type="color" value="'+(clr||'#2563eb')+'" title="Custom series color" style="width:32px;height:28px;padding:0;cursor:pointer;border:1px solid var(--vm-border);border-radius:4px;margin-left:4px;background:none" />'
        + '</div>';
    }
    ...
    reg('line', {
      ...
      defaultVisual:function(){
        return {
          ...
          options:{ style:'line', mode:'lines+markers', shape:'spline', dash:'solid', hovermode:'closest', connectgaps:false, xScrollbar:false, maxPoints:1500, perMeasureModes:false, style_y:'', style_y3:'', style_y4:'', trellisOn:false, trellisCols:3, statsMode:'none', includeOutliers:false, drillEnabled:false, drillLevel:0, crossFilterEmit:true, crossFilterApply:true, forecastMethod:'none', forecastPoints:0, whatIfPct:0, insightPanel:true, alertEnabled:false, alertOp:'gt', alertValue:'', semanticDateGrain:'auto', timeCalc:'none' }
        };
      },
      ...
      buildOptionsUI:function(v,ctx){
        return ''
          ...
          + '<option value="none" '+(String(v.options.statsMode||'none')==='none'?'selected':'')+'>None</option>'
          ...
          + '</select></div>'
          + '<div class="lbl">Advanced stats lines</div>'
          + '<div class="row"><label class="note"><input type="checkbox" data-opt="showStatsOverlay" '+(v.options.showStatsOverlay?'checked':'')+'> Show Mean/Median/CI/Regression overlays</label></div>'
          + '<div class="lbl">Drill Hierarchy</div>'
          + '<div class="row"><label class="note"><input type="checkbox" data-opt="drillEnabled" '+(v.options.drillEnabled?'checked':'')+'> Enable drill through selected X hierarchy columns (Shift+click on point)</label></div>'
          + '<div class="lbl">Cross-filtering</div>'
          + '<div class="row"><label class="note"><input type="checkbox" data-opt="crossFilterEmit" '+(v.options.crossFilterEmit!==false?'checked':'')+'> Emit filter on click to other visuals</label></div>'
          + '<div class="lbl">Forecast</div>'
          + '<div class="row" style="gap:8px">'
          + '<select data-opt="forecastMethod" style="min-width:140px"><option value="none" '+((v.options.forecastMethod||'none')==='none'?'selected':'')+'>None</option><option value="linear" '+(v.options.forecastMethod==='linear'?'selected':'')+'>Linear Trend</option></select>'
          + '<input data-opt="forecastPoints" type="number" min="0" max="48" step="1" value="'+escapeHtml(String(v.options.forecastPoints||0))+'" style="width:90px" />'
          + '<span class="note">points</span></div>'
          + '<div class="lbl">What-if (%)</div>'
          + '<div class="row"><input data-opt="whatIfPct" type="number" min="-95" max="500" step="1" value="'+escapeHtml(String(v.options.whatIfPct||0))+'" style="width:110px" /><span class="note">Adjust Y values for scenario comparison</span></div>'
          + '<div class="lbl">Semantic / Time Calc</div>'
          + '<div class="row" style="gap:8px">'
          + '<select data-opt="semanticDateGrain" style="min-width:120px"><option value="auto" '+((v.options.semanticDateGrain||'auto')==='auto'?'selected':'')+'>Date grain: Auto</option><option value="month" '+(v.options.semanticDateGrain==='month'?'selected':'')+'>Month</option><option value="quarter" '+(v.options.semanticDateGrain==='quarter'?'selected':'')+'>Quarter</option><option value="year" '+(v.options.semanticDateGrain==='year'?'selected':'')+'>Year</option></select>'
          + '<select data-opt="timeCalc" style="min-width:120px"><option value="none" '+((v.options.timeCalc||'none')==='none'?'selected':'')+'>Calc: None</option><option value="mom" '+(v.options.timeCalc==='mom'?'selected':'')+'>MoM %</option><option value="yoy" '+(v.options.timeCalc==='yoy'?'selected':'')+'>YoY %</option><option value="ytd" '+(v.options.timeCalc==='ytd'?'selected':'')+'>YTD</option></select>'
          + '</div>'
          + '<div class="lbl">Insight / Alerts</div>'
          + '<div class="row"><label class="note"><input type="checkbox" data-opt="insightPanel" '+(v.options.insightPanel!==false?'checked':'')+'> Show explain insights in statistics table</label></div>'
          + '<div class="row" style="gap:8px"><label class="note"><input type="checkbox" data-opt="alertEnabled" '+(v.options.alertEnabled?'checked':'')+'> Alert</label><select data-opt="alertOp" style="width:78px"><option value="gt" '+((v.options.alertOp||'gt')==='gt'?'selected':'')+'>></option><option value="lt" '+(v.options.alertOp==='lt'?'selected':'')+'><</option></select><input data-opt="alertValue" type="number" value="'+escapeHtml(String(v.options.alertValue||''))+'" style="width:90px" /></div>'
          + '</div>';
      },
      render:function(v, rows, ctx, el){
        var map = v.mapping||{};
        var xKeysAll = Array.isArray(map.x) ? map.x.filter(function(k){return !!k;}) : (map.x ? [map.x] : []);
        var drillEnabled = !!(v.options && v.options.drillEnabled);
        var drillLevel = clamp(parseInt((v.options&&v.options.drillLevel)||0,10)||0, 0, Math.max(0, xKeysAll.length-1));
        var xKeys = (drillEnabled && xKeysAll.length>1) ? [xKeysAll[drillLevel]] : xKeysAll.slice();
        ...
        if(v.options && Array.isArray(v.options.removedPoints) && v.options.removedPoints.length){
           ...
           dataRows = dataRows.filter(function(r){
             ...
             var keyAny = JSON.stringify([kx, ky, 0]);
             return !(rmSet.has(key) || rmSet.has(keyAny));
           });
        }
        ...
        var labelsOn = !!(v.options && (v.options.showLabels || v.options.labelsOn));
        var gapMode = (v.options && (v.options.gapsMode || v.options.gapMode)) ? String(v.options.gapsMode || v.options.gapMode) : 'break';
        ...
        Object.keys(bySeries).forEach(function(seriesName){
          ...
          function pushTrace(measureKey, label, ys, axis){
            ...
            if(st==='bar'){
              ...
              tr={
                ...
                x: xs,
                y: ys,
                connectgaps: (gapMode==='connect' || gapMode==='interp' || gapMode==='interpolate'),
            ...
            /* Apply color if set */
            var mColorMode = String((mObj && mObj.colorMode) || (mObj && mObj.color ? 'custom' : 'palette'));
            if(mColorMode==='custom' && mObj.color){
              tr.marker = tr.marker || {};
            ...
            var xHoverLabel = (v.options && v.options.xTitle) ? String(v.options.xTitle) : vmColLabel(ctx, xKey);
            var yHoverLabel = (v.options && v.options.yTitle) ? String(v.options.yTitle) : vmColLabel(ctx, (mObj.colKey||measureKey));
            var trHt = escapeHtml(xHoverLabel||'X')+': %{x}<br>'+escapeHtml(yHoverLabel||'Y')+': '+yFmt;
            if(useHover){
          ...
          var yLbl = hasSeriesField ? (showMeasureInName ? (seriesName + ' � ' + (yM.colKey||'Y')) : seriesName) : (showMeasureInName ? (yM.colKey||'Y') : (yM.colKey||'Y'));
          pushTrace('y', yLbl, base.m.y, 'y');
          if(hasY3) pushTrace('y3', hasSeriesField ? (seriesName + ' � ' + (y3M.colKey||'Y3')) : (y3M.colKey||'Y3'), base.m.y3, 'y');
          if(hasY4) pushTrace('y4', hasSeriesField ? (seriesName + ' � ' + (y4M.colKey||'Y4')) : (y4M.colKey||'Y4'), base.m.y4, 'y');
          if(hasY2) pushTrace('y2', hasSeriesField ? (seriesName + ' � ' + (y2M.colKey||'Y2')) : (y2M.colKey||'Y2'), base.m.y2, 'y2');
        });

        function vmApplyTimeCalcSeries(vals, mode){
          var out = vals.slice();
          if(mode==='none') return out;
          if(mode==='ytd'){
            var run=0; for(var iT=0;iT<out.length;iT++){ var nv=toNumber(out[iT]); run += (nv||0); out[iT]=run; }
            return out;
          }
          var lag = (mode==='yoy') ? 12 : 1;
          for(var iM=0;iM<out.length;iM++){
            var cur=toNumber(out[iM]); var prev=(iM-lag>=0)?toNumber(out[iM-lag]):null;
            if(cur==null || prev==null || Math.abs(prev)<1e-12) out[iM]=null;
            else out[iM]=((cur-prev)/Math.abs(prev))*100;
          }
          return out;
        }
        var timeCalcMode = String((v.options&&v.options.timeCalc)||'none');
        var whatIfPct = parseFloat(v.options&&v.options.whatIfPct); if(!isFinite(whatIfPct)) whatIfPct=0;
        var whatIfFactor = 1 + (whatIfPct/100);
        var forecastMethod = String((v.options&&v.options.forecastMethod)||'none');
        var forecastN = clamp(parseInt((v.options&&v.options.forecastPoints)||0,10)||0, 0, 48);
        var extraForecast=[];
        for(var tix=0;tix<traces.length;tix++){
          var trc = traces[tix];
          if(!Array.isArray(trc.y)) continue;
          if(timeCalcMode!=='none') trc.y = vmApplyTimeCalcSeries(trc.y, timeCalcMode);
          if(Math.abs(whatIfPct)>1e-9){ trc.y = trc.y.map(function(vv){ var n=toNumber(vv); return n==null?null:n*whatIfFactor; }); }
          if(forecastMethod==='linear' && forecastN>0 && trc.type==='scatter' && String(trc.mode||'').indexOf('lines')>=0){
            var yn = trc.y.map(toNumber).filter(function(n){return n!=null;});
            if(yn.length>=2){
              var n=yn.length, sx=0,sy=0,sxx=0,sxy=0;
              for(var fi=0;fi<n;fi++){ sx+=fi; sy+=yn[fi]; sxx+=fi*fi; sxy+=fi*yn[fi]; }
              var den=n*sxx-sx*sx;
              if(Math.abs(den)>1e-12){
                var b=(n*sxy-sx*sy)/den, a=(sy-b*sx)/n;
                var fx=[], fy=[];
                var lastX = Array.isArray(trc.x) && trc.x.length ? trc.x[trc.x.length-1] : 'X';
                for(var fp=1;fp<=forecastN;fp++){ fx.push(String(lastX)+'+F'+fp); fy.push(a+b*(n-1+fp)); }
                extraForecast.push({type:'scatter', mode:'lines', name:String(trc.name||'Series')+' Forecast', x:[trc.x[trc.x.length-1]].concat(fx), y:[trc.y[trc.y.length-1]].concat(fy), line:{dash:'dot', width:2, color:(trc.line&&trc.line.color)||'#6b7280'}});
              }
            }
          }
        }
        if(extraForecast.length) traces = traces.concat(extraForecast);

        var xAutoTitle = xKey ? vmColLabel(ctx, xKey) : '';
        var yAutoTitle = yM && yM.colKey ? vmColLabel(ctx, yM.colKey) : '';
        var layout = layoutBase({
          margin:{l:48,r:18,t:28,b:42},
          xaxis:{title:{text:(v.options && v.options.xTitle) ? String(v.options.xTitle) : xAutoTitle}},
          yaxis:{title:{text:(v.options && v.options.yTitle) ? String(v.options.yTitle) : yAutoTitle}}
        });
        ...
        var statsModes = [];
        if(v.options){
          if(Object.prototype.hasOwnProperty.call(v.options,'statsModes') && Array.isArray(v.options.statsModes)){
            statsModes = v.options.statsModes.filter(function(m){ return m && m !== 'none'; });
          }else if(v.options.statsMode && v.options.statsMode !== 'none'){
            statsModes = [String(v.options.statsMode)];
          }
          if(v.options.showStatsOverlay !== true) statsModes = [];
        }
        // Keep regression overlays explicit: outliers should not auto-enable regression.
        if(statsModes.length){
           try{
             ...
             if(allY.length > 1){
               ...
               var statsColors = (v.options && v.options.statsColors) ? v.options.statsColors : {};
               var meanColor = statsColors.mean || 'rgba(255,0,0,0.7)';
               var medianColor = statsColors.median || 'rgba(0,0,255,0.7)';
               var ciColor = statsColors.ci || 'rgba(0,128,0,0.8)';
               var regColor = statsColors.regression || 'orange';
               var outlierColor = statsColors.outlier || '#ef4444';

               if(statsModes.indexOf('mean')>=0 && traces.length && traces[0].x){
                  traces.push({type:'scatter',mode:'lines',name:'Mean',x:traces[0].x,y:traces[0].x.map(function(){return mean;}),line:{color:meanColor,width:2,dash:'dash'}});
               }
               if(statsModes.indexOf('median')>=0 && traces.length && traces[0].x){
                  traces.push({type:'scatter',mode:'lines',name:'Median',x:traces[0].x,y:traces[0].x.map(function(){return median;}),line:{color:medianColor,width:2,dash:'dot'}});
               }
               if(statsModes.indexOf('ci')>=0 && traces.length && traces[0].x){
                  var sq2 = 0; for(var kk2=0;kk2<allY.length;kk2++) sq2 += Math.pow(allY[kk2]-mean,2);
                  var sd2 = Math.sqrt(sq2 / allY.length);
                  var u2 = mean + 2*sd2, l2 = mean - 2*sd2;
                  traces.push({type:'scatter',mode:'lines',name:'CI Upper',x:traces[0].x,y:traces[0].x.map(function(){return u2;}),line:{color:ciColor,width:1,dash:'dot'},hoverinfo:'skip'});
                  traces.push({type:'scatter',mode:'lines',name:'CI Band',x:traces[0].x,y:traces[0].x.map(function(){return l2;}),line:{color:ciColor,width:1,dash:'dot'},fill:'tonexty',fillcolor:String(ciColor).indexOf('rgba(')===0?ciColor.replace('0.8','0.12'):'rgba(16,185,129,.12)',hoverinfo:'skip'});
               }
               if(statsModes.indexOf('regression')>=0){
                  ...
                  if(regPts.length > 2){
                     ...
                     traces.push({
                        ...
                        x: [minX, maxX], y: [y1, y2],
                        line: { color: regColor, width: 3, dash: 'dot' },
                        hoverinfo: 'skip'
                     });

                     if(v.options.regressionShowOutliers){
                       var method = String(v.options.outlierMethod || 'std');
                       var kVal = parseFloat(v.options.outlierK);
                       if(!isFinite(kVal) || kVal<=0) kVal = 2;
                       var residuals = regPts.map(function(p){ return p.y - (tm*p.x + tb); });
                       var outlierFlags = residuals.map(function(){ return false; });
                       if(method==='iqr'){
                         var rs = residuals.slice().sort(function(a,b){ return a-b; });
                         var q1 = rs[Math.floor((rs.length-1)*0.25)];
                         var q3 = rs[Math.floor((rs.length-1)*0.75)];
                         var iqr = q3 - q1;
                         var low = q1 - kVal*iqr;
                         var high = q3 + kVal*iqr;
                         for(var rr=0;rr<residuals.length;rr++) outlierFlags[rr] = (residuals[rr] < low || residuals[rr] > high);
                       }else{
                         var meanR = 0;
                         for(var mr=0;mr<residuals.length;mr++) meanR += residuals[mr];
                         meanR = residuals.length ? (meanR / residuals.length) : 0;
                         var absDev = [];
                         for(var ar=0;ar<residuals.length;ar++) absDev.push(Math.abs(residuals[ar] - meanR));
                         var scale = 0;
                         if(method==='mad'){
                           absDev.sort(function(a,b){ return a-b; });
                           scale = absDev[Math.floor(absDev.length/2)] * 1.4826;
                         }else{
                           var sq=0;
                           for(var sr=0;sr<residuals.length;sr++) sq += Math.pow(residuals[sr]-meanR,2);
                           scale = Math.sqrt(sq / Math.max(1,residuals.length));
                         }
                         var thr = kVal * (scale || 1e-9);
                         for(var trr=0;trr<residuals.length;trr++) outlierFlags[trr] = Math.abs(residuals[trr]-meanR) > thr;
                       }
                       var ox=[], oy=[];
                       for(var op=0;op<regPts.length;op++){
                         if(outlierFlags[op]){ ox.push(regPts[op].x); oy.push(regPts[op].y); }
                       }
                       if(ox.length){
                         traces.push({
                           type:'scatter',
                           mode:'markers',
                           name:'Regression Outliers',
                           x:ox,
                           y:oy,
                           marker:{size:8,color:outlierColor,line:{width:1,color:'#ffffff'}},
                           hovertemplate:'Outlier<br>X: %{x}<br>Y: %{y}<extra></extra>'
                         });
                       }
                     }
                  }
          ...
          // Targets are rendered as layout shapes only to avoid legend clutter and unintended autoscale effects.
...
if(v.title){ layout.margin.t = 4; }
    ...
    reg('box', {
      ...
      defaultVisual:function(){
        return {
          ...
          filter: defaultFilter(),
          options:{ view:'box', points:'outliers', targetsEnabled:false, targets:'', boxMeanLine:false, boxNotch:false, boxJitter:0.3, violinSide:'both' }
        };
      },
      buildMappingUI:function(v, ctx){
        var cols=ctx.cols;
      ...
      buildOptionsUI:function(v){
        return ''
          ...
          + '<option value="false" '+(String(v.options.points)==='false'?'selected':'')+'>None</option>'
          + '</select></div>'
          + '<div class="lbl">Mean line</div>'
          + '<div><label class="note"><input type="checkbox" data-opt="boxMeanLine" '+(v.options.boxMeanLine?'checked':'')+'> Show mean line</label></div>'
          + '<div class="lbl">Notched box</div>'
          + '<div><label class="note"><input type="checkbox" data-opt="boxNotch" '+(v.options.boxNotch?'checked':'')+'> Enable notch</label></div>'
          + '<div class="lbl">Point jitter</div>'
          + '<div><input data-opt="boxJitter" type="number" step="0.05" min="0" max="1" value="'+escapeHtml(String(v.options.boxJitter!=null?v.options.boxJitter:0.3))+'" style="width:90px" /></div>'
          + '<div class="lbl">Violin side</div>'
          + '<div><select data-opt="violinSide"><option value="both" '+((v.options.violinSide||'both')==='both'?'selected':'')+'>Both</option><option value="positive" '+(v.options.violinSide==='positive'?'selected':'')+'>Positive</option><option value="negative" '+(v.options.violinSide==='negative'?'selected':'')+'>Negative</option></select></div>'
      ...
      render:function(v, rows, ctx, el){
        ...
        var boxMeanLine = !!(v.options && v.options.boxMeanLine);
        var boxNotch = !!(v.options && v.options.boxNotch);
        var boxJitter = parseFloat(v.options && v.options.boxJitter);
        if(!isFinite(boxJitter) || boxJitter<0) boxJitter = 0.3;
        if(boxJitter>1) boxJitter = 1;
        var violinSide = (v.options && v.options.violinSide) ? String(v.options.violinSide) : 'both';
        if(['both','positive','negative'].indexOf(violinSide)<0) violinSide='both';
        var traces=[];
        ...
        Object.keys(byGroup).forEach(function(name){
          ...
          if(view==='violin'){
            traces.push({type:'violin', name:name, y:vals, side:violinSide, points: pts, box:{visible:true}, meanline:{visible:boxMeanLine}});
            return;
          }
          ...
          if(view==='raincloud'){
            traces.push({
              ...
              name:name,
              y:vals,
              side:violinSide==='both'?'positive':violinSide,
              points: (pts===false) ? false : 'all',
              jitter: boxJitter,
              pointpos: 0,
              box:{visible:true},
              meanline:{visible:boxMeanLine}
            });
            return;
          }
          ...
          if(view==='strip' || view==='swarm'){
            ...
            traces.push({
              ...
              jitter: Math.max(jit, boxJitter),
              pointpos: 0,
              boxmean: boxMeanLine ? true : false,
              notched: boxNotch,
          ...
          // default: box
          traces.push({type:'box', name:name, y:vals, boxpoints: pts, notched:boxNotch, boxmean:(boxMeanLine?true:false), jitter:boxJitter});
        });
        ...
        if(v.options && v.options.trellisOn && seriesKey && traces.length>1){
          var cols = clamp(parseInt(v.options.trellisCols||3,10)||3,1,4);
          var groups=[];
          function gName(nm){ var p=String(nm||'').split(' • '); return p[0]||String(nm||''); }
          traces.forEach(function(t){ var g=gName(t.name); if(groups.indexOf(g)<0) groups.push(g); });
          var rowsN = Math.max(1, Math.ceil(groups.length/cols));
          layout.grid = {rows:rowsN, columns:cols, pattern:'independent'};
          traces.forEach(function(t){
            var gi = groups.indexOf(gName(t.name));
            var ax = gi+1; var sx = (ax===1)?'':''+ax;
            t.xaxis = 'x'+sx; t.yaxis='y'+sx;
          });
        }
        if(v.title){ layout.margin = layout.margin || {}; layout.margin.t = 4; }
        Plotly.newPlot(el, traces, layout, plotlyCfg(v));
    ...
    reg('scatter', {
      ...
      buildOptionsUI:function(v){
        return ''
          ...
          + '<div class="lbl">Statistics overlays</div>'
          + '<div class="row"><label class="note"><input type="checkbox" data-opt="showStatsOverlay" '+(v.options.showStatsOverlay?'checked':'')+'> Show mean/median/CI/regression lines</label></div>'
          + '</div>';
      },
      render:function(v, rows, ctx, el){
        var map=v.mapping||{};
        ...
        var filtered=applyFilter(rows, v.filter);
        var xKey=map.x, yKey=map.y, y2Key=(map.y2||''), cKey=map.color||'';

        var hoverKeys = Array.isArray(map.hoverCols) ? map.hoverCols.filter(function(k){return !!k;}) : [];
        var useHover = (hoverKeys.length>0);
        var xHoverLabel = (v.options && v.options.xTitle) ? String(v.options.xTitle) : vmColLabel(ctx, xKey);
        var yHoverLabel = (v.options && v.options.yTitle) ? String(v.options.yTitle) : vmColLabel(ctx, yKey);
        var hoverTemplate = null;
        if(useHover){
          var ht=escapeHtml(xHoverLabel)+': %{x}<br>'+escapeHtml(yHoverLabel)+': %{y}';
          for(var hk=0;hk<hoverKeys.length;hk++) ht += '<br>'+escapeHtml(hoverKeys[hk])+': %{customdata['+hk+']}';
          ht += '<extra></extra>';
          hoverTemplate = ht;
        }
        ...
        if(cKey){
          ...
          Object.keys(by).forEach(function(name){
            ...
            palIdx++;
            if(useHover){ tr.customdata = by[name].cd; tr.hovertemplate = hoverTemplate; } else { tr.hovertemplate = escapeHtml(xHoverLabel)+': %{x}<br>'+escapeHtml(yHoverLabel)+': %{y}<extra>%{fullData.name}</extra>'; }
            traces.push(tr);
          });
        }else{
          var xs=[], ys=[], cds=[], cfBgs=[];
          ...
          if(useHover){ tr0.customdata = cds; tr0.hovertemplate = hoverTemplate; } else { tr0.hovertemplate = escapeHtml(xHoverLabel)+': %{x}<br>'+escapeHtml(yHoverLabel)+': %{y}<extra>%{fullData.name}</extra>'; }
          traces.push(tr0);
        }

        // Optional second Y measure plotted against same X
        if(y2Key){
          var ys2=[];
          var xs2=[];
          for(var j2=0;j2<dataRows.length;j2++){
            xs2.push(toNumber(dataRows[j2][xKey]));
            ys2.push(toNumber(dataRows[j2][y2Key]));
          }
          var tr2={type:pickScatterType(xs2.length), mode:v.options.mode||'markers', name:(y2Key||'Y2'), x:xs2, y:ys2};
          var pal2 = vmGetPalette(v);
          if((!tr2.marker || !tr2.marker.color) && pal2.length){ tr2.marker = tr2.marker || {}; tr2.marker.color = pal2[Math.min(1,pal2.length-1)]; }
          tr2.hovertemplate = escapeHtml(xHoverLabel)+': %{x}<br>' + escapeHtml(vmColLabel(ctx, y2Key)) + ': %{y}<extra>%{fullData.name}</extra>';
          traces.push(tr2);
        }

        // Scatter statistics overlays (per series or combined, with axis controls)
        var sm = [];
        if(v.options){
          if(Array.isArray(v.options.statsModes)) sm = v.options.statsModes.filter(function(m){ return m && m!=='none'; });
          else if(v.options.statsMode && v.options.statsMode!=='none') sm = [String(v.options.statsMode)];
          // Keep regression overlays explicit: outliers should not auto-enable regression.
          if(v.options.showStatsOverlay !== true) sm = [];
        }
        if(sm.length){
          var sc = (v.options && v.options.statsColors) ? v.options.statsColors : {};
          var cMean = sc.mean || '#ff0000', cMedian = sc.median || '#0000ff', cCI = sc.ci || '#008000', cReg = sc.regression || '#ffa500', cOut = sc.outlier || '#ef4444';
          var statsMode = String((v.options && v.options.scatterStatsMode) || 'per-series');
          var axisStats = String((v.options && v.options.scatterAxisStats) || 'y');
          var regAxes = String((v.options && v.options.scatterRegressionAxes) || 'y');

          function buildPtsFromTrace(trBase){
            if(!trBase || !trBase.x || !trBase.y || String(trBase.mode||'').indexOf('markers')<0) return [];
            var pts=[];
            for(var pi=0; pi<trBase.x.length; pi++){
              var xv=Number(trBase.x[pi]), yv=Number(trBase.y[pi]);
              if(isFinite(xv) && isFinite(yv)) pts.push({x:xv,y:yv});
            }
            return pts;
          }

          function addStatsForPts(pts, nameBase){
            if(!pts || pts.length<2) return;
            var xs = pts.map(function(p){return p.x;}).slice().sort(function(a,b){return a-b;});
            var ys = pts.map(function(p){return p.y;}).slice().sort(function(a,b){return a-b;});
            var sumX=0,sumY=0; for(var i=0;i<pts.length;i++){ sumX+=pts[i].x; sumY+=pts[i].y; }
            var meanX=sumX/pts.length, meanY=sumY/pts.length;
            ...
            if(sm.indexOf('mean')>=0){
              traces.push({type:'scatter',mode:'lines',name:nameBase+' Mean Y',x:[minX,maxX],y:[meanY,meanY],line:{color:cMean,width:2,dash:'dash'}});
              if(axisStats==='xy') traces.push({type:'scatter',mode:'lines',name:nameBase+' Mean X',x:[meanX,meanX],y:[minY,maxY],line:{color:cMean,width:2,dash:'dash'}});
            }
            if(sm.indexOf('median')>=0){
              traces.push({type:'scatter',mode:'lines',name:nameBase+' Median Y',x:[minX,maxX],y:[medY,medY],line:{color:cMedian,width:2,dash:'dot'}});
              if(axisStats==='xy') traces.push({type:'scatter',mode:'lines',name:nameBase+' Median X',x:[medX,medX],y:[minY,maxY],line:{color:cMedian,width:2,dash:'dot'}});
            }
            if(sm.indexOf('ci')>=0){
              var varSum=0; for(var vi=0; vi<pts.length; vi++) varSum += Math.pow(pts[vi].y-meanY,2);
              var std = Math.sqrt(varSum/Math.max(1,pts.length));
              var up = meanY + 2*std, lo = meanY - 2*std;
              traces.push({type:'scatter',mode:'lines',name:nameBase+' CI Upper',x:[minX,maxX],y:[up,up],line:{color:cCI,width:1,dash:'dot'},hoverinfo:'skip'});
              traces.push({type:'scatter',mode:'lines',name:nameBase+' CI Band',x:[minX,maxX],y:[lo,lo],line:{color:cCI,width:1,dash:'dot'},fill:'tonexty',fillcolor:'rgba(16,185,129,.12)',hoverinfo:'skip'});
            }
            if(sm.indexOf('regression')>=0){
              var n=0,sx=0,sy=0,sxx=0,sxy=0;
              for(var rk=0; rk<pts.length; rk++){ n++; sx+=pts[rk].x; sy+=pts[rk].y; sxx+=pts[rk].x*pts[rk].x; sxy+=pts[rk].x*pts[rk].y; }
              var den=(n*sxx-sx*sx);
              if(Math.abs(den)>1e-12){
                var b=(n*sxy-sx*sy)/den, a=(sy-b*sx)/n;
                traces.push({type:'scatter',mode:'lines',name:nameBase+' Regression Y~X',x:[minX,maxX],y:[a+b*minX,a+b*maxX],line:{color:cReg,width:2,dash:'dot'}});
                if(regAxes==='both'){
                  var by=0, ay=0, syy=0, syx=0;
                  for(var rk2=0; rk2<pts.length; rk2++){ syy += pts[rk2].y*pts[rk2].y; syx += pts[rk2].y*pts[rk2].x; }
                  var den2=(n*syy-sy*sy);
                  if(Math.abs(den2)>1e-12){ by=(n*syx-sy*sx)/den2; ay=(sx-by*sy)/n; traces.push({type:'scatter',mode:'lines',name:nameBase+' Regression X~Y',x:[ay+by*minY,ay+by*maxY],y:[minY,maxY],line:{color:cReg,width:2,dash:'dash'}}); }
                }
                if(v.options && v.options.regressionShowOutliers){
                  var method=String(v.options.outlierMethod||'std');
                  var kVal=parseFloat(v.options.outlierK); if(!isFinite(kVal)||kVal<=0) kVal=2;
                  var residuals=pts.map(function(p){ return p.y-(a+b*p.x); });
                  var flags=residuals.map(function(){ return false; });
                  if(method==='iqr'){
                    var rs=residuals.slice().sort(function(a,b){return a-b;});
                    var q1=rs[Math.floor((rs.length-1)*0.25)], q3=rs[Math.floor((rs.length-1)*0.75)], iqr=q3-q1;
                    var low=q1-kVal*iqr, high=q3+kVal*iqr;
                    for(var rr=0; rr<residuals.length; rr++) flags[rr]=(residuals[rr]<low||residuals[rr]>high);
                  }else{
                    var mr=0; for(var mri=0;mri<residuals.length;mri++) mr+=residuals[mri]; mr = residuals.length?mr/residuals.length:0;
                    var scale=0;
                    if(method==='mad'){ var ad=[]; for(var ai=0;ai<residuals.length;ai++) ad.push(Math.abs(residuals[ai]-mr)); ad.sort(function(a,b){return a-b;}); scale = ad[Math.floor(ad.length/2)]*1.4826; }
                    else { var sq=0; for(var qi=0;qi<residuals.length;qi++) sq += Math.pow(residuals[qi]-mr,2); scale = Math.sqrt(sq/Math.max(1,residuals.length)); }
                    var thr=kVal*(scale||1e-9);
                    for(var fi=0;fi<residuals.length;fi++) flags[fi]=Math.abs(residuals[fi]-mr)>thr;
                  }
                  var ox=[],oy=[];
                  for(var oi=0;oi<pts.length;oi++){ if(flags[oi]){ ox.push(pts[oi].x); oy.push(pts[oi].y); } }
                  if(ox.length) traces.push({type:'scatter',mode:'markers',name:nameBase+' Regression Outliers',x:ox,y:oy,marker:{size:8,color:cOut,line:{width:1,color:'#fff'}}});
                }
              }
            }
          }
          ...
          if(statsMode==='combined'){
            var allPts=[];
            for(var ti=0; ti<traces.length; ti++) allPts = allPts.concat(buildPtsFromTrace(traces[ti]));
            addStatsForPts(allPts, 'Combined');
          }else{
            for(var ti2=0; ti2<traces.length; ti2++){
              var trBase = traces[ti2];
              var pts2 = buildPtsFromTrace(trBase);
              var nm = trBase && trBase.name ? String(trBase.name) : ('Series '+(ti2+1));
              addStatsForPts(pts2, nm);
            }
          }
        }
        ...
        var layout = layoutBase({
          ...
          xaxis:{title:{text:(v.options && v.options.xTitle) ? String(v.options.xTitle) : vmColLabel(ctx, xKey)}},
          yaxis:{title:{text:(v.options && v.options.yTitle) ? String(v.options.yTitle) : vmColLabel(ctx, yKey)}}
        });
        try{ vmAddTargetsToLayout(layout, v, ctx, dataRows); }catch(_){ }
        try{ vmAddReferenceLinesToLayout(layout, v); }catch(_){ }
        if(v.title){ layout.margin.t = 4; }
      ...
      render:function(v, rows, ctx, el){
        ...
        if(seriesKey){
          ...
          Object.keys(by).forEach(function(name){
            ...
            if(useHover){ tr.customdata = by[name].cd; tr.hovertemplate = hoverTemplate; } else { tr.hovertemplate = 'Value: %{x}<br>Count: %{y}<extra>%{fullData.name}</extra>'; }
            traces.push(tr);
          });
        }else{
          var xs=[], cds=[];
          ...
          if(useHover){ tr0.customdata = cds; tr0.hovertemplate = hoverTemplate; } else { tr0.hovertemplate = 'Value: %{x}<br>Count: %{y}<extra>%{fullData.name}</extra>'; }
          traces.push(tr0);
        }
    ...
    reg('table', {
      ...
      defaultVisual:function(){
        return {
          ...
          transform: defaultTransform('raw'),
          filter: defaultFilter(),
          options:{ limit: 0, wrap:false, colWidths:{}, cfRules:[], striped:true, compact:false, headerBg:'', headerColor:'' }
        };
      },
      buildMappingUI:function(v, ctx){
        v.mapping = v.mapping || {};
      ...
      buildOptionsUI:function(v, ctx){
        ...
        out += '<div class="form">'
          ...
          + '<div class="row">'
          + '<input data-opt="limit" type="number" min="0" max="5000" step="10" value="'+escapeHtml(String(v.options.limit==null?0:v.options.limit))+'" style="width:140px" />'
          ...
          + '  <label class="note"><input data-opt="fixedFirstCol" type="checkbox" '+(v.options.fixedFirstCol?'checked':'')+'> Fixed first column (with horizontal scroll)</label>'
          + '  <label class="note"><input data-opt="showColTotals" type="checkbox" '+(v.options.showColTotals?'checked':'')+'> Column totals</label>'
          + '  <label class="note"><input data-opt="showRowTotals" type="checkbox" '+(v.options.showRowTotals?'checked':'')+'> Row totals</label>'
          + '</div>'
          + '<div class="lbl" style="margin-top:8px">Hyperlinks</div>'
          + '<div class="row" style="gap:8px;flex-wrap:wrap">'
          + '  <select data-opt="linkCol" style="min-width:180px"><option value="">No hyperlink column</option>'+colOptionsHtml(cols, v.options.linkCol, true)+'</select>'
          + '  <input data-opt="linkTemplate" placeholder="URL template (use {value})" value="'+escapeHtml(String(v.options.linkTemplate||''))+'" style="min-width:260px" />'
          + '  <select data-opt="linkTarget" style="min-width:120px"><option value="_blank" '+((v.options.linkTarget||'_blank')==='_blank'?'selected':'')+'>New tab</option><option value="_self" '+((v.options.linkTarget||'_blank')==='_self'?'selected':'')+'>Same tab</option></select>'
          + '</div>';
      ...
      render:function(v, rows, ctx, el){
        ...
        var limitRaw = parseInt((v.options||{}).limit==null?0:(v.options||{}).limit, 10);
        if(!isFinite(limitRaw)) limitRaw = 0;
        ...
        var cfRules = Array.isArray(v.options.cfRules) ? v.options.cfRules : [];
        var cdec = (v.options && v.options.colDecimals && typeof v.options.colDecimals==='object') ? v.options.colDecimals : {};
        var cdate = (v.options && v.options.colDateFmts && typeof v.options.colDateFmts==='object') ? v.options.colDateFmts : {};
        var cfont = (v.options && v.options.colFontPx && typeof v.options.colFontPx==='object') ? v.options.colFontPx : {};
        function fmtDateBy(val, fmt){
          var d = new Date(val);
          if(!d || isNaN(d.getTime())) return asString(val);
          var y=d.getFullYear(), m=('0'+(d.getMonth()+1)).slice(-2), da=('0'+d.getDate()).slice(-2);
          var mons=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
          if(fmt==='MM/DD/YYYY') return m+'/'+da+'/'+y;
          if(fmt==='DD-MMM-YYYY') return da+'-'+mons[d.getMonth()]+'-'+y;
          return y+'-'+m+'-'+da;
        }
        function fmtCellVal(k, val){
          if(val==null) return '';
          var dn = (cdec[k]!=null && cdec[k]!=='') ? parseInt(cdec[k],10) : null;
          var n = toNumber(val);
          if(isFinite(n) && dn!=null && isFinite(dn)) return Number(n).toFixed(Math.max(0,Math.min(10,dn)));
          var df = String(cdate[k]||'');
          if(df) return fmtDateBy(val, df);
          return asString(val);
        }
        ...
        out += '<table class="vm-table'+(compact?' vm-table-compact':'')+(v.options&&v.options.fixedFirstCol?' vm-table-fixed-first':'')+'" data-vid="'+escapeHtml(v.id)+'" data-table-sort-col="'+escapeHtml(sortCol)+'" data-table-sort-dir="'+escapeHtml(sortDir)+'">';
        // THEAD
        var thStyle = '';
        ...
        if(v.options && v.options.showRowTotals) out += '<th>Row Total</th>';
        out += '</tr></thead><tbody>';
        ...
        for(var i=0;i<n;i++){
          ...
          for(var j=0;j<cols.length;j++){
            ...
            var fpx = (cfont[k]!=null && cfont[k]!=='' ) ? parseInt(cfont[k],10) : 0;
            if(fpx>0) s += 'font-size:'+fpx+'px;';
            ...
            var stAll = s ? (' style="'+s+'"') : '';
            var cellContent = escapeHtml(fmtCellVal(k, r[k]));
            var __nr=toNumber(r[k]); if(__nr!=null) __rowNums.push(__nr);
            var _lc = String(v.options && v.options.linkCol || '');
            var _lt = String(v.options && v.options.linkTemplate || '');
            if(_lc && _lt && k===_lc){
              var _rv = (r[k]==null?'':String(r[k]));
              var _url = _lt.indexOf('{value}')>=0 ? _lt.split('{value}').join(encodeURIComponent(_rv)) : (_lt + encodeURIComponent(_rv));
              cellContent = '<a href="'+escapeAttr(_url)+'" target="'+escapeAttr(String((v.options&&v.options.linkTarget)||'_blank'))+'" rel="noopener noreferrer">'+cellContent+'</a>';
            }
            if(cfMatch){
          ...
          if(v.options && v.options.showRowTotals){ var __rt=applyAgg('sum',__rowNums); out += '<td style="font-weight:700">'+escapeHtml(__rt==null?'':__rt)+'</td>'; }
          out += '</tr>';
        }
        if(v.options && v.options.showColTotals){
          var cagg=(v.options.colAggs&&typeof v.options.colAggs==='object')?v.options.colAggs:{};
          out += '<tr style="font-weight:700;background:rgba(2,132,199,.08)">';
          for(var tj=0;tj<cols.length;tj++){ var tk=String(cols[tj]||''); var av=[]; for(var tr=0;tr<n;tr++) av.push(filtered[tr]?filtered[tr][tk]:null); var tv=applyAgg(String(cagg[tk]||'none'),av); out += '<td>'+escapeHtml(tv==null?'':fmtCellVal(tk,tv))+'</td>'; }
          if(v.options.showRowTotals) out += '<td></td>';
          out += '</tr>';
        }
        out += '</tbody></table></div>';
    ...
    reg('staticText', {
      ...
      implemented:true,
      defaultVisual:function(){
        return {id:uid(), type:'staticText', title:'Static Text', show:true,
          mapping:{}, measures:{}, transform:defaultTransform('raw'), filter:defaultFilter(),
          options:{text:'', hyperlink:'', openInNewTab:true, textAlign:'left', fontSize:14}};
      },
      buildMappingUI:function(){ return '<div class="note">No data mapping required.</div>'; },
      buildOptionsUI:function(v){
        return '<div class="form">'
          + '<div class="lbl">Text</div><div><textarea data-opt="text" style="min-height:90px" placeholder="Enter text">'+escapeHtml(String(v.options.text||''))+'</textarea></div>'
          + '<div class="lbl">Hyperlink (optional)</div><div><input data-opt="hyperlink" value="'+escapeHtml(String(v.options.hyperlink||''))+'" placeholder="/path/or/full-url" /></div>'
          + '<div class="lbl">Style</div><div class="row" style="gap:8px;align-items:center;flex-wrap:wrap">'
          + '<select data-opt="textAlign"><option value="left" '+((v.options.textAlign||'left')==='left'?'selected':'')+'>Left</option><option value="center" '+((v.options.textAlign||'left')==='center'?'selected':'')+'>Center</option><option value="right" '+((v.options.textAlign||'left')==='right'?'selected':'')+'>Right</option></select>'
          + '<input data-opt="fontSize" type="number" min="10" max="48" step="1" value="'+escapeHtml(String(v.options.fontSize||14))+'" style="width:90px" />'
          + '<label class="note"><input data-opt="openInNewTab" type="checkbox" '+(v.options.openInNewTab!==false?'checked':'')+'> Open link in new tab</label>'
          + '</div></div>';
      },
      render:function(v,rows,ctx,el){
        var txt = String((v.options&&v.options.text)||'').trim();
        var link = String((v.options&&v.options.hyperlink)||'').trim();
        var align = String((v.options&&v.options.textAlign)||'left');
        var fs = clamp(parseInt((v.options&&v.options.fontSize)||14,10)||14,10,48);
        var body = txt ? escapeHtml(txt).replace(/\n/g,'<br>') : '<span class="note">No text configured.</span>';
        if(link && txt){
          var t = (v.options&&v.options.openInNewTab)!==false ? '_blank' : '_self';
          body = '<a href="'+escapeAttr(link)+'" target="'+escapeAttr(t)+'" rel="noopener noreferrer">'+body+'</a>';
        }else if(link && !txt){
          var t2 = (v.options&&v.options.openInNewTab)!==false ? '_blank' : '_self';
          body = '<a href="'+escapeAttr(link)+'" target="'+escapeAttr(t2)+'" rel="noopener noreferrer">'+escapeHtml(link)+'</a>';
        }
        el.innerHTML = '<div style="padding:12px;line-height:1.45;text-align:'+escapeAttr(align)+';font-size:'+fs+'px;">'+body+'</div>';
      }
    });
    ...
    reg('heatmap', {
      ...
      render:function(v, rows, ctx, el){
        var xKey=firstKey(v.mapping.x), yKey=firstKey(v.mapping.y);
    ...
    reg('donut', {
      ...
      render:function(v,rows,ctx,el){
        ...
        var filtered=applyFilter(rows,v.filter);
        var lim=parseInt((v.options&&v.options.limit)==null?0:(v.options&&v.options.limit),10);
        if(!isFinite(lim)) lim=0; lim=clamp(lim,0,5000); if(lim>0 && filtered.length>lim) filtered=filtered.slice(0,lim);
    ...
    reg('funnel', {
      ...
      render:function(v,rows,ctx,el){
        ...
        var stageKey = firstKey(m.stage);
        if(!stageKey||!vm.colKey){renderEmptyWithModebar(el, 'Map Stage and Value.', v);return;}
        var filtered=applyFilter(rows,v.filter);
        var data=aggregateRows(filtered,{mode:'aggregate',groupBy:[stageKey],measures:[{colKey:vm.colKey,agg:vm.agg,asKey:'__v'}]},[]);
        var labels=[],vals=[];
        for(var i=0;i<data.length;i++){labels.push(asString(data[i][stageKey]));vals.push(toNumber(data[i].__v)||0);}
    ...
    reg('sunburst', {
      ...
      render:function(v,rows,ctx,el){
        var m=v.mapping||{};var vm=(v.measures||{}).value||{};
        var labelsKey = firstKey(m.labels);
        var parentsKey = firstKey(m.parents);
        if(!labelsKey||!vm.colKey){renderEmptyWithModebar(el, 'Map Labels and Value.', v);return;}
        var filtered=applyFilter(rows,v.filter);
        var data=aggregateRows(filtered,{mode:'aggregate',groupBy:[labelsKey,parentsKey].filter(Boolean),measures:[{colKey:vm.colKey,agg:vm.agg,asKey:'__v'}]},[]);
        var labels=[],parents=[],vals=[];
        for(var i=0;i<data.length;i++){
          labels.push(asString(data[i][labelsKey]));
          parents.push(parentsKey?asString(data[i][parentsKey]):'');
    ...
    reg('stackedBar', {
      ...
      defaultVisual:function(){
        return {id:uid(), type:'stackedBar', title:'Stacked Bar', show:true,
          mapping:{x:'',series:[]}, measures:{y:{colKey:'',agg:'none'}},
          transform:defaultTransform('aggregate'), filter:defaultFilter(), options:{orientation:'v',normalize:false,viewType:'vstack'}};
      },
      buildMappingUI:function(v,ctx){
        ensureMeasure(v,'y');
        return '<div class="form">'
      ...
      buildOptionsUI:function(v){
        return '<div class="form">'
          + '<div class="lbl">Type / View</div>'
          + '<div><select data-opt="viewType">'
          + '<option value="vbar" '+((v.options.viewType||'vstack')==='vbar'?'selected':'')+'>Vertical Bar</option>'
          + '<option value="hbar" '+((v.options.viewType||'vstack')==='hbar'?'selected':'')+'>Horizontal Bar</option>'
          + '<option value="vstack" '+((v.options.viewType||'vstack')==='vstack'?'selected':'')+'>Vertical Stacked Bar</option>'
          + '<option value="hstack" '+((v.options.viewType||'vstack')==='hstack'?'selected':'')+'>Horizontal Stacked Bar</option>'
          + '<option value="v100" '+((v.options.viewType||'vstack')==='v100'?'selected':'')+'>100% Vertical Stacked</option>'
          + '<option value="h100" '+((v.options.viewType||'vstack')==='h100'?'selected':'')+'>100% Horizontal Stacked</option>'
          + '</select></div>'
      ...
      render:function(v,rows,ctx,el){
        ...
        for(var i=0;i<data.length;i++){var r=data[i];var name=seriesKey?asString(r[seriesKey]):'Series';(bySeries[name]=bySeries[name]||{x:[],y:[]}).x.push(r[m.x]);bySeries[name].y.push(toNumber(r.__y)||0);}
        var traces=[];
        var vt=String((v.options&&v.options.viewType)||'vstack');
        var horiz=(vt==='hbar'||vt==='hstack'||vt==='h100') || (v.options.orientation==='h');
        var isStack=(vt==='vstack'||vt==='hstack'||vt==='v100'||vt==='h100');
        var is100=(vt==='v100'||vt==='h100') || !!v.options.normalize;
        Object.keys(bySeries).forEach(function(name){
          var d=bySeries[name];
        ...
        var layout=layoutBase({barmode:isStack?'stack':'group',barnorm:is100?'percent':undefined,margin:{l:horiz?110:48,r:18,t:4,b:42},
          xaxis:{title:{text:horiz?vmColLabel(ctx,yM.colKey||''):vmColLabel(ctx,m.x)}},
          yaxis:{title:{text:horiz?vmColLabel(ctx,m.x):vmColLabel(ctx,yM.colKey||'')}}});
        if(v.title){layout.margin.t=4;}
        Plotly.newPlot(el,traces,layout,plotlyCfg(v));
        bindLegendHover(el);
      }
    ...
    reg('lineBar', {
      ...
      defaultVisual:function(){
        return {id:uid(), type:'lineBar', title:'Line + Bar', show:true,
          mapping:{x:''}, measures:{bar:{colKey:'',agg:'none'},line:{colKey:'',agg:'none'},line2:{colKey:'',agg:'none'},line3:{colKey:'',agg:'none'},line4:{colKey:'',agg:'none'},line5:{colKey:'',agg:'none'},line6:{colKey:'',agg:'none'}},
          transform:defaultTransform('aggregate'), filter:defaultFilter(), options:{barOnY2:true}};
      },
      buildMappingUI:function(v,ctx){
        ensureMeasure(v,'bar');ensureMeasure(v,'line');
        ensureMeasure(v,'line2');ensureMeasure(v,'line3');ensureMeasure(v,'line4');ensureMeasure(v,'line5');ensureMeasure(v,'line6');
        return '<div class="form">'
          + mappingField('X','x',ctx.cols,v.mapping.x,true)
          + measureField('Bar measure (Y2 axis)','bar',ctx.cols,v.measures.bar)
          + measureField('Line 1 (Y1 axis)','line',ctx.cols,v.measures.line)
          + measureField('Line 2 (optional)','line2',ctx.cols,v.measures.line2)
          + measureField('Line 3 (optional)','line3',ctx.cols,v.measures.line3)
          + measureField('Line 4 (optional)','line4',ctx.cols,v.measures.line4)
          + measureField('Line 5 (optional)','line5',ctx.cols,v.measures.line5)
          + measureField('Line 6 (optional)','line6',ctx.cols,v.measures.line6)
          + '</div>';
      },
      buildOptionsUI:function(v){
        return '<div class="form">'
          + '<div class="lbl">Bar axis</div>'
          + '<div><select data-opt="barOnY2"><option value="1" '+((v.options&&v.options.barOnY2!==false)?'selected':'')+'>Y2 (right)</option><option value="0" '+((v.options&&v.options.barOnY2===false)?'selected':'')+'>Y1 (left)</option></select></div>'
          + '<div class="note" style="grid-column:1/-1">Supports up to 6 trend lines on Y1 and one bar series on Y2.</div>'
          + '</div>';
      },
      render:function(v,rows,ctx,el){
        var m=v.mapping||{}; var ms=(v.measures||{});
        var bM=ms.bar||{};
        var lineKeys=['line','line2','line3','line4','line5','line6'];
        var lineMs=[]; for(var li=0; li<lineKeys.length; li++){ var lm=ms[lineKeys[li]]||{}; if(lm.colKey) lineMs.push({key:lineKeys[li], m:lm}); }
        if(!m.x || (!bM.colKey && !lineMs.length)){ renderEmptyWithModebar(el, 'Map X and at least one measure.', v); return; }
        var filtered=applyFilter(rows,v.filter);
        var meas=[];
        if(bM.colKey) meas.push({colKey:bM.colKey,agg:bM.agg,asKey:'__bar'});
        for(var mj=0;mj<lineMs.length;mj++) meas.push({colKey:lineMs[mj].m.colKey,agg:lineMs[mj].m.agg,asKey:'__'+lineMs[mj].key});
        var data=aggregateRows(filtered,{mode:'aggregate',groupBy:[m.x],measures:meas},[]);
        var xs=[]; for(var i=0;i<data.length;i++) xs.push(asString(data[i][m.x]));
        var traces=[]; var barOnY2 = !(v.options && (v.options.barOnY2===false || v.options.barOnY2==='0'));
        if(bM.colKey){
          var bv=[]; for(var ib=0;ib<data.length;ib++) bv.push(toNumber(data[ib].__bar)||0);
          var btr={type:'bar',name:bM.colKey||'Bar',x:xs,y:bv};
          if(barOnY2) btr.yaxis='y2';
          btr.hovertemplate = escapeHtml((v.options&&v.options.xTitle)||vmColLabel(ctx,m.x))+': %{x}<br>'+escapeHtml((v.options&&v.options.y2Title)||vmColLabel(ctx,bM.colKey))+': %{y}<extra></extra>';
          traces.push(btr);
        }
        for(var tl=0;tl<lineMs.length;tl++){
          var lk='__'+lineMs[tl].key; var lv=[];
          for(var il=0;il<data.length;il++) lv.push(toNumber(data[il][lk])||0);
          traces.push({type:'scatter',mode:'lines+markers',name:lineMs[tl].m.colKey||('Line '+(tl+1)),x:xs,y:lv,
            hovertemplate: escapeHtml((v.options&&v.options.xTitle)||vmColLabel(ctx,m.x))+': %{x}<br>'+escapeHtml((v.options&&v.options.yTitle)||vmColLabel(ctx,lineMs[tl].m.colKey))+': %{y}<extra></extra>'});
        }
        var th=plotTheme();
        var layout=layoutBase({margin:{l:48,r:56,t:4,b:42},barmode:'group',
          xaxis:{title:{text:(v.options&&v.options.xTitle)?String(v.options.xTitle):vmColLabel(ctx,m.x)}},
          yaxis:{title:{text:(v.options&&v.options.yTitle)?String(v.options.yTitle):(lineMs[0]&&lineMs[0].m.colKey?vmColLabel(ctx,lineMs[0].m.colKey):'Y1')}},
          yaxis2:{overlaying:'y',side:'right',showgrid:false,gridcolor:th.grid,zerolinecolor:th.grid,automargin:true,title:{text:(v.options&&v.options.y2Title)?String(v.options.y2Title):(bM.colKey?vmColLabel(ctx,bM.colKey):'Y2')}}});
        if(v.title){layout.margin.t=4;}
        Plotly.newPlot(el,traces,layout,plotlyCfg(v));
        bindLegendHover(el);
      }
    ...
    reg('multiAxis', {
      ...
      defaultVisual:function(){
        return {id:uid(), type:'multiAxis', title:'Multi Y', show:true,
          mapping:{x:''}, measures:{y1:{colKey:'',agg:'none'},y2:{colKey:'',agg:'none'},y3:{colKey:'',agg:'none'},y4:{colKey:'',agg:'none'},y5:{colKey:'',agg:'none'},y6:{colKey:'',agg:'none'}},
          transform:defaultTransform('aggregate'), filter:defaultFilter(),
          options:{y1type:'line',y2type:'line',y3type:'line',y4type:'line',y5type:'line',y6type:'line',y1dash:'solid',y2dash:'dot',y3dash:'solid',y4dash:'dash',y5dash:'dot',y6dash:'dashdot',y1markers:true,y2markers:true,y3markers:true,y4markers:true,y5markers:true,y6markers:true,y1scale:'linear',y2scale:'linear',y3scale:'linear',y4scale:'linear',y5scale:'linear',y6scale:'linear'}};
      },
      buildMappingUI:function(v,ctx){
        ensureMeasure(v,'y1');ensureMeasure(v,'y2');ensureMeasure(v,'y3');ensureMeasure(v,'y4');ensureMeasure(v,'y5');ensureMeasure(v,'y6');
        return '<div class="form">'
          + mappingField('X','x',ctx.cols,v.mapping.x,true)
          + measureField('Y1 (left axis)','y1',ctx.cols,v.measures.y1)
          + measureField('Y2 (right axis)','y2',ctx.cols,v.measures.y2)
          + measureField('Y3 (optional)','y3',ctx.cols,v.measures.y3)
          + measureField('Y4 (optional)','y4',ctx.cols,v.measures.y4)
          + measureField('Y5 (optional)','y5',ctx.cols,v.measures.y5)
          + measureField('Y6 (optional)','y6',ctx.cols,v.measures.y6)
          + '</div>';
      },
      buildOptionsUI:function(v){
        var o=v.options||{};
        ...
        for(var ai=1; ai<=6; ai++){
          out += '<div class="lbl">Y'+ai+' chart type</div><div><select data-opt="y'+ai+'type">'
            + '<option value="line" '+((o['y'+ai+'type']||'line')==='line'?'selected':'')+'>Line</option>'
            + '<option value="spline" '+((o['y'+ai+'type']||'line')==='spline'?'selected':'')+'>Spline</option>'
            + '<option value="step" '+((o['y'+ai+'type']||'line')==='step'?'selected':'')+'>Step</option>'
            + '<option value="area" '+((o['y'+ai+'type']||'line')==='area'?'selected':'')+'>Area</option>'
            + '<option value="scatter" '+((o['y'+ai+'type']||'line')==='scatter'?'selected':'')+'>Scatter</option>'
            + '<option value="bar" '+((o['y'+ai+'type']||'line')==='bar'?'selected':'')+'>Bar</option>'
            + '</select></div>'
            + '<div class="lbl">Y'+ai+' line style</div><div><select data-opt="y'+ai+'dash">'
            + '<option value="solid" '+((o['y'+ai+'dash']||'solid')==='solid'?'selected':'')+'>Solid</option>'
            + '<option value="dash" '+((o['y'+ai+'dash']||'solid')==='dash'?'selected':'')+'>Dash</option>'
            + '<option value="dot" '+((o['y'+ai+'dash']||'solid')==='dot'?'selected':'')+'>Dot</option>'
            + '<option value="dashdot" '+((o['y'+ai+'dash']||'solid')==='dashdot'?'selected':'')+'>DashDot</option>'
            + '</select></div>'
            + '<div class="lbl">Y'+ai+' scale</div><div><select data-opt="y'+ai+'scale">'
            + '<option value="linear" '+((o['y'+ai+'scale']||'linear')==='linear'?'selected':'')+'>Linear</option>'
            + '<option value="log" '+((o['y'+ai+'scale']||'linear')==='log'?'selected':'')+'>Log</option>'
            + '</select></div>'
            + '<div><label class="note"><input data-opt="y'+ai+'markers" type="checkbox" '+(o['y'+ai+'markers']!==false?'checked':'')+'> Show Y'+ai+' markers</label></div>';
        }
        out += '</div>';
        return out;
      },
      render:function(v,rows,ctx,el){
        var m=v.mapping||{};var ms=v.measures||{};
        if(!m.x||!(ms.y1||{}).colKey){renderEmptyWithModebar(el, 'Map X and at least Y1.', v);return;}
        var filtered=applyFilter(rows,v.filter);
        var meas=[]; for(var mi=1; mi<=6; mi++){ var mk='y'+mi; var mm=ms[mk]||{}; if(mm.colKey) meas.push({colKey:mm.colKey,agg:mm.agg,asKey:'__'+mk}); }
        var data=aggregateRows(filtered,{mode:'aggregate',groupBy:[m.x],measures:meas},[]);
        var xs=[]; for(var i=0;i<data.length;i++) xs.push(asString(data[i][m.x]));
        var o=v.options||{};

        function buildAxisTrace(chartType, name, xVals, yVals, axisRef, dash, markersOn){
          var isBar = chartType==='bar';
          var isScatterOnly = chartType==='scatter';
          var tr = {type:(isBar?'bar':'scatter'), name:name, x:xVals, y:yVals};
          if(axisRef) tr.yaxis = axisRef;
          if(!isBar){
            tr.mode = isScatterOnly ? 'markers' : ((markersOn===false)?'lines':'lines+markers');
            tr.line = tr.line || {};
            if(!isScatterOnly){
              tr.line.dash = dash || 'solid';
              if(chartType==='spline') tr.line.shape = 'spline';
              if(chartType==='step') tr.line.shape = 'hv';
              if(chartType==='area') tr.fill = 'tozeroy';
            }
          }
          return tr;
        }
        ...
        var traces=[];
        for(var tj=1; tj<=6; tj++){
          var tk='y'+tj; var tM=ms[tk]||{}; if(!tM.colKey) continue;
          var tv=[]; for(var ti=0; ti<data.length; ti++) tv.push(toNumber(data[ti]['__'+tk])||0);
          var axisRef = (tj===1) ? null : ('y'+tj);
          traces.push(buildAxisTrace(String(o[tk+'type']||'line'), tM.colKey||('Y'+tj), xs, tv, axisRef, o[tk+'dash']||'solid', o[tk+'markers']!==false));
        }
        var th=plotTheme();
        var layout=layoutBase({
          margin:{l:52,r:90,t:8,b:44},
          hovermode:'x unified',
          xaxis:{title:{text:(v.options&&v.options.xTitle)?String(v.options.xTitle):vmColLabel(ctx,m.x)}},
          yaxis:{automargin:true, type:(String(o.y1scale||'linear')==='log'?'log':'linear'), title:{text:(v.options&&v.options.yTitle)?String(v.options.yTitle):vmColLabel(ctx,(ms.y1||{}).colKey||'')}}
        });
        var rightPositions=[1.00,0.96,0.92,0.88,0.84];
        for(var a=2; a<=6; a++){
          var ak='y'+a; var am=ms[ak]||{}; if(!am.colKey) continue;
          layout['yaxis'+a] = {overlaying:'y',side:'right',position:rightPositions[a-2],showgrid:false,gridcolor:th.grid,zerolinecolor:th.grid,automargin:true,
            type:(String(o[ak+'scale']||'linear')==='log'?'log':'linear'), title:{text:vmColLabel(ctx,am.colKey||('Y'+a))}};
        }
        if(v.title){layout.margin.t=4;}
        Plotly.newPlot(el,traces,layout,plotlyCfg(v));
        bindLegendHover(el);
    ...
    reg('regHist', {
      ...
      defaultVisual:function(){
        return {id:uid(), type:'regHist', title:'Regression', show:true,
          ...
          options:{mode:'scatter', showTrend:false, trendMethod:'linear', showHist:false, histBins:30, outlierMethod:'kmeans', ciLevel:95}};
      },
      buildMappingUI:function(v,ctx){
      ...
      buildOptionsUI:function(v){
        return '<div class="form">'
          ...
          + '<option value="scattergl" '+(v.options.mode==='scattergl'?'selected':'')+'>ScatterGL (large data)</option>'
          + '<option value="bar" '+(v.options.mode==='bar'?'selected':'')+'>Bar</option>'
          + '<option value="histogram" '+(v.options.mode==='histogram'?'selected':'')+'>Histogram (Y)</option>'
          + '<option value="hist+kde" '+(v.options.mode==='hist+kde'?'selected':'')+'>Histogram + KDE</option>'
          + '</select></div>'
          ...
          + '<option value="poly2" '+(v.options.trendMethod==='poly2'?'selected':'')+'>Quadratic (Poly2)</option>'
          + '</select></div>'
          + '<div class="lbl">Histogram bins</div><div><input data-opt="histBins" type="number" min="5" max="200" step="1" value="'+escapeHtml(String(v.options.histBins!=null?v.options.histBins:30))+'" style="width:90px" /></div>'
          + '<div class="lbl">CI level (%)</div><div><input data-opt="ciLevel" type="number" min="50" max="99.9" step="0.1" value="'+escapeHtml(String(v.options.ciLevel!=null?v.options.ciLevel:95))+'" style="width:90px" /></div>'
          + '<div class="note">Statistics/Regression options below apply here too (Mean/Median/CI/Regression/Outliers).</div>'
          + '</div>';
      },
      render:function(v,rows,ctx,el){
        ...
        var xs=[],ys=[];
        var hoverKeys = Array.isArray((v.mapping||{}).hoverCols) ? v.mapping.hoverCols.filter(function(k){return !!k;}) : [];
        var cds=[];
        for(var i=0;i<filtered.length;i++){
          ...
          if(xv!=null && yv!=null){
            ...
            if(hoverKeys.length){
              var cd=[]; for(var hh=0;hh<hoverKeys.length;hh++) cd.push(filtered[i][hoverKeys[hh]]);
              cds.push(cd);
            }
          }
        }
        ...
        var sc=(v.options&&v.options.statsColors)?v.options.statsColors:{};
        var cMean=sc.mean||'#9ca3af', cMedian=sc.median||'#2563eb', cCI=sc.ci||'#90A4AE', cReg=sc.regression||'#6b7280', cOut=sc.outlier||'#f97316';

        function fitModel(xv,yv,method){
          var n=xv.length;
          if(n<2) return null;
          if(method==='poly2' && n>=3){
            var f=fitPoly2(xv,yv);
            if(f) return {method:'poly2',predict:function(x){return f.a*x*x+f.b*x+f.c;},xMin:Math.min.apply(null,xv),xMax:Math.max.apply(null,xv)};
          }
          var sx=0,sy=0,sxx=0,sxy=0;
          for(var j=0;j<n;j++){sx+=xv[j];sy+=yv[j];sxx+=xv[j]*xv[j];sxy+=xv[j]*yv[j];}
          var den=n*sxx-sx*sx;
          if(Math.abs(den)<=1e-15) return null;
          var b=(n*sxy-sx*sy)/den, a=(sy-b*sx)/n;
          return {method:'linear',a:a,b:b,predict:function(x){return a+b*x;},xMin:Math.min.apply(null,xv),xMax:Math.max.apply(null,xv),n:n,sx:sx,sy:sy,sxx:sxx,den:den};
        }

        function colorToRgba(c, alpha){
          if(!c) return 'rgba(144,164,174,'+alpha+')';
          var col=String(c).trim();
          if(col.indexOf('rgba(')===0){
            return col.replace(/rgba\(([^,]+),([^,]+),([^,]+),[^\)]+\)/,'rgba($1,$2,$3,'+alpha+')');
          }
        ...
        function zFromConfidence(levelPct){
          var lv = isFinite(levelPct) ? levelPct : 95;
          lv = Math.max(50, Math.min(99.9, lv));
          var zMap={80:1.282,85:1.440,90:1.645,95:1.960,98:2.326,99:2.576,99.5:2.807};
          if(zMap[lv]) return zMap[lv];
          var nearest=95, best=Infinity;
          Object.keys(zMap).forEach(function(k){
            var kv=parseFloat(k), d=Math.abs(kv-lv);
            if(d<best){best=d; nearest=kv;}
          });
          return zMap[nearest];
        }

        function distanceFlags(points){
          ...
          for(var i1=0;i1<n;i1++){
            ...
            for(var j1=0;j1<n;j1++){
              if(i1===j1) continue;
              var dx=points[i1].x-points[j1].x, dy=points[i1].y-points[j1].y;
              sum += Math.sqrt(dx*dx+dy*dy);
            }
            ds.push(sum/Math.max(1,n-1));
          }
          var mean=0; for(var mi=0;mi<ds.length;mi++) mean+=ds[mi]; mean/=Math.max(1,ds.length);
          var varr=0; for(var vi=0;vi<ds.length;vi++){ var d0=ds[vi]-mean; varr += d0*d0; }
          var sd=Math.sqrt(varr/Math.max(1,ds.length));
          var thres=mean+2*sd;
          return ds.map(function(vv){ return vv>thres; });
        }

        function hierarchicalFlags(points){
          var n=points.length; if(!n) return [];
          var clusters=points.map(function(_,i){ return [i]; });
          while(clusters.length>2){
            var bi=0,bj=1,best=Infinity;
            for(var i=0;i<clusters.length;i++){
              for(var j=i+1;j<clusters.length;j++){
                var dist=Infinity;
                for(var a=0;a<clusters[i].length;a++){
                  for(var b=0;b<clusters[j].length;b++){
                    var p1=points[clusters[i][a]], p2=points[clusters[j][b]];
          ...
          if(clusters.length<2) return points.map(function(){return false;});
          var outCluster = clusters[0].length <= clusters[1].length ? clusters[0] : clusters[1];
          var flags=points.map(function(){return false;});
          for(var oi=0;oi<outCluster.length;oi++) flags[outCluster[oi]]=true;
          return flags;
        }

        function kmeansFlags(points){
          var n=points.length; if(!n) return [];
          var c1={x:points[0].x,y:points[0].y}, c2={x:points[n-1].x,y:points[n-1].y};
          var assign=new Array(n);
          for(var it=0;it<10;it++){
            for(var i=0;i<n;i++){
              var d1=Math.pow(points[i].x-c1.x,2)+Math.pow(points[i].y-c1.y,2);
              var d2=Math.pow(points[i].x-c2.x,2)+Math.pow(points[i].y-c2.y,2);
              assign[i]=d1<d2?0:1;
            }
            var s1x=0,s1y=0,n1=0,s2x=0,s2y=0,n2=0;
            for(var j=0;j<n;j++){
              if(assign[j]===0){ s1x+=points[j].x; s1y+=points[j].y; n1++; }
              else { s2x+=points[j].x; s2y+=points[j].y; n2++; }
            }
            if(n1){ c1={x:s1x/n1,y:s1y/n1}; }
            if(n2){ c2={x:s2x/n2,y:s2y/n2}; }
          }
          var cnt0=0,cnt1=0;
          for(var ci=0;ci<assign.length;ci++){ if(assign[ci]===0) cnt0++; else cnt1++; }
          var out=cnt0<=cnt1?0:1;
          return assign.map(function(a){ return a===out; });
        }

        function getOutlierFlags(points){
          var method=String((v.options&&v.options.outlierMethod)||'kmeans').toLowerCase();
          if(method==='distance') return distanceFlags(points);
          if(method==='hierarchical') return hierarchicalFlags(points);
          if(method==='kmeans') return kmeansFlags(points);
          // backward compatibility for std/iqr/mad modes
          var xv=points.map(function(p){return p.x;}), yv=points.map(function(p){return p.y;});
          var base=fitModel(xv,yv,'linear');
          if(!base) return points.map(function(){return false;});
          var kVal=parseFloat(v.options&&v.options.outlierK); if(!isFinite(kVal)||kVal<=0) kVal=2;
          var res=yv.map(function(y,ii){return y-base.predict(xv[ii]);});
          var flags=res.map(function(){return false;});
          if(method==='iqr'){
            var rs=res.slice().sort(function(a,b){return a-b;});
            var q1=rs[Math.floor((rs.length-1)*0.25)], q3=rs[Math.floor((rs.length-1)*0.75)], iqr=q3-q1;
            var lo=q1-kVal*iqr, hi=q3+kVal*iqr;
            for(var ri=0;ri<res.length;ri++) flags[ri]=(res[ri]<lo||res[ri]>hi);
          }else{
            var mr=0; for(var rm=0;rm<res.length;rm++) mr+=res[rm]; mr=res.length?mr/res.length:0;
            var scale=0;
            if(method==='mad'){
              var ad=[]; for(var ai=0;ai<res.length;ai++) ad.push(Math.abs(res[ai]-mr));
              ad.sort(function(a,b){return a-b;});
              scale=ad[Math.floor(ad.length/2)]*1.4826;
            }else{
              var sq=0; for(var qi=0;qi<res.length;qi++) sq+=Math.pow(res[qi]-mr,2);
              scale=Math.sqrt(sq/Math.max(1,res.length));
            }
            var thr=kVal*(scale||1e-9);
            for(var fi=0;fi<res.length;fi++) flags[fi]=Math.abs(res[fi]-mr)>thr;
          }
          return flags;
        }

        if(mode==='histogram' || mode==='hist+kde'){
          var hBins = clamp(parseInt(v.options.histBins||30,10)||30,5,200);
          traces.push({type:'histogram', x:ys, nbinsx:hBins, marker:{color:th.accent, opacity:0.7}, name:yM.colKey});
          if(mode==='hist+kde'){
            var sorted = ys.slice().sort(function(a,b){return a-b;});
            if(sorted.length>=2){
              var nK=sorted.length, meanK=0; for(var mk=0;mk<nK;mk++) meanK+=sorted[mk]; meanK/=nK;
              var vv=0; for(var vk=0;vk<nK;vk++){ var dk=sorted[vk]-meanK; vv+=dk*dk; }
              var h=1.06*Math.sqrt(vv/Math.max(1,nK-1))*Math.pow(nK,-0.2);
              if(!isFinite(h)||h<=0) h=(sorted[nK-1]-sorted[0])/Math.max(20,Math.min(80,nK));
              if(!isFinite(h)||h<=0) h=1;
              var minYk=sorted[0], maxYk=sorted[nK-1]; if(minYk===maxYk){minYk-=1;maxYk+=1;}
              var gx=[],gy=[],inv=1/(Math.sqrt(2*Math.PI)*h*nK);
              for(var gi=0;gi<120;gi++){
                var x0=minYk + (maxYk-minYk)*gi/119; gx.push(x0);
                var s0=0; for(var gp=0;gp<nK;gp++){ var z=(x0-sorted[gp])/h; s0+=Math.exp(-0.5*z*z); }
                gy.push(inv*s0*nK);
              }
              traces.push({type:'scatter', mode:'lines', x:gx, y:gy, name:'KDE', yaxis:'y2', line:{color:cReg, width:2}});
            }
          }
        }else if(mode==='bar'){
          var bucketCount = clamp(parseInt(v.options.histBins||20,10)||20,5,120);
          var minB=Math.min.apply(null,ys), maxB=Math.max.apply(null,ys), spanB=Math.max(1e-9,maxB-minB), stepB=spanB/bucketCount;
          var bins=new Array(bucketCount); for(var bi=0;bi<bucketCount;bi++) bins[bi]=0;
          for(var bj=0;bj<ys.length;bj++){ var idx=Math.min(bucketCount-1,Math.max(0,Math.floor((ys[bj]-minB)/stepB))); bins[idx]+=1; }
          var bx=[],by=[]; for(var bk=0;bk<bucketCount;bk++){ bx.push((minB+stepB*(bk+0.5)).toFixed(2)); by.push(bins[bk]); }
          traces.push({type:'bar', x:bx, y:by, name:'Count', marker:{color:th.accent, opacity:0.8}});
        }else{
          var method = String((v.options&&v.options.trendMethod)||'linear');
          var pts=[]; for(var pp=0;pp<xs.length;pp++) pts.push({x:xs[pp],y:ys[pp]});
          var flags = getOutlierFlags(pts);
          var showOut = (v.options&&v.options.showOutliers)!==false;
          var exclOut = !!(v.options&&v.options.excludeOutliers);
          var nx=[],ny=[],ox=[],oy=[],regPts=[];
          for(var pi=0;pi<pts.length;pi++){
            var p=pts[pi], isOut=!!flags[pi];
            if(isOut){
              if(!exclOut && showOut){ ox.push(p.x); oy.push(p.y); }
              if(!exclOut) regPts.push(p);
            }else{
              nx.push(p.x); ny.push(p.y); regPts.push(p);
            }
          }
          var rx=regPts.map(function(p){return p.x;}), ry=regPts.map(function(p){return p.y;});
          var model = fitModel(rx,ry,method) || fitModel(rx,ry,'linear');

          var rxLbl=(v.options&&v.options.xTitle)?String(v.options.xTitle):vmColLabel(ctx,xM.colKey); var ryLbl=(v.options&&v.options.yTitle)?String(v.options.yTitle):vmColLabel(ctx,yM.colKey);
          traces.push({type:(mode==='scattergl'?'scattergl':'scatter'), mode:'markers', name:'Normal', x:nx, y:ny, marker:{size:7,color:th.accent,opacity:0.9,line:{width:0.5,color:'#fff'}}, hovertemplate:escapeHtml(rxLbl)+': %{x}<br>'+escapeHtml(ryLbl)+': %{y}<extra>Normal</extra>'});
          if(ox.length) traces.push({type:(mode==='scattergl'?'scattergl':'scatter'), mode:'markers', name:'Outlier', x:ox, y:oy, marker:{size:8,color:cOut,opacity:0.95,line:{width:0.5,color:'#fff'}}, hovertemplate:escapeHtml(rxLbl)+': %{x}<br>'+escapeHtml(ryLbl)+': %{y}<extra>Outlier</extra>'});

          if(model){
            var allX=pts.map(function(p){return p.x;}), allY=pts.map(function(p){return p.y;});
            var xMin=Math.min.apply(null,allX), xMax=Math.max.apply(null,allX);
            var yMin=Math.min.apply(null,allY), yMax=Math.max.apply(null,allY);
            var meanX=0, meanY=0; for(var mi2=0;mi2<rx.length;mi2++){ meanX+=rx[mi2]; meanY+=ry[mi2]; } meanX/=Math.max(1,rx.length); meanY/=Math.max(1,ry.length);
            var ysSorted=ry.slice().sort(function(a,b){return a-b;});
            var medianY=ysSorted[Math.floor(ysSorted.length/2)];

            if(sm.indexOf('mean')>=0){ traces.push({type:'scatter',mode:'lines',name:'X Mean',x:[meanX,meanX],y:[yMin,yMax],line:{color:cMean,width:1.5,dash:'dash'}}); }
            if(sm.indexOf('median')>=0){ traces.push({type:'scatter',mode:'lines',name:'Median Y',x:[xMin,xMax],y:[medianY,medianY],line:{color:cMedian,width:1.5,dash:'dot'}}); }
            if(v.options && v.options.regressionShowYMean){ traces.push({type:'scatter',mode:'lines',name:'Y Mean',x:[xMin,xMax],y:[meanY,meanY],line:{color:cMean,width:1.5,dash:'dash'}}); }
            if(v.options && v.options.regressionShowYMedian){ traces.push({type:'scatter',mode:'lines',name:'Y Median',x:[xMin,xMax],y:[medianY,medianY],line:{color:cMedian,width:1.5,dash:'dot'}}); }

            var doReg = sm.indexOf('regression')>=0;
            if(doReg){
              var lineX=[], lineY=[];
              for(var li=0;li<=100;li++){ var xv=lineX[li]=xMin + (xMax-xMin)*li/100; lineY[li]=model.predict(xv); }
              traces.push({type:'scatter',mode:'lines',name:(model.method==='poly2'?'Trend (Poly2)':'Linear'),x:lineX,y:lineY,line:{color:cReg,width:2.5}});

              if(sm.indexOf('ci')>=0 && model.method==='linear' && rx.length>=3){
                var n=rx.length, sumErr2=0;
                for(var ei=0;ei<n;ei++){ var e=ry[ei]-model.predict(rx[ei]); sumErr2 += e*e; }
                var rmse=Math.sqrt(sumErr2/Math.max(1,n-2));
                var sxx=0; for(var sxj=0;sxj<n;sxj++){ var dx=rx[sxj]-meanX; sxx += dx*dx; }
                if(sxx>1e-12){
                  var ciLevel = parseFloat(v.options && v.options.ciLevel);
                  if(!isFinite(ciLevel)) ciLevel = 95;
                  ciLevel = Math.max(50, Math.min(99.9, ciLevel));
                  var t = zFromConfidence(ciLevel);
                  var xBand=[], yLo=[], yHi=[];
                  for(var ci=0;ci<=60;ci++){
                    var xb=xMin + (xMax-xMin)*ci/60;
                    var yHat=model.predict(xb);
                    var se=rmse*Math.sqrt(1/n + Math.pow(xb-meanX,2)/sxx);
                    xBand.push(xb); yLo.push(yHat-t*se); yHi.push(yHat+t*se);
                  }
                  traces.push({type:'scatter',mode:'lines',x:xBand,y:yLo,line:{color:'rgba(0,0,0,0)'},name:'CI Lower',hoverinfo:'skip',showlegend:false});
                  traces.push({type:'scatter',mode:'lines',x:xBand,y:yHi,fill:'tonexty',fillcolor:colorToRgba(cCI,0.22),line:{color:'rgba(0,0,0,0)'},name:ciLevel.toFixed(1).replace(/\.0$/,'')+'% CI Band',hoverinfo:'skip'});
                }
              }

              if(model.method==='linear'){
                var ssTot=0, ssRes=0;
                for(var ri2=0;ri2<ry.length;ri2++){
                  var yp=model.predict(rx[ri2]);
                  ssTot += Math.pow(ry[ri2]-meanY,2);
                  ssRes += Math.pow(ry[ri2]-yp,2);
                }
                var r2=ssTot===0?0:(1-ssRes/ssTot);
                var xLabel=(v.options&&v.options.xTitle)?String(v.options.xTitle):vmColLabel(ctx,xM.colKey);
                var yLabel=(v.options&&v.options.yTitle)?String(v.options.yTitle):vmColLabel(ctx,yM.colKey);
                layoutTitle = '<b>'+yLabel+' vs '+xLabel+'</b><br><span style="font-size:.86em">(R² = '+(r2*100).toFixed(2)+'%)</span>'; 
              }
            }
          }
        }

        var __w = (el && el.clientWidth) ? el.clientWidth : 520;
        var __baseTitleSize = clamp(parseInt(v.titleSize,10)||14,10,22);
        var __responsiveTitleSize = clamp(Math.round(__baseTitleSize * Math.max(0.72, Math.min(1, __w/560))), 10, 22);
        var layout=layoutBase({
          title: layoutTitle?{text:layoutTitle, x:0.02, xanchor:'left', y:0.98, yanchor:'top', font:{size:__responsiveTitleSize}, pad:{t:2,b:0}}:undefined,
          margin:{l:56,r:18,t:(layoutTitle?(__responsiveTitleSize+34):24),b:58},
          xaxis:{title:{text:(v.options&&v.options.xTitle)?String(v.options.xTitle):vmColLabel(ctx,xM.colKey)},automargin:true},
          yaxis:{title:{text:(v.options&&v.options.yTitle)?String(v.options.yTitle):vmColLabel(ctx,yM.colKey)},automargin:true}
        });
        if(mode==='histogram' || mode==='hist+kde' || mode==='bar') layout.yaxis.title = {text:'Count'};
        if(layout.title){ layout.title.automargin = true; }
        if(mode==='hist+kde'){ layout.yaxis2 = {overlaying:'y', side:'right', showgrid:false, title:{text:'Density'}}; }
        var xang2 = parseFloat(v.options && v.options.xTickAngle);
        layout.xaxis.tickangle = isFinite(xang2) ? xang2 : undefined;
        if(v.title && !layoutTitle){layout.margin.t=4;}
        Plotly.newPlot(el,traces,layout,plotlyCfg(v));
        bindLegendHover(el);
      }
    ...
    reg('correlation', {
      ...
      render:function(v,rows,ctx,el){
        ...
        // Extract numeric columns (preserve nulls to avoid false correlations)
        var data={};
        for(var c=0;c<cols.length;c++) data[cols[c]]=[];
        for(var i=0;i<filtered.length;i++){
          for(var c2=0;c2<cols.length;c2++){
            var nv=toNumber(filtered[i][cols[c2]]);
            data[cols[c2]].push(nv!=null?nv:null);
          }
        }
        ...
        for(var r2=0;r2<n;r2++){
          ...
          for(var c3=0;c3<n;c3++){
            ...
            var len=0, sa=0,sb=0,sa2=0,sb2=0,sab=0;
            for(var k=0;k<a.length;k++){
              var av=a[k], bv=b[k];
              if(av==null || bv==null || !isFinite(av) || !isFinite(bv)) continue;
              len++; sa+=av; sb+=bv; sa2+=av*av; sb2+=bv*bv; sab+=av*bv;
            }
            var denom2=Math.sqrt((len*sa2-sa*sa)*(len*sb2-sb*sb));
            var corr = (len>=2 && denom2>1e-15) ? (len*sab-sa*sb)/denom2 : 0;
    ...
    reg('pivot', {
      ...
      defaultVisual:function(){
        return {id:uid(), type:'pivot', title:'Pivot Table', show:true,
          mapping:{row:[], col:[], values:[]}, measures:{value:{colKey:'',agg:'sum'}},
          transform:defaultTransform('aggregate'), filter:defaultFilter(), options:{limit:0,heatColor:true,fixedFirstCol:true,valueAgg:'sum',showRowTotals:true,showColTotals:true,showSubtotals:false,sortRows:'none',sortCols:'none',suppressRepeatHeaders:true,cfRules:[]}};
      },
      buildMappingUI:function(v,ctx){
        v.mapping = v.mapping || {};
        v.mapping.row = Array.isArray(v.mapping.row) ? v.mapping.row : (v.mapping.row?[v.mapping.row]:[]);
        v.mapping.col = Array.isArray(v.mapping.col) ? v.mapping.col : (v.mapping.col?[v.mapping.col]:[]);
        v.mapping.values = Array.isArray(v.mapping.values) ? v.mapping.values : [];
        var ncols = (ctx.cols||[]).filter(function(c){ return c && c.kind==='number'; });
        if(!ncols.length) ncols = ctx.cols || [];
        return '<div class="form">'
          + mappingFieldMulti('Row headers', 'row', ctx.cols, v.mapping.row, false, 'Select one or more row dimensions.')
          + mappingFieldMulti('Column headers', 'col', ctx.cols, v.mapping.col, false, 'Select one or more column dimensions.')
          + mappingFieldMulti('Value columns', 'values', ncols, v.mapping.values, false, 'Select one or more numeric value columns.')
          + '</div>';
      },
      buildOptionsUI:function(v){
        var agg = String((v.options&&v.options.valueAgg)||'sum');
        return '<div class="form">'
          + '<div class="lbl">Row limit</div><div class="row"><input data-opt="limit" type="number" min="0" max="5000" step="10" value="'+escapeHtml(String(v.options.limit==null?0:v.options.limit))+'" style="width:140px" /><span class="note">0 = render all rows</span></div>'
          + '<div class="lbl">Value aggregation</div><div><select data-opt="valueAgg" style="min-width:140px">'
          ...
          + '</select></div>'
          + '<div class="lbl">Sort labels</div><div class="row" style="gap:8px"><select data-opt="sortRows" style="min-width:130px"><option value="none" '+((v.options.sortRows||'none')==='none'?'selected':'')+'>Rows: None</option><option value="asc" '+(v.options.sortRows==='asc'?'selected':'')+'>Rows: A→Z</option><option value="desc" '+(v.options.sortRows==='desc'?'selected':'')+'>Rows: Z→A</option></select><select data-opt="sortCols" style="min-width:130px"><option value="none" '+((v.options.sortCols||'none')==='none'?'selected':'')+'>Cols: None</option><option value="asc" '+(v.options.sortCols==='asc'?'selected':'')+'>Cols: A→Z</option><option value="desc" '+(v.options.sortCols==='desc'?'selected':'')+'>Cols: Z→A</option></select></div>'
          ...
          + '<div class="lbl">Horizontal scroll</div><div><label class="note"><input data-opt="fixedFirstCol" type="checkbox" '+(v.options.fixedFirstCol!==false?'checked':'')+'> Fixed first column</label></div>'
          + '<div class="lbl">Totals</div><div class="row"><label class="note"><input data-opt="showRowTotals" type="checkbox" '+(v.options.showRowTotals!==false?'checked':'')+'> Row totals</label><label class="note"><input data-opt="showColTotals" type="checkbox" '+(v.options.showColTotals!==false?'checked':'')+'> Column totals</label><label class="note"><input data-opt="showSubtotals" type="checkbox" '+(v.options.showSubtotals?'checked':'')+'> Subtotals (first row field)</label></div>'
          + '<div class="lbl">Headers</div><div class="row"><label class="note"><input data-opt="suppressRepeatHeaders" type="checkbox" '+(v.options.suppressRepeatHeaders!==false?'checked':'')+'> Suppress repeated row headers</label></div>'
          + '</div>';
      },
      render:function(v,rows,ctx,el){
        var m=v.mapping||{};
        var rowCols = Array.isArray(m.row) ? m.row.filter(Boolean) : (m.row?[m.row]:[]);
        var colCols = Array.isArray(m.col) ? m.col.filter(Boolean) : (m.col?[m.col]:[]);
        var valCols = Array.isArray(m.values) ? m.values.filter(Boolean) : [];
        if(!valCols.length && v.measures && v.measures.value && v.measures.value.colKey) valCols=[v.measures.value.colKey];
        if(!rowCols.length || !colCols.length || !valCols.length){ renderEmptyWithModebar(el, 'Map Row headers, Column headers, and Value columns.', v); return; }
        var filtered=applyFilter(rows,v.filter);
        var agg = String((v.options&&v.options.valueAgg)||'sum');
        var cagg=(v.options&&v.options.colAggs&&typeof v.options.colAggs==='object')?v.options.colAggs:{};
        var cdec=(v.options&&v.options.colDecimals&&typeof v.options.colDecimals==='object')?v.options.colDecimals:{};
        var cdate=(v.options&&v.options.colDateFmts&&typeof v.options.colDateFmts==='object')?v.options.colDateFmts:{};
        var cfont=(v.options&&v.options.colFontPx&&typeof v.options.colFontPx==='object')?v.options.colFontPx:{};
        var cw=(v.options&&v.options.colWidths&&typeof v.options.colWidths==='object')?v.options.colWidths:{};
        var ca=(v.options&&v.options.colAligns&&typeof v.options.colAligns==='object')?v.options.colAligns:{};
        var cfRules = Array.isArray((v.options||{}).cfRules) ? v.options.cfRules : [];
        var suppressRepeat = (v.options && v.options.suppressRepeatHeaders)!==false;
        function fmtPivot(col,val){ if(val==null) return ''; var d=(cdec[col]!=null&&cdec[col]!=='')?parseInt(cdec[col],10):null; var n=toNumber(val); if(isFinite(n)&&d!=null&&isFinite(d)) return Number(n).toFixed(Math.max(0,Math.min(10,d))); var df=String(cdate[col]||''); if(df){ var dd=new Date(val); if(dd&&!isNaN(dd.getTime())){ var y=dd.getFullYear(),m=('0'+(dd.getMonth()+1)).slice(-2),da=('0'+dd.getDate()).slice(-2),mons=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']; if(df==='MM/DD/YYYY') return m+'/'+da+'/'+y; if(df==='DD-MMM-YYYY') return da+'-'+mons[dd.getMonth()]+'-'+y; return y+'-'+m+'-'+da; }} return asString(val); }

        var measures=[];
        for(var vi=0; vi<valCols.length; vi++){ var vk=valCols[vi]; measures.push({colKey:vk, agg:String(cagg[vk]||agg||'sum'), asKey:'__v'+vi}); }
        var groupBy = rowCols.concat(colCols);
        var data=aggregateRows(filtered,{mode:'aggregate',groupBy:groupBy,measures:measures});

        var rowKeySet={}, colKeySet={}, cells={};
        for(var i=0;i<data.length;i++){
          var r=data[i];
          var rk = rowCols.map(function(k){ return asString(r[k]); }).join(' • ');
          var ckBase = colCols.map(function(k){ return asString(r[k]); }).join(' • ');
          rowKeySet[rk]=1;
          for(var vix=0; vix<valCols.length; vix++){
            var ck = (valCols.length>1) ? (ckBase + ' | ' + valCols[vix]) : ckBase;
            colKeySet[ck]=1;
            cells[rk+'|||'+ck] = toNumber(r['__v'+vix]);
          }
        }
        var rks=Object.keys(rowKeySet).sort();
        var cks=Object.keys(colKeySet).sort();
        var rowSort=String((v.options&&v.options.sortRows)||'none'); if(rowSort==='desc') rks.reverse();
        var colSort=String((v.options&&v.options.sortCols)||'none'); if(colSort==='desc') cks.reverse();
        var allVals = Object.keys(cells).map(function(k){ return cells[k]; }).filter(function(x){return x!=null && isFinite(x);});
        var vMin = allVals.length ? Math.min.apply(null,allVals) : 0;
        var vMax = allVals.length ? Math.max.apply(null,allVals) : 0;

        var rowHdr = rowCols.join(' • ');
        var rowHeadStyle=''; var rowW=(cw[rowCols[0]]!=null&&cw[rowCols[0]]!=='')?parseInt(cw[rowCols[0]],10):0; if(rowW>0) rowHeadStyle+='width:'+rowW+'px;max-width:'+rowW+'px;'; if(ca[rowCols[0]]) rowHeadStyle+='text-align:'+ca[rowCols[0]]+';'; if(cfont[rowCols[0]]) rowHeadStyle+='font-size:'+parseInt(cfont[rowCols[0]],10)+'px;';
        var out='<div class="vm-table-wrap"><table class="vm-table'+((v.options&&v.options.fixedFirstCol!==false)?' vm-table-fixed-first':'')+'"><thead><tr><th'+(rowHeadStyle?' style="'+rowHeadStyle+'"':'')+'>'+escapeHtml(rowHdr)+' \ '+escapeHtml(colCols.join(' • '))+'</th>';
        for(var ci=0;ci<cks.length;ci++){ var cksrc=String(cks[ci]).split(' | ').pop(); var hs=''; var hw=(cw[cksrc]!=null&&cw[cksrc]!=='')?parseInt(cw[cksrc],10):0; if(hw>0) hs+='width:'+hw+'px;max-width:'+hw+'px;'; if(ca[cksrc]) hs+='text-align:'+ca[cksrc]+';'; if(cfont[cksrc]) hs+='font-size:'+parseInt(cfont[cksrc],10)+'px;'; out+='<th'+(hs?' style="'+hs+'"':'')+'>'+escapeHtml(cks[ci])+'</th>'; }
        if(v.options && v.options.showRowTotals!==false) out+='<th>Row Total</th>';
        out+='</tr></thead><tbody>';
        var _subCur=''; var _subVals=[];
        var _lastRowHdr='';
        function _emitSubtotal(){
          if(!(v.options&&v.options.showSubtotals) || !_subVals.length) return;
          out+='<tr style="font-weight:700;background:rgba(34,197,94,.10)"><td>Subtotal: '+escapeHtml(_subCur)+'</td>';
          var _sg=0;
          for(var _si=0;_si<cks.length;_si++){ var _sv=0; for(var _sj=0;_sj<_subVals.length;_sj++){ var _v=_subVals[_sj][_si]; if(_v!=null&&isFinite(_v)) _sv+=_v; } _sg+=_sv; var _ks=String(cks[_si]).split(' | ').pop(); out+='<td>'+escapeHtml(fmtPivot(_ks,_sv))+'</td>'; }
          if(v.options.showRowTotals!==false) out+='<td>'+Number(_sg).toLocaleString()+'</td>';
          out+='</tr>';
        }
        for(var ri=0;ri<rks.length;ri++){
          var _rg = String(rks[ri]).split(' • ')[0];
          if(v.options&&v.options.showSubtotals && _subCur!=='' && _rg!==_subCur){ _emitSubtotal(); _subVals=[]; }
          _subCur=_rg;
          var rowHdrVal = rks[ri]; if(suppressRepeat && rowHdrVal===_lastRowHdr) rowHdrVal=''; else _lastRowHdr = rks[ri];
          var rstyle='font-weight:600;'; if(ca[rowCols[0]]) rstyle+='text-align:'+ca[rowCols[0]]+';'; if(cfont[rowCols[0]]) rstyle+='font-size:'+parseInt(cfont[rowCols[0]],10)+'px;'; out+='<tr><td style="'+rstyle+'">'+escapeHtml(rowHdrVal)+'</td>';
          var _rowVals=[];
          for(var cj=0;cj<cks.length;cj++){
            var cv=cells[rks[ri]+'|||'+cks[cj]]; _rowVals.push(cv);
            var st='';
            if(v.options.heatColor!==false && cv!=null && isFinite(cv) && vMax>vMin){
              var pct=(cv-vMin)/(vMax-vMin);
              var rr=Math.round(255-(pct*100)), gg=Math.round(200+pct*55), bb=Math.round(255-(pct*80));
              st='background:rgba('+clamp(rr,0,255)+','+clamp(gg,0,255)+','+clamp(bb,0,255)+',0.4)';
            }
            var cksrc2=String(cks[cj]).split(' | ').pop();
            if(cfRules.length){
              for(var cfi=0;cfi<cfRules.length;cfi++){
                var cfr=cfRules[cfi]||{};
                if(cfr.col && cfr.col!==cksrc2) continue;
                if(vmEvalCFRule(cfr, cv)){
                  if(cfr.bg) st += 'background:'+cfr.bg+';';
                  if(cfr.fg) st += 'color:'+cfr.fg+';';
                  break;
                }
              }
            }
            if(ca[cksrc2]) st += 'text-align:'+ca[cksrc2]+';'; if(cfont[cksrc2]) st += 'font-size:'+parseInt(cfont[cksrc2],10)+'px;'; var hw2=(cw[cksrc2]!=null&&cw[cksrc2]!=='')?parseInt(cw[cksrc2],10):0; if(hw2>0) st+='width:'+hw2+'px;max-width:'+hw2+'px;'; out+='<td'+(st?' style="'+st+'"':'')+'>'+(cv!=null&&isFinite(cv)?escapeHtml(fmtPivot(cksrc2,cv)):'')+'</td>';
          }
          if(v.options && v.options.showRowTotals!==false){ var rtot=0; for(var cj2=0;cj2<cks.length;cj2++){ var rv=cells[rks[ri]+'|||'+cks[cj2]]; if(rv!=null&&isFinite(rv)) rtot+=rv; } out+='<td style="font-weight:700">'+escapeHtml(fmtPivot('',rtot))+'</td>'; }
          out+='</tr>';
          if(v.options&&v.options.showSubtotals) _subVals.push(_rowVals);
        }
        _emitSubtotal();
        if(v.options && v.options.showColTotals!==false){
          out+='<tr style="font-weight:700;background:rgba(2,132,199,.08)"><td>Column Total</td>';
          var grand=0;
          for(var ct=0;ct<cks.length;ct++){ var ctot=0; for(var rr=0;rr<rks.length;rr++){ var cv2=cells[rks[rr]+'|||'+cks[ct]]; if(cv2!=null&&isFinite(cv2)){ ctot+=cv2; grand+=cv2; }} var _kct=String(cks[ct]).split(' | ').pop(); out+='<td>'+escapeHtml(fmtPivot(_kct,ctot))+'</td>'; }
          if(v.options.showRowTotals!==false) out+='<td>'+Number(grand).toLocaleString()+'</td>';
          out+='</tr>';
        }
        out+='</tbody></table></div>';
        el.innerHTML=out;
    ...
    reg('control', {
      ...
      render:function(v,rows,ctx,el){
        ...
        var xs=[], ys=[];
        var xKeyCtrl = firstKey(m.x);
        for(var i=0;i<filtered.length;i++){
          xs.push(xKeyCtrl?asString(filtered[i][xKeyCtrl]):String(i+1));
        ...
        var layout=layoutBase({margin:{l:48,r:16,t:4,b:42},xaxis:{title:{text:mappedAxisTitle(v, ctx, v.options&&v.options.xTitle, [firstKey(m.x)])}},yaxis:{title:{text:mappedAxisTitle(v, ctx, v.options&&v.options.yTitle, [yM.colKey])}}});
        if(v.title){layout.margin.t=4;}
        Plotly.newPlot(el,traces,layout,plotlyCfg(v));
        bindLegendHover(el);
      }
    ...
    function suggestVisualTypeByCandidates(cols, candidateKeys){
      var keys = Array.isArray(candidateKeys) ? candidateKeys.filter(Boolean) : [];
      if(!keys.length) return 'line';
      var cset = {};
      for(var i=0;i<keys.length;i++) cset[keys[i]] = true;
      var num = 0, dt = 0, dim = 0;
      for(var j=0;j<(cols||[]).length;j++){
        var c = cols[j];
        if(!c || !cset[c.key]) continue;
        var k = String(c.kind||'').toLowerCase();
        if(k==='number') num++;
        else if(k==='date' || k==='datetime') dt++;
        else dim++;
      }
      if(num>=2 && dim===0) return 'scatter';
      if(num>=1 && (dt>=1 || dim>=1)) return 'line';
      if(num>=1 && dim>=2) return 'bar';
      if(num===1 && dim===1) return 'bar';
      if(num===1 && dim===0) return 'kpi';
      if(dim>=2 && num===0) return 'table';
      return 'line';
    }

    function vmResolveTargetRange(v, rows){
      var opts = (v && v.options) ? v.options : {};
      if(opts.showTargets !== true) return { min:null, max:null };
      var mode = String(opts.targetMode || 'none');
      if(mode==='none') return { min:null, max:null };
      var minVal = opts.targetMin;
      var maxVal = opts.targetMax;
      if(mode === 'columns'){
        if(!opts.targetMinCol && !opts.targetMaxCol) return { min:null, max:null };
      ...
      function toFiniteOrNull(vv){
        if(vv==null || vv==='') return null;
        var n = Number(vv);
        return isFinite(n) ? n : null;
      }
      minVal = toFiniteOrNull(minVal);
      maxVal = toFiniteOrNull(maxVal);
      return { min: minVal, max: maxVal };
    }

    function vmAutoMapVisualByCandidates(v, cols, candidateKeys){
      if(!v) return v;
      cols = Array.isArray(cols) ? cols : [];
      var keys = Array.isArray(candidateKeys) ? candidateKeys.filter(Boolean) : [];
      if(!keys.length){
        for(var i0=0;i0<cols.length;i0++) keys.push(String(cols[i0].key||''));
        keys = keys.filter(Boolean).slice(0, 8);
      }
      if(!keys.length) return v;
      var byKey = {};
      for(var i=0;i<cols.length;i++) if(cols[i] && cols[i].key!=null) byKey[String(cols[i].key)] = cols[i];
      var num=[], dt=[], dim=[];
      for(var j=0;j<keys.length;j++){
        var k = String(keys[j]||'');
        var c = byKey[k];
        var kind = String((c&&c.kind)||'string').toLowerCase();
        if(kind==='number') num.push(k);
        else if(kind==='date' || kind==='datetime') dt.push(k);
        else dim.push(k);
      }
      var used = {};
      var pick = function(arr){
        for(var p=0;p<arr.length;p++){ var k=arr[p]; if(!used[k]){ used[k]=1; return k; } }
        return arr[0] || '';
      };
      var xKey = pick(dt) || pick(dim) || pick(num);
      var yKey = pick(num) || pick(dim);
      var zKey = pick(num);
      var seriesKey = pick(dim) || pick(dt);
      var labelKey = pick(dim) || xKey;

      v.mapping = v.mapping || {};
      var map = v.mapping;
      var setMap = function(k,val){
        if(!(k in map)) return;
        if(Array.isArray(map[k])){ if((!map[k].length || (map[k].length===1 && !map[k][0])) && val) map[k] = [val]; }
        else { if((map[k]==null || map[k]==='') && val) map[k] = val; }
      };
      setMap('x', xKey);
      setMap('y', yKey);
      setMap('label', labelKey);
      setMap('labels', labelKey);
      setMap('value', yKey || zKey);
      setMap('values', yKey || zKey);
      setMap('category', xKey || labelKey);
      setMap('task', labelKey);
      setMap('start', dt[0] || xKey);
      setMap('end', dt[1] || dt[0]);
      setMap('date', dt[0] || xKey);
      setMap('open', num[0] || yKey);
      setMap('high', num[1] || num[0]);
      setMap('low', num[2] || num[0]);
      setMap('close', num[3] || num[0]);
      setMap('theta', dim[0] || xKey);
      setMap('row', dim[0] || xKey);
      setMap('col', dim[1] || seriesKey);
      setMap('series', seriesKey);

      v.measures = v.measures || {};
      var nidx = 0;
      Object.keys(v.measures).forEach(function(mk){
        var m = v.measures[mk] = v.measures[mk] || {};
        if(!m.colKey){
          var pickN = num[nidx] || num[0] || yKey || xKey;
          if(pickN) m.colKey = pickN;
          nidx++;
        }
      });
      return v;
    ...
    function vmAutoMapNthMeasure(v, cols, candidateKeys, nth){
      v = vmAutoMapVisualByCandidates(v, cols, candidateKeys);
      cols = Array.isArray(cols) ? cols : [];
      var byKey = {};
      for(var i=0;i<cols.length;i++) if(cols[i] && cols[i].key!=null) byKey[String(cols[i].key)] = cols[i];
      var keys = Array.isArray(candidateKeys) ? candidateKeys.filter(Boolean) : [];
      if(!keys.length) keys = Object.keys(byKey);
      var num = keys.filter(function(k){ var c=byKey[k]; var kind=String((c&&c.kind)||'string').toLowerCase(); return kind==='number'; });
      if(!num.length) return v;
      var pick = num[Math.max(0, nth % num.length)];
      if(v.measures && typeof v.measures==='object'){
        var firstM = null;
        Object.keys(v.measures).some(function(k){
          var m=v.measures[k];
          if(m && typeof m==='object' && Object.prototype.hasOwnProperty.call(m,'colKey')){ firstM = m; return true; }
          return false;
        });
        if(firstM) firstM.colKey = pick;
      }
      if(v.mapping && Object.prototype.hasOwnProperty.call(v.mapping,'y') && !Array.isArray(v.mapping.y)) v.mapping.y = pick;
      if(v.mapping && Object.prototype.hasOwnProperty.call(v.mapping,'value') && !Array.isArray(v.mapping.value)) v.mapping.value = pick;
      return v;
    }
    ...
    function defaultState(){
      return {
        globals:{
          ...
          cardShadow: 'soft',
          wrapMax: 'full',
          modebarIcons: {}, crossFilterEnabled:false, crossFilter:null, subscriptionEnabled:false
        },
        sections:{
          enabled:false,
          splitCol:'',
    ...
    function ensureVisualDefaults(v){
      ...
      if(typeof v.options.targetMode==='undefined') v.options.targetMode = 'none';
      if(v.options.targetMode==='none') v.options.showTargets = false;
      if(typeof v.options.showRefLines==='undefined') v.options.showRefLines = false;
      if((v.type==='line' || v.type==='scatter') && typeof v.options.showStatsOverlay==='undefined') v.options.showStatsOverlay = false;
      if(v.type==='line' && typeof v.options.showTrend==='undefined') v.options.showTrend = false;
      if(v.type==='scatter' && typeof v.options.showTrend==='undefined') v.options.showTrend = false;
      ...
      if(!v.__settingsBaseline || typeof v.__settingsBaseline!=='object'){ v.__settingsBaseline = deepClone(v.options || {}); }
      return v;
    }
    ...
    var vmTbSvgs = {
      ...
      more: '<svg viewBox="0 0 24 24"><circle cx="12" cy="6" r="1.5"/><circle cx="12" cy="12" r="1.5"/><circle cx="12" cy="18" r="1.5"/></svg>'
    };
    ...
    var VM_DEFAULT_MODEBAR_ICONS = ['reset','points','yaxis','targets','filter','zoom','status','remove','download','fs','settings'];
    ...
    function allowedModebarIconsForType(type){
      ...
      var allCharts = ['line','bar','pie','scatter','histogram','box','heatmap','waterfall','treemap','donut','funnel','sunburst','bubble','stackedArea','stackedBar','lineBar','radar','candlestick','ohlc','multiAxis','boxPlot','regHist','control','gantt','calendarHeatmap','staticText'];
      ...
      if(dataTypes.indexOf(t) >= 0){
        ...
        return ['reset','status','remove','download','fs','settings'];
      }
      if(kpiTypes.indexOf(t) >= 0){
        /* KPI visuals: minimal modebar */
        return ['status','remove','download','fs','settings'];
      }
      if(cartesianTypes.indexOf(t) >= 0){
        /* Full modebar for cartesian */
        return ['reset','points','yaxis','targets','filter','zoom','status','remove','download','fs','settings'];
      }
      if(allCharts.indexOf(t) >= 0){
        /* Other charts: moderate modebar */
        return ['reset','remove','download','fs','settings'];
      }
    ...
    function bizButtonsHtml(v){
      ...
      var html = '';
      var allIcons = getVisualModebarIcons(v);
      var icons = allIcons.filter(function(k){ return k!=='fs' && k!=='settings'; });
      html += '<div class="vm-vtb" data-vid="'+escapeHtml(vid)+'">';
      if(SHOW_VISUAL_SETTINGS_ICON){ html += vmTbBtn('settings','Visual settings',vid); }
      html += vmTbBtn('fs','Fullscreen',vid);
      html += '<button class="vm-tb-btn" data-act="more" data-vid="'+escapeHtml(vid)+'" data-icons="'+escapeHtml(icons.join(','))+'" title="Visual actions">'+(vmTbSvgs.more||'<span class="vm-dot"></span>')+'</button>';
      html += '</div>';
      return html;
    }
    ...
    function btnRect(gd){
      ...
      if(gd.getBoundingClientRect && !(gd.querySelector && gd.classList && gd.classList.contains('js-plotly-plot'))){
        var br = gd.getBoundingClientRect();
        if(br && (br.width||br.height)) return br;
      }
      var modebar = gd.querySelector ? gd.querySelector('.modebar') : null;
      if(modebar) return modebar.getBoundingClientRect();
      var rect = gd.getBoundingClientRect ? gd.getBoundingClientRect() : {right:innerWidth-8, top:8, left:8};
    ...
    function vmOpenViewMenu(vid, gd){
      vmToggleMenu('view', function(){
        ...
        setTimeout(function(){
          vmPlaceMenu(pop, btnRect((__vmLastToolbarBtnByVid[vid]||gd)));
        }, 10);
      });
    }
    ...
    function vmToggleStatusTable(vid, gd){
      ...
      if(!card && vid){ card = document.querySelector('.card[data-vid="'+cssEscape(String(vid))+'"]'); }
      if(!card) return;
      ...
      try{
        var data = (gd && gd.data) ? gd.data : [];
        if((!data || !data.length) && card){
          var tableRows = card.querySelectorAll('table tbody tr');
          if(tableRows && tableRows.length){
            html = '<table style="width:100%;border-collapse:collapse;font-size:11px">';
            html += '<tr><th style="text-align:left;padding:2px 4px">Column</th><th>Sample Value</th></tr>';
            var first = tableRows[0].querySelectorAll('td');
            var ths = card.querySelectorAll('table thead th');
            for(var ci=0; ci<first.length; ci++){
              var cn = ths[ci] ? String(ths[ci].textContent||('Col '+(ci+1))) : ('Col '+(ci+1));
              var cv = first[ci] ? String(first[ci].textContent||'') : '';
              html += '<tr><td style="padding:2px 4px">'+escapeHtml(cn)+'</td><td style="padding:2px 4px">'+escapeHtml(cv)+'</td></tr>';
            }
            html += '</table>';
            tbl.innerHTML = html;
      ...
      try{
        ...
        if(vv && vv.options && vv.options.insightPanel!==false && gd && gd.data){
          var pts=[];
          (gd.data||[]).forEach(function(t){
            var xa=t.x||[], ya=t.y||[];
            for(var i=0;i<Math.min(xa.length,ya.length);i++){
              var xv=toNumber(xa[i]), yv=toNumber(ya[i]);
              if(xv!=null && yv!=null) pts.push({x:xv,y:yv});
            }
          });
          if(pts.length>=3){
            var n=pts.length,sx=0,sy=0,sxx=0,sxy=0;
            for(var j=0;j<n;j++){sx+=pts[j].x;sy+=pts[j].y;sxx+=pts[j].x*pts[j].x;sxy+=pts[j].x*pts[j].y;}
            var den=n*sxx-sx*sx;
            var slope=Math.abs(den)>1e-12?((n*sxy-sx*sy)/den):0;
            var dir = slope>0 ? 'increasing' : (slope<0 ? 'decreasing' : 'flat');
            html += '<div class="sep"></div><div class="note"><b>Insight:</b> Trend appears <b>'+dir+'</b> (slope '+slope.toFixed(4)+') across '+n+' plotted points.</div>';
            if(vv.options.alertEnabled){
              var op=String(vv.options.alertOp||'gt'); var thr=toNumber(vv.options.alertValue);
              if(thr!=null){
                var latest = pts[pts.length-1].y;
                var hit = (op==='lt') ? (latest<thr) : (latest>thr);
                html += '<div class="note" style="margin-top:4px;color:'+(hit?'#dc2626':'var(--vm-sub)')+'"><b>Alert check:</b> latest value '+latest.toFixed(3)+' '+(op==='lt'?'<':'>' )+' '+thr+' ? '+(hit?'TRIGGERED':'ok')+'</div>';
              }
            }
          }
        }
    ...
    var __vmRenderQueue = Object.create(null);
    function requestVisualRender(vid){
      if(!vid) return;
      if(__vmRenderQueue[vid]) return;
      __vmRenderQueue[vid] = true;
      window.requestAnimationFrame(function(){
        __vmRenderQueue[vid] = false;
        vmRenderSingle(vid);
      });
    }
    ...
    function vmOpenPointsMenu(vid, gd){
      vmToggleMenu('points', function(){
        ...
        html += '<label class="seg"><input type="radio" name="hover" value="x" '+(hoverMode==='x'?'checked':'')+'><span>Compare (all lines)</span></label>';
        ...
        pop.addEventListener('change', function(e){
          ...
          else if(t.name==='labels'){ opts.showLabels = (t.value==='1'); opts.labelsOn = opts.showLabels; changed = true; }
          ...
          if(changed){
            applySettingChanges();
            requestVisualRender(vid);
          }
        });

        document.body.appendChild(pop);
        setTimeout(function(){ vmPlaceMenu(pop, btnRect((__vmLastToolbarBtnByVid[vid]||gd))); }, 10);
      });
    }
    ...
    function vmOpenStatsMenu(vid, gd){
      vmToggleMenu('stats', function(){
        ...
        var opts = visual.options || {};
        var statsModes = Array.isArray(opts.statsModes) ? opts.statsModes.slice() : [];
        if(!statsModes.length && opts.statsMode && opts.statsMode!=='none') statsModes = [String(opts.statsMode)];
        var statsScope = String(opts.statsScope || 'by-group');
        var showMA = !!(opts.showMovingAvg);
        var maWindow = parseInt(opts.maWindow || 3);
        opts.statsColors = opts.statsColors || {};
        var regOut = !!opts.regressionShowOutliers;
        var outMethod = String(opts.outlierMethod || 'kmeans');
        var outK = parseFloat(opts.outlierK);
        if(!isFinite(outK) || outK<=0) outK = 2;
        ...
        html += '<div class="body-inner">';
        html += '<div class="hdr">Statistics (multi-select)</div>';
        function chk(mode,label){
          return '<label style="display:flex;align-items:center;gap:8px;margin:4px 0"><input type="checkbox" name="sm" value="'+mode+'" '+(statsModes.indexOf(mode)>=0?'checked':'')+'><span>'+label+'</span></label>';
        }
        html += chk('mean','Mean');
        html += chk('median','Median');
        html += chk('ci','CI / Band');
        html += chk('regression','Regression');

        html += '<div class="hdr" style="margin-top:8px">Line Colors</div>';
        html += '<div class="row" style="gap:8px;flex-wrap:wrap">';
        html += '<label style="font-size:11px">Mean <input type="color" name="c-mean" value="'+escapeHtml(opts.statsColors.mean || '#ff0000')+'"></label>';
        html += '<label style="font-size:11px">Median <input type="color" name="c-median" value="'+escapeHtml(opts.statsColors.median || '#0000ff')+'"></label>';
        html += '<label style="font-size:11px">CI <input type="color" name="c-ci" value="'+escapeHtml(opts.statsColors.ci || '#008000')+'"></label>';
        html += '<label style="font-size:11px">Regression <input type="color" name="c-reg" value="'+escapeHtml(opts.statsColors.regression || '#ffa500')+'"></label>';
        html += '</div>';
        ...
        html += '<div class="hdr" style="margin-top:8px">Regression Outliers</div>';
        html += '<div class="row" style="align-items:center;gap:8px;flex-wrap:wrap">';
        html += '<label style="display:flex;align-items:center;gap:8px"><input type="checkbox" name="reg-out" '+(regOut?'checked':'')+'> Highlight outliers</label>';
        html += '<select name="out-method"><option value="kmeans" '+(outMethod==='kmeans'?'selected':'')+'>KMeans</option><option value="distance" '+(outMethod==='distance'?'selected':'')+'>Distance</option><option value="hierarchical" '+(outMethod==='hierarchical'?'selected':'')+'>Hierarchical</option><option value="std" '+(outMethod==='std'?'selected':'')+'>STD</option><option value="iqr" '+(outMethod==='iqr'?'selected':'')+'>IQR</option><option value="mad" '+(outMethod==='mad'?'selected':'')+'>MAD</option></select>';
        html += '<input type="number" step="0.1" min="0.5" max="5" name="out-k" value="'+outK+'" title="Threshold">';
        html += '<input type="number" step="0.1" min="50" max="99.9" name="ci-level" value="'+escapeHtml(String(opts.ciLevel!=null?opts.ciLevel:95))+'" title="CI %" style="width:84px">';
        html += '<label style="font-size:11px">Outlier color <input type="color" name="c-out" value="'+escapeHtml(opts.statsColors.outlier || '#ef4444')+'"></label>';
        html += '</div>';
        html += '</div>'; // Close body-inner
        ...
        pop.addEventListener('change', function(e){
          ...
          if(t.name==='sm'){
            var modes = [];
            pop.querySelectorAll('input[name="sm"]:checked').forEach(function(elm){ modes.push(elm.value); });
            opts.statsModes = modes;
            opts.statsMode = modes.length ? modes[0] : 'none';
            opts.showStatsOverlay = modes.length > 0;
            if(modes.indexOf('regression')<0) opts.regressionShowOutliers = false;
            changed = true;
          }
          ...
          else if(t.name==='reg-out'){ opts.regressionShowOutliers = t.checked; changed = true; }
          else if(t.name==='out-method'){ opts.outlierMethod = t.value; changed = true; }
          else if(t.name==='ci-level'){ var cv=parseFloat(t.value); opts.ciLevel = isFinite(cv)?Math.max(50,Math.min(99.9,cv)):95; changed = true; }
          else if(t.name==='out-k'){ opts.outlierK = parseFloat(t.value)||2; changed = true; }
          else if(t.name==='c-mean'){ opts.statsColors.mean = t.value; changed = true; }
          else if(t.name==='c-median'){ opts.statsColors.median = t.value; changed = true; }
          else if(t.name==='c-ci'){ opts.statsColors.ci = t.value; changed = true; }
          else if(t.name==='c-reg'){ opts.statsColors.regression = t.value; changed = true; }
          else if(t.name==='c-out'){ opts.statsColors.outlier = t.value; changed = true; }
    ...
    function vmOpenTargetsMenu(vid, gd){
      vmToggleMenu('targets', function(){
        ...
        var opts = visual.options || {};
        var targetStyle = String(opts.targetStyle || (opts.targetBand ? 'band' : 'line'));
        var minTarget = Number.isFinite(Number(opts.targetMin)) ? Number(opts.targetMin) : null;
        var maxTarget = Number.isFinite(Number(opts.targetMax)) ? Number(opts.targetMax) : null;
        ...
        html += '<label class="seg"><input type="radio" name="band" value="band" '+(targetStyle==='band'?'checked':'')+'><span>Band</span></label>';
        html += '</div>';
        ...
        pop.addEventListener('change', function(e){
          if(e.target.name==='band'){
            opts.targetStyle = String(e.target.value||'line');
            opts.targetMode = opts.targetMode && opts.targetMode!=='none' ? opts.targetMode : 'manual';
        ...
        pop.addEventListener('click', function(e){
          ...
          if(act==='apply'){
            ...
            opts.targetMin = Number.isFinite(mi) ? mi : null;
            opts.targetMax = Number.isFinite(ma) ? ma : null;
            opts.targetMode = 'manual';
            opts.showTargets = (opts.targetMin!=null || opts.targetMax!=null);
            applySettingChanges();
            requestVisualRender(vid);
            vmCloseMenus();
            __vmOpenMenu=null;
    ...
    var _ctxDataCache = null;

    function invalidateCtxDataCache(){
      _ctxDataCache = null;
    }
    ...
    function buildCtx(){
      if(!_ctxDataCache){
        ...
        var colMeta = getNarrativeColumnsMeta();
        var cols = inferColumns(rows, colMeta);
        _ctxDataCache = { rows: rows, cols: cols };
      }
      return {
        rows: _ctxDataCache.rows,
        cols: _ctxDataCache.cols,
        sections: {
    ...
    function setGridVars(visualCount){
      ...
      var cardShadow = String(STATE.globals.cardShadow || 'soft');
      var wrapMax = String(STATE.globals.wrapMax || 'full');
      var cardShadowMap = {none:'none', soft:'0 2px 8px rgba(0,0,0,.08)', medium:'0 8px 20px rgba(0,0,0,.12)', strong:'0 12px 28px rgba(0,0,0,.18)'};
      HOST.style.setProperty('--vm-card-shadow', cardShadowMap[cardShadow] || cardShadowMap.soft);
      HOST.style.setProperty('--vm-wrap-max', (wrapMax==='full' ? 'none' : (String(parseInt(wrapMax,10)||0)+'px')));
      if(gridBg){
    ...
    function renderGrid(){
      ...
      if(rt) {
        var _t = STATE.globals.title || 'Visual Manager';
        var _tu = String(STATE.globals.titleLink||'').trim();
        if(_tu){ rt.innerHTML = '<a href="'+escapeAttr(_tu)+'" target="'+escapeAttr(String(STATE.globals.titleLinkTarget||'_blank'))+'" rel="noopener noreferrer">'+escapeHtml(_t)+'</a>'; }
        else rt.textContent = _t;
      ...
      if(rs) {
        var _s = STATE.globals.subtitle || '';
        var _su = String(STATE.globals.subtitleLink||'').trim();
        if(_su && _s){ rs.innerHTML = '<a href="'+escapeAttr(_su)+'" target="'+escapeAttr(String(STATE.globals.subtitleLinkTarget||'_blank'))+'" rel="noopener noreferrer">'+escapeHtml(_s)+'</a>'; }
        else rs.textContent = _s;
      ...
      function cardsHtmlForSection(secKey){
        ...
        function renderSingleCard(v, inRow){
          ...
          var gtd = (STATE.globals && STATE.globals.visualTitleDefaults) ? STATE.globals.visualTitleDefaults : {};
          var titleAlign = v.titleAlign || gtd.align || 'left';
          var titleSize = v.titleSize || gtd.size || 14;
          var titleColor = v.titleColor || gtd.color || '';
          ...
          return ''
            + '<div class="card" data-vid="'+escapeHtml(v.id)+'" data-vtype="'+escapeHtml(v.type||'')+'" data-sec="'+escapeHtml(secKey||'')+'"'+groupKeyAttr+' style="'+escapeHtml(bits.join(';'))+'">'
            + '  <div class="hdr">'
      ...
      if(isCustom){
        ...
        for(var ti=0; ti<tiles.length; ti++){
          ...
          html += '<div class="card" data-vid="'+escapeHtml(vObj.id)+'" data-vtype="'+escapeHtml(vObj.type||'')+'" data-sec="" style="'+escapeHtml(gs)+'">';
    ...
    function requestManagerRender(){
      if(_mgrRenderReq) return;
      _mgrRenderReq = 1;
      window.requestAnimationFrame(function(){
        _mgrRenderReq = 0;
        if(mgrDraft) renderManager();
      });
    }
    ...
    function openManager(){
      ...
      mgrTab = 'layout';
      renderManager();
      try{ HOST.appendChild(mgrBackdrop); }catch(_){ }
    ...
    function wireManagerSearch(){
      var old = document.getElementById('vm-mgr-search');
      if(!old){
        var box = document.createElement('div');
        box.className='row'; box.style.marginBottom='8px';
        box.innerHTML = '<input id="vm-mgr-search" placeholder="Search in this tab..." style="width:100%" />';
        if(mgrBody.firstChild) mgrBody.insertBefore(box, mgrBody.firstChild); else mgrBody.appendChild(box);
      }
      var inp = document.getElementById('vm-mgr-search');
      if(!inp) return;
      inp.oninput = function(){
        var qv = String(inp.value||'').toLowerCase().trim();
        mgrBody.querySelectorAll('.section-hdr').forEach(function(h){
          var sec = h.getAttribute('data-section');
          var b = mgrBody.querySelector('.section-body[data-section="'+sec+'"]');
          var t = ((h.textContent||'')+' '+(b?b.textContent:'' )).toLowerCase();
          var on = !qv || t.indexOf(qv)>=0;
          h.style.display = on ? '' : 'none';
          if(b) b.style.display = on ? '' : 'none';
        });
        mgrBody.querySelectorAll('.list-item').forEach(function(li){
          var on = !qv || String(li.textContent||'').toLowerCase().indexOf(qv)>=0;
          li.style.display = on ? '' : 'none';
        });
      };
    }

    function wireVisualSearchAndSections(){
      var inp = document.getElementById('vm-vis-search');
      if(inp){
        inp.oninput = function(){
          var qv = String(inp.value||'').toLowerCase().trim();
          visBody.querySelectorAll('details').forEach(function(d){
            var t = String(d.textContent||'').toLowerCase();
            d.style.display = (!qv || t.indexOf(qv)>=0) ? '' : 'none';
          });
          visBody.querySelectorAll('.list-item').forEach(function(li){
            var on = (!qv || String(li.textContent||'').toLowerCase().indexOf(qv)>=0);
            li.style.display = on ? '' : 'none';
          });
        };
      }
      var smBtn = document.getElementById('vm-stats-btn');
      var smMenu = document.getElementById('vm-stats-menu');
      var smSearch = document.getElementById('vm-stats-search');
      if(smBtn && smMenu){
        smBtn.onclick = function(ev){ ev.preventDefault(); ev.stopPropagation(); smMenu.style.display = (smMenu.style.display==='none' || !smMenu.style.display) ? 'block' : 'none'; };
        smMenu.addEventListener('click', function(ev){ ev.stopPropagation(); });
        document.addEventListener('click', function(){ if(smMenu) smMenu.style.display='none'; });
        var updateStatsBtn = function(){
          var c = visBody.querySelectorAll('input[data-opt-statsmode]:checked').length;
          smBtn.textContent = c ? ('Statistics ('+c+')') : 'Statistics (multi-select)';
        };
        visBody.querySelectorAll('input[data-opt-statsmode]').forEach(function(cb){ cb.onchange = updateStatsBtn; });
        updateStatsBtn();
        if(smSearch){
          smSearch.oninput = function(){
            var q = String(smSearch.value||'').toLowerCase().trim();
            smMenu.querySelectorAll('.vm-msel-item').forEach(function(row){
      ...
      visBody.querySelectorAll('[data-map-multi-btn]').forEach(function(btn){
        var key = btn.getAttribute('data-map-multi-btn');
        var menu = visBody.querySelector('[data-map-multi-menu="'+key+'"]');
        var sinp = visBody.querySelector('[data-map-multi-search="'+key+'"]');
        if(!menu) return;
        var updateBtn = function(){
          var cnt = menu.querySelectorAll('input[data-map-multi]:checked').length;
          btn.textContent = cnt ? (cnt + ' selected') : 'Select values';
        };
        btn.onclick = function(ev){ ev.preventDefault(); ev.stopPropagation(); visBody.querySelectorAll('[data-map-multi-menu]').forEach(function(m){ if(m!==menu) m.style.display='none'; }); menu.style.display = (menu.style.display==='none' || !menu.style.display) ? 'block' : 'none'; };
        menu.addEventListener('click', function(ev){ ev.stopPropagation(); });
        menu.querySelectorAll('input[data-map-multi]').forEach(function(cb){ cb.onchange = updateBtn; });
        updateBtn();
        if(sinp){
          sinp.oninput = function(){
            var q = String(sinp.value||'').toLowerCase().trim();
            menu.querySelectorAll('.vm-msel-item').forEach(function(row){
              var t = String(row.getAttribute('data-map-multi-label') || row.textContent || '').toLowerCase();
              row.style.display = (!q || t.indexOf(q)>=0) ? '' : 'none';
            });
          };
        }
      });
      document.addEventListener('click', function(){ visBody.querySelectorAll('[data-map-multi-menu]').forEach(function(m){ m.style.display='none'; }); });
      visBody.querySelectorAll('[data-vs-sections]').forEach(function(btn){
        btn.onclick = function(){
          var mode = btn.getAttribute('data-vs-sections');
          visBody.querySelectorAll('details').forEach(function(d){ d.open = (mode==='expand'); });
        };
      });
      visBody.querySelectorAll('details.vm-sec').forEach(function(d){
        d.addEventListener('toggle', function(){
          if(!d.open) return;
          visBody.querySelectorAll('details.vm-sec').forEach(function(o){ if(o!==d) o.open=false; });
        });
      });
    }
    ...
    function renderManager(){
      ...
      if(mgrTab==='layout'){
        ...
        h += '<div class="row" style="justify-content:flex-end;gap:6px;margin-bottom:8px">'
           + '<button class="btn" type="button" data-mgr-sections="expand">Expand all</button>'
           + '<button class="btn" type="button" data-mgr-sections="collapse">Collapse all</button>'
           + '</div>';
        h += '<div class="vm-settings-quicknav" style="margin-top:-2px">'
           + '<button class="chip" type="button" data-mgr-jump="general">General</button>'
           + '<button class="chip" type="button" data-mgr-jump="layout">Layout</button>'
           + '<button class="chip" type="button" data-mgr-jump="appearance">Appearance</button>'
           + '<button class="chip" type="button" data-mgr-jump="modebar">Modebar</button>'
           + '<button class="chip" type="button" data-mgr-jump="sections">Sections</button>'
           + '</div>';
        ...
        h += '<div class="section-hdr" data-section="general"><span class="arrow">&#9660;</span> General</div>'
          ...
          + '  <input id="mgr-title-link" value="'+escapeHtml(g.titleLink||'')+'" placeholder="Title hyperlink URL" />'
          + '  <select id="mgr-title-link-target" style="width:120px"><option value="_blank" '+((g.titleLinkTarget||'_blank')==='_blank'?'selected':'')+'>New tab</option><option value="_self" '+((g.titleLinkTarget||'_blank')==='_self'?'selected':'')+'>Same tab</option></select>'
          + '</div>'
          ...
          + '<div class="row" style="gap:8px;margin-top:6px">'
          + '  <input id="mgr-sub-link" value="'+escapeHtml(g.subtitleLink||'')+'" placeholder="Subtitle hyperlink URL" />'
          + '  <select id="mgr-sub-link-target" style="width:120px"><option value="_blank" '+((g.subtitleLinkTarget||'_blank')==='_blank'?'selected':'')+'>New tab</option><option value="_self" '+((g.subtitleLinkTarget||'_blank')==='_self'?'selected':'')+'>Same tab</option></select>'
          + '</div>'
          + '<div class="lbl" style="font-size:10px;margin-top:4px">Format</div>'
          + '<div class="row" style="gap:4px;margin-top:-6px;flex-wrap:wrap">'
        ...
        // === SECTION: Layout Mode ===
        h += '<div class="section-hdr" data-section="layout"><span class="arrow">&#9660;</span> Layout Mode <span class="s-badge">'+escapeHtml(mode==='custom'?'Custom Grid':'Automatic')+'</span></div>'
          + '<div class="section-body" data-section="layout">'
          + '<div class="form">'
          + '<div class="lbl">Mode</div>'
          + '<div class="row">'
        ...
        h += '<div class="sep"></div><div class="row"><label class="note"><input id="mgr-crossfilter-on" type="checkbox" '+(g.crossFilterEnabled?'checked':'')+'> Enable cross-filter graph between visuals</label></div>' + '<div class="row"><button class="btn" type="button" id="mgr-crossfilter-clear">Clear active cross-filter</button></div>' + '</div></div>';
        ...
        h += '<div class="section-hdr" data-section="appearance"><span class="arrow">&#9660;</span> Appearance</div>'
          ...
          + '</div>'
          + '<div class="lbl">Card shadow</div>'
          + '<div class="row">'
          + '  <select id="mgr-card-shadow" style="min-width:180px">'
          + '    <option value="none" '+((g.cardShadow||'soft')==='none'?'selected':'')+'>None</option>'
          + '    <option value="soft" '+((g.cardShadow||'soft')==='soft'?'selected':'')+'>Soft</option>'
          + '    <option value="medium" '+((g.cardShadow||'soft')==='medium'?'selected':'')+'>Medium</option>'
          + '    <option value="strong" '+((g.cardShadow||'soft')==='strong'?'selected':'')+'>Strong</option>'
          + '  </select>'
          + '</div>'
          + '<div class="lbl">Dashboard content width</div>'
          + '<div class="row">'
          + '  <select id="mgr-wrap-max" style="min-width:200px">'
          + '    <option value="full" '+((g.wrapMax||'full')==='full'?'selected':'')+'>Full width</option>'
          + '    <option value="1600" '+(String(g.wrapMax||'full')==='1600'?'selected':'')+'>1600px</option>'
          + '    <option value="1366" '+(String(g.wrapMax||'full')==='1366'?'selected':'')+'>1366px</option>'
          + '    <option value="1200" '+(String(g.wrapMax||'full')==='1200'?'selected':'')+'>1200px</option>'
          + '  </select>'
          + '</div>'
          + '<div class="lbl">Global Palette Preset</div>'
          + '<div class="row" style="gap:8px;flex-wrap:wrap">'
          + '  <select id="mgr-pal-preset" style="min-width:220px">'
          + '    <option value="">(Use custom values below)</option>'
          + '    <option value="tableau10" '+((g.palettePreset||'')==='tableau10'?'selected':'')+'>Tableau 10</option>'
          + '    <option value="powerbi" '+((g.palettePreset||'')==='powerbi'?'selected':'')+'>Power BI</option>'
          + '    <option value="oracle" '+((g.palettePreset||'')==='oracle'?'selected':'')+'>Oracle Analytics</option>'
          + '    <option value="pastel" '+((g.palettePreset||'')==='pastel'?'selected':'')+'>Pastel</option>'
          + '    <option value="vibrant" '+((g.palettePreset||'')==='vibrant'?'selected':'')+'>Vibrant</option>'
          + '    <option value="colorblind" '+((g.palettePreset||'')==='colorblind'?'selected':'')+'>Colorblind Safe</option>'
          + '    <option value="material" '+((g.palettePreset||'')==='material'?'selected':'')+'>Material</option>'
          + '    <option value="set2" '+((g.palettePreset||'')==='set2'?'selected':'')+'>Set 2</option>'
          + '  </select>'
          + '  <span class="note">Applies globally unless a visual overrides Palette.</span>'
          + '</div>'
        ...
        h += '<div class="section-hdr" data-section="modebar"><span class="arrow">&#9660;</span> Modebar & Interactions</div>'
        ...
        var mbDefNames = {reset:'Reset / Home', points:'Points / Labels / Styles', filter:'Outliers', zoom:'Zoom / Pan', status:'Statistics Table', remove:'Remove Point', download:'Download', fs:'Fullscreen'};
        ...
        mgrBody.querySelectorAll('[data-mgr-sections]').forEach(function(btn){
          btn.onclick = function(){
            var mode = btn.getAttribute('data-mgr-sections');
            mgrBody.querySelectorAll('.section-hdr').forEach(function(h){
              var sec = h.getAttribute('data-section');
              var b = mgrBody.querySelector('.section-body[data-section="'+sec+'"]');
              if(!b) return;
              ...
              b.style.maxHeight = collapse ? '0' : 'none';
            });
          };
        });
        ...
        mgrBody.querySelectorAll('.section-hdr').forEach(function(hdr){
          ...
          var sec = hdr.getAttribute('data-section');
          var body = mgrBody.querySelector('.section-body[data-section="'+sec+'"]');
          if(body){ hdr.classList.add('collapsed'); body.classList.add('collapsed'); body.style.maxHeight = '0'; }
        });

        // Wire quick-jump chips (General/Layout/Appearance/Modebar/Sections)
        mgrBody.querySelectorAll('[data-mgr-jump]').forEach(function(btn){
          btn.onclick = function(){
            var sec = btn.getAttribute('data-mgr-jump');
            var hdr = mgrBody.querySelector('.section-hdr[data-section="'+sec+'"]');
            var body = mgrBody.querySelector('.section-body[data-section="'+sec+'"]');
            if(!hdr || !body) return;
            hdr.classList.remove('collapsed');
            body.classList.remove('collapsed');
        ...
        var cfClearBtn = q('#mgr-crossfilter-clear');
        if(cfClearBtn){ cfClearBtn.onclick = function(){ mgrDraft.globals.crossFilter = null; renderManager(); }; }
        ...
        var palPresetSel = document.getElementById('mgr-pal-preset');
        if(palPresetSel){
          palPresetSel.onchange = function(){
            var pv = String(palPresetSel.value||'');
            mgrDraft.globals.palettePreset = pv;
            if(pv){
              var raw = vmPalettePresetRaw(pv);
              mgrDraft.globals.paletteLight = raw;
              mgrDraft.globals.paletteDark = raw;
              if(palLight) palLight.value = raw;
              if(palDark) palDark.value = raw;
            }
          };
        }
        ...
        var layoutLiveEls = [
          ...
          {id:'mgr-title-link', key:'titleLink'},
          {id:'mgr-title-link-target', key:'titleLinkTarget'},
          {id:'mgr-sub-link', key:'subtitleLink'},
          {id:'mgr-sub-link-target', key:'subtitleLinkTarget'},
          ...
          {id:'mgr-card-shadow', key:'cardShadow'},
          {id:'mgr-theme', key:'theme'},
          {id:'mgr-wrap-max', key:'wrapMax'},
          {id:'mgr-pal-preset', key:'palettePreset'},
        ...
        layoutLiveEls.forEach(function(cfg){
          ...
          el.onchange = function(){
            ...
            /* Keep draft only; apply on Save to avoid heavy menu redraws */
          };
        });
        ...
        if(btnTitleBold){
          btnTitleBold.onclick = function(ev){
            ...
            /* Defer grid redraw until Apply for better manager performance */
          };
        }
      ...
      if(mgrTab==='visuals'){
        ...
        mgrDraft.globals = mgrDraft.globals || {};
        mgrDraft.globals.candidateColumns = Array.isArray(mgrDraft.globals.candidateColumns) ? mgrDraft.globals.candidateColumns : [];

        var configuredTypeSet = {};
        for(var vc=0; vc<visuals.length; vc++){
          var tv = visuals[vc] && visuals[vc].type ? visuals[vc].type : '';
          if(tv) configuredTypeSet[tv] = true;
        }
        var configuredTypeCount = Object.keys(configuredTypeSet).length;

        vhtml += '<div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;margin-bottom:10px">'
          + '<div class="note">'+visuals.length+' visual'+(visuals.length!==1?'s':'')+' configured ('+configuredTypeCount+' unique types) &nbsp; <span class="badge">'+implCount+' types available</span>'+(niCount?' <span class="badge">'+niCount+' coming soon</span>':'')+' <span class="badge" title="Why counts differ">Configured count can be higher due to duplicate visual types.</span></div>'
          + '<div class="row" style="gap:8px">'
          + '  <select id="mgr-add-type" style="min-width:260px">'
          + '    <option value="__auto__">Auto (pick best visual from Candidate Columns)</option>';

        // Group visuals by category
        var categories = {};
        var categoryOrder = ['Trends & Distribution', 'Comparisons', 'Parts of Whole', 'Performance Metrics', 'Relationships', 'Data Display', 'Tables & Text', 'Other'];
        ...
        vhtml += '  </select>'
          + '  <input id="mgr-add-count" type="number" min="1" max="30" step="1" value="1" title="How many visuals to add" style="width:70px" />'
          + '  <button class="btn primary" id="mgr-add-btn" type="button">+ Add</button>'
          + '</div></div>';

        vhtml += '<div style="margin:0 0 10px">'
          + '<div class="lbl">Candidate Columns (multi-select)</div>'
          + '<div class="note" style="margin-bottom:6px">Used by <b>Auto</b> visual add mode to pick a best-fit chart type.</div>';
        if(ctx.cols && ctx.cols.length){
          vhtml += '<div class="vm-msel" id="mgr-candidate-wrap" style="width:100%">'
                + '<button type="button" class="btn vm-msel-btn" id="mgr-candidate-btn">Select columns</button>'
                + '<div class="vm-msel-menu" id="mgr-candidate-menu" style="display:none">'
                + '<input class="search" id="mgr-candidate-search" placeholder="Search..." />'
                + '<div id="mgr-candidate-list">';
          for(var cci=0; cci<ctx.cols.length; cci++){
            var ccol = ctx.cols[cci];
            var ck = String(ccol.key||'');
            var baseLbl = (ccol.label||ck) + ' [' + String(ccol.kind||'text') + ']';
            var checked = mgrDraft.globals.candidateColumns.indexOf(ck)>=0 ? 'checked' : '';
            vhtml += '<label class="vm-msel-item" data-key="'+escapeHtml(ck)+'" data-label="'+escapeHtml(baseLbl.toLowerCase())+'"><input type="checkbox" data-cand-col="'+escapeHtml(ck)+'" '+checked+'> '+escapeHtml(baseLbl)+'</label>';
          }
          vhtml += '</div></div></div>';
          vhtml += '<div class="note" style="margin-top:4px">Use search + checkboxes to select candidate columns.</div>';
        } else {
          vhtml += '<div class="note">No columns detected yet. Open <b>Data & Defaults</b> and load data first.</div>';
        }
        vhtml += '</div>';
        ...
        var candBtn = document.getElementById('mgr-candidate-btn');
        var candMenu = document.getElementById('mgr-candidate-menu');
        var candSearch = document.getElementById('mgr-candidate-search');
        var updateCandBtn = function(){
          if(!candBtn) return;
          var cnt = Array.isArray(mgrDraft.globals.candidateColumns) ? mgrDraft.globals.candidateColumns.length : 0;
          candBtn.textContent = cnt ? ('Candidate Columns ('+cnt+')') : 'Select columns';
        };
        if(candBtn && candMenu){
          updateCandBtn();
          candBtn.onclick = function(ev){ ev.preventDefault(); ev.stopPropagation(); candMenu.style.display = (candMenu.style.display==='none' || !candMenu.style.display) ? 'block' : 'none'; };
          candMenu.addEventListener('click', function(ev){ ev.stopPropagation(); });
          document.addEventListener('click', function(){ if(candMenu) candMenu.style.display='none'; });
          candMenu.querySelectorAll('input[data-cand-col]').forEach(function(cb){
            cb.onchange = function(){
              var arr=[]; candMenu.querySelectorAll('input[data-cand-col]').forEach(function(x){ if(x.checked) arr.push(String(x.getAttribute('data-cand-col')||'')); });
              mgrDraft.globals.candidateColumns = arr.filter(Boolean);
              updateCandBtn();
            };
          });
          if(candSearch){
            candSearch.oninput = function(){
              var q = String(candSearch.value||'').toLowerCase().trim();
              candMenu.querySelectorAll('label.vm-msel-item[data-label]').forEach(function(row){
                var t = String(row.getAttribute('data-label')||'');
                row.style.display = (!q || t.indexOf(q)>=0) ? '' : 'none';
              });
            };
          }
        ...
        addBtn.onclick = function(){
          ...
          var countEl = document.getElementById('mgr-add-count');
          var addCount = clamp(parseInt(countEl && countEl.value,10)||1, 1, 30);
          if(typ==='__auto__'){
            typ = suggestVisualTypeByCandidates(ctx.cols||[], mgrDraft.globals.candidateColumns||[]);
          }
          var def = Registry[typ];
          if(!def) return;
          for(var ai=0; ai<addCount; ai++){
            var v = def.defaultVisual();
            try{ vmAutoMapNthMeasure(v, ctx.cols||[], mgrDraft.globals.candidateColumns||[], ai); }catch(_){ }
      ...
      if(mgrTab==='help'){
        mgrBody.innerHTML = ''
          ...
          + '<div class="note">Visual Manager is a unified OAS Narrative template supporting <b>34 implemented visual types</b> (with additional NI placeholders) using a registry pattern. Each visual has mapping, aggregation, filtering, and layout options configured via the modal tabs (Layout, Visuals, Data).</div>'
          + '</div>'
          ...
          + '</div></div>'
          + '<div class="section-hdr" data-section="h-recent"><span class="arrow">&#9660;</span> Recent Changes & Enhancements</div>'
          + '<div class="section-body" data-section="h-recent">'
          + '<div class="note"><b>Layout quick-nav fixed:</b> General, Layout, Appearance, Modebar, and Sections chips now scroll to the correct section in Layout settings.</div>'
          + '<div class="note"><b>Visual Catalog enhancements:</b> Added <b>Auto</b> option in Add Visual dropdown and Candidate Columns multi-select to drive automatic visual recommendation.</div>'
          + '<div class="note"><b>Count clarification:</b> Visuals configured can exceed types available because multiple visuals can use the same chart type.</div>'
          + '<div class="note"><b>ChartEngine docs alignment:</b> Added roadmap coverage for event overlays, target lines/bands, advanced axis tick formatting, conditional formatting, and table/pivot workflows from ChartEngine reference docs.</div>'
          + '<div class="note"><b>Layout enhancements:</b> Added dashboard content width presets and card shadow levels inspired by chart-properties-layout panel design guidance.</div>'
          + '<div class="note"><b>Roadmap implemented now:</b> Text-only settings tabs (no icons), lighter candidate selection UI, and cached settings context for faster menu navigation.</div>'
          + '</div>'
    ...
    var btnMgrReset = q('#vm-manager-reset');
    function resetManagerCurrentTab(){
      var def = defaultState();
      if(!mgrDraft) mgrDraft = deepClone(def);
      if(mgrTab==='layout'){ mgrDraft.globals = deepClone(def.globals||{}); mgrDraft.sections = deepClone(def.sections||{}); }
      else if(mgrTab==='visuals'){ mgrDraft.visuals = deepClone(def.visuals||[]); }
      else if(mgrTab==='data'){ mgrDraft.data = deepClone(def.data||{}); }
    }
    if(btnMgrReset) btnMgrReset.onclick = function(){ resetManagerCurrentTab(); renderManager(); };
    ...
    if(btnMgrApply) btnMgrApply.onclick = function(){
      ...
      var cardShadowEl = q('#mgr-card-shadow');
      var wrapMaxEl = q('#mgr-wrap-max');
      ...
      if(cardShadowEl) mgrDraft.globals.cardShadow = String(cardShadowEl.value||'soft');
      if(wrapMaxEl) mgrDraft.globals.wrapMax = String(wrapMaxEl.value||'full');
      ...
      var palPresetEl = q('#mgr-pal-preset');
      if(palPresetEl) mgrDraft.globals.palettePreset = String(palPresetEl.value||'').trim();
      ...
      var cfOnEl = q('#mgr-crossfilter-on');
      if(cfOnEl) mgrDraft.globals.crossFilterEnabled = !!cfOnEl.checked;
      ...
      renderGrid();
      // Keep manager open after apply for multi-tab edits
      mgrDraft = deepClone(STATE);
      renderManager();
    };
    ...
    function openVisualSettings(id, tab){
      try{ vmCloseMenus(); }catch(_){}
      try{ vmPopClose(); }catch(_){}
      var idx = STATE.visuals.findIndex(function(v){return v && v.id===id;});
      if(idx<0) return;
      visId = id;
      visDraft = deepClone(ensureVisualDefaults(STATE.visuals[idx]));
      visTab = (tab === 'mapping' || tab === 'filter' || tab === 'options' || tab === 'advanced') ? tab : 'mapping';
      renderVisualSettings();
      try{ HOST.appendChild(visBackdrop); }catch(_){ }
    ...
    function vmRenderColumnPropertiesUI(v, ctx, cols){
      cols = Array.isArray(cols) ? cols : [];
      var cw=(v.options&&v.options.colWidths&&typeof v.options.colWidths==='object')?v.options.colWidths:{};
      var ca=(v.options&&v.options.colAligns&&typeof v.options.colAligns==='object')?v.options.colAligns:{};
      var cagg=(v.options&&v.options.colAggs&&typeof v.options.colAggs==='object')?v.options.colAggs:{};
      ...
      var cdate=(v.options&&v.options.colDateFmts&&typeof v.options.colDateFmts==='object')?v.options.colDateFmts:{};
      var cfont=(v.options&&v.options.colFontPx&&typeof v.options.colFontPx==='object')?v.options.colFontPx:{};
      var kindMap=Object.create(null); (ctx.cols||[]).forEach(function(c){ if(c&&c.key) kindMap[String(c.key)] = String(c.kind||'string'); });
      var html = '<div class="note" style="margin-bottom:6px">Set Width, Decimals, Alignment, Date format, and Font size per column.</div>';
      if(!cols.length) return html + '<div class="note">Map columns first to configure properties.</div>';
      html += '<div class="vm-colprops-wrap"><div class="vm-colprops-grid"><div class="vm-colprops-head"><span>Column</span><span>Width</span><span>Agg</span><span>Dec</span><span>Align</span><span>Date</span><span>Font px</span></div>';
      for(var i=0;i<cols.length;i++){
        var k=String(cols[i]||''); if(!k) continue;
        var kind=kindMap[k]||'string';
        html += '<div class="vm-colprops-row">'
          + '<span class="badge" title="'+escapeHtml(k)+'">'+escapeHtml(k)+'</span>'
          + '<input type="number" min="40" max="1200" step="10" data-opt-colw="'+escapeHtml(k)+'" value="'+escapeHtml(String(cw[k]==null?'':cw[k]))+'" placeholder="auto" />'
          + '<select data-opt-colagg="'+escapeHtml(k)+'">'+aggOptionsHtml(String(cagg[k]||'none'))+'</select>'
          + '<input type="number" min="0" max="10" step="1" data-opt-coldec="'+escapeHtml(k)+'" value="'+escapeHtml(String(cdec[k]==null?'':cdec[k]))+'" placeholder="auto" '+(kind==='number'?'':'disabled')+' />'
          + '<select data-opt-colalign="'+escapeHtml(k)+'"><option value="" '+((ca[k]||'')===''?'selected':'')+'>A</option><option value="left" '+(ca[k]==='left'?'selected':'')+'>L</option><option value="center" '+(ca[k]==='center'?'selected':'')+'>C</option><option value="right" '+(ca[k]==='right'?'selected':'')+'>R</option></select>'
          + '<select data-opt-coldate="'+escapeHtml(k)+'" '+(kind==='date'?'':'disabled')+'><option value="" '+((cdate[k]||'')===''?'selected':'')+'>Auto</option><option value="YYYY-MM-DD" '+(cdate[k]==='YYYY-MM-DD'?'selected':'')+'>YYYY-MM-DD</option><option value="MM/DD/YYYY" '+(cdate[k]==='MM/DD/YYYY'?'selected':'')+'>MM/DD/YYYY</option><option value="DD-MMM-YYYY" '+(cdate[k]==='DD-MMM-YYYY'?'selected':'')+'>DD-MMM-YYYY</option></select>'
          + '<input type="number" min="8" max="36" step="1" data-opt-colfont="'+escapeHtml(k)+'" value="'+escapeHtml(String(cfont[k]==null?'':cfont[k]))+'" placeholder="auto" />'
          + '</div>';
      }
      return html + '</div></div>';
    }

    function vmTypeChoicesFor(cur){
      cur = String(cur||'');
      ...
      if(cur==='stackedBar') return ['stackedBar','bar'];
      return [cur];
    }

    function vmRenderTableCfUI(v, cols){
      cols = Array.isArray(cols) ? cols : [];
      var cfRules = Array.isArray(v.options.cfRules) ? v.options.cfRules : [];
      var html = '<div style="border-top:1px solid var(--vm-border);margin:4px 0 8px;padding-top:10px"><div class="note" style="margin-bottom:8px">Color cells or rows based on conditions. Supports text and numeric columns.</div>';
      for(var ri=0;ri<cfRules.length;ri++){
        var rule = cfRules[ri]||{};
        html += '<div class="list-item" data-cf-rule="'+ri+'" style="display:flex;flex-wrap:wrap;align-items:center;gap:6px;padding:6px 8px;margin:0 0 4px;border:1px solid var(--vm-border);border-radius:8px;background:var(--vm-input2)">'
          + '<select data-cf="col" style="min-width:120px"><option value="">Column\u2026</option>';
        for(var ci=0;ci<cols.length;ci++) html += '<option value="'+escapeHtml(cols[ci])+'" '+(rule.col===cols[ci]?'selected':'')+'>'+escapeHtml(cols[ci])+'</option>';
        html += '</select>'
          + '<select data-cf="op" style="width:90px"><option value="gt" '+(rule.op==='gt'?'selected':'')+'>&gt;</option><option value="gte" '+(rule.op==='gte'?'selected':'')+'>&ge;</option><option value="lt" '+(rule.op==='lt'?'selected':'')+'>&lt;</option><option value="lte" '+(rule.op==='lte'?'selected':'')+'>&le;</option><option value="eq" '+(rule.op==='eq'?'selected':'')+'>=</option><option value="neq" '+(rule.op==='neq'?'selected':'')+'>&ne;</option><option value="between" '+(rule.op==='between'?'selected':'')+'>Between</option><option value="contains" '+(rule.op==='contains'?'selected':'')+'>Contains</option><option value="notcontains" '+(rule.op==='notcontains'?'selected':'')+'>Not Contains</option><option value="startswith" '+(rule.op==='startswith'?'selected':'')+'>Starts With</option><option value="endswith" '+(rule.op==='endswith'?'selected':'')+'>Ends With</option><option value="isblank" '+(rule.op==='isblank'?'selected':'')+'>Is Blank</option><option value="notblank" '+(rule.op==='notblank'?'selected':'')+'>Not Blank</option></select>'
          ...
          + '<span class="note">Bg</span><input data-cf="bg" type="color" value="'+(rule.bg||'#fca5a5')+'" style="width:30px;height:24px;cursor:pointer;border:1px solid var(--vm-border);border-radius:4px" />'
          + '<span class="note">Text</span><input data-cf="fg" type="color" value="'+(rule.fg||'#111827')+'" style="width:30px;height:24px;cursor:pointer;border:1px solid var(--vm-border);border-radius:4px" />'
          + '<span class="note">Apply</span><select data-cf="applyTo" style="width:60px"><option value="cell" '+(rule.applyTo==='cell'||!rule.applyTo?'selected':'')+'>Cell</option><option value="row" '+(rule.applyTo==='row'?'selected':'')+'>Row</option></select>'
          + '<button class="btn danger" data-act="del-cf" style="padding:2px 8px;font-size:11px" title="Remove rule">&#10005;</button></div>';
      }
      return html + '<button class="btn" data-act="add-cf" style="margin-top:4px;font-size:12px">+ Add Rule</button></div>';
    }
    ...
    function renderVisualSettings(){
      ...
      var html='';
      html += '<div class="row" style="justify-content:flex-end;gap:6px;margin-bottom:6px"><button class="btn" type="button" data-vs-sections="expand">Expand all</button><button class="btn" type="button" data-vs-sections="collapse">Collapse all</button></div>';
      if(visTab==='options') html += '<div class="row" style="margin-bottom:8px"><input id="vm-vis-search" placeholder="Search functions in this tab..." style="width:100%" /></div>';
      ...
      if(visTab==='mapping'){
        ...
        html += '<details class="vm-sec"'+detOpen('Mapping', false)+'>'
             + '<summary class="lbl" style="font-weight:700;cursor:pointer">Mapping</summary>'
             + '<div style="margin-top:8px">'
             + def.buildMappingUI(visDraft, ctx)
             + '</div></details>';
        var mapTypeChoices = vmTypeChoicesFor(visDraft.type);
        if(mapTypeChoices.length>1){
          html += '<div class="sep"></div>';
          html += '<details class="vm-sec"'+detOpen('Type / View', true)+'>';
          html += '<summary class="lbl" style="font-weight:700;cursor:pointer">Type / View</summary>';
          html += '<div class="form" style="margin-top:8px"><div class="row" style="gap:8px;align-items:center">'
               + '  <select data-core="type" style="min-width:200px">'
               + mapTypeChoices.map(function(t){ var sel=(t===visDraft.type)?' selected':''; return '<option value="'+escapeHtml(t)+'"'+sel+'>'+escapeHtml(t.charAt(0).toUpperCase()+t.slice(1))+'</option>'; }).join('')
               + '  </select><span class="note">Switch compatible view type.</span></div></div></details>';
        }
      } else if(visTab==='options'){
        var isCartesian = isCartesianType(visDraft.type);
        var isTableLike = (visDraft.type==='table' || visDraft.type==='pivot');
        var isTable = visDraft.type==='table';
        var chips = [
          {k:'title',t:'Title',on:true},
          {k:'card',t:'Card',on:true},
          {k:'presets',t:'Presets',on:true},
          {k:'style',t:'Style',on:!isTableLike},
          {k:'stats',t:'Stats',on:!isTableLike},
          {k:'legend',t:'Legend',on:!isTableLike},
          {k:'labels',t:'Labels',on:!isTableLike},
          {k:'sort',t:'Sort',on:true},
          {k:'axes',t:'Axes',on:isCartesian},
          {k:'ticks',t:'Ticks',on:isCartesian},
          {k:'targets',t:'Targets',on:isCartesian},
          {k:'refs',t:'Refs',on:isCartesian},
          {k:'palette',t:'Palette',on:true},
          {k:'axistitle',t:'Axis Title',on:isCartesian},
          {k:'modebar',t:'Modebar',on:true},
          {k:'cf',t:'Cond. Format',on:true},
          {k:'filter',t:'Filter',on:true},
          {k:'colprops',t:'Column Props',on:isTableLike},
          {k:'tablecf',t:'Table/Pivot CF',on:isTableLike}
        ];
        html += '<div class="vm-settings-quicknav">' + chips.filter(function(c){return c.on;}).map(function(c){
          return '<button class="chip" type="button" data-vs-jump="'+escapeHtml(c.k)+'">'+escapeHtml(c.t)+'</button>';
        }).join('') + '</div>';

        html += '<details class="vm-sec"'+detOpen('Title', true)+'>'
          + '<summary class="lbl" data-vs-anchor="title" style="font-weight:700;cursor:pointer">Title</summary>'
          ...
          + '  <label class="note"><input type="checkbox" data-core="setGlobalTitleDefaults"> Apply title format as global default</label>'
          + '</div>'
          + '</div></details>';
        html += '<div class="sep"></div>';
        html += '<details class="vm-sec"'+detOpen('Card Appearance', true)+'>'
          + '<summary class="lbl" data-vs-anchor="card" style="font-weight:700;cursor:pointer">Card Appearance</summary>'
          + '<div class="form" style="margin-top:8px">'
          + '<div class="row" style="gap:8px">'
          + '  <span class="note">Background</span>'
        ...
        html += '<details class="vm-sec"'+detOpen('Formatting Presets', false)+'>'
             + '<summary class="lbl" data-vs-anchor="presets" style="font-weight:700;cursor:pointer">Formatting Presets</summary>'
             + '<div class="note">One-click style presets for titles, axes, labels, and spacing.</div>'
             + '<div class="vm-fmt-presets">'
             + '  <button class="btn" type="button" data-visfmt="executive">Executive</button>'
             + '  <button class="btn" type="button" data-visfmt="compact">Compact</button>'
             + '  <button class="btn" type="button" data-visfmt="analysis">Analysis</button>'
             + '</div>'
             + '</details>';

        html += '<details class="vm-sec"'+detOpen('Chart Options', false)+'>'
             + '<summary class="lbl" data-vs-anchor="style" style="font-weight:700;cursor:pointer">Visual Behavior & Style</summary>'
             + '<div style="margin-top:8px">'
             + def.buildOptionsUI(visDraft, ctx)
             + '</div></details>';

        if(visDraft.type==='table' || visDraft.type==='pivot'){
          var cpCols = [];
          if(visDraft.type==='table') cpCols = (visDraft.mapping && Array.isArray(visDraft.mapping.cols) && visDraft.mapping.cols.length) ? visDraft.mapping.cols.slice() : (ctx.cols||[]).map(function(c){ return c && c.key ? String(c.key) : ''; }).filter(Boolean);
          else {
            var mr = (visDraft.mapping && Array.isArray(visDraft.mapping.row)) ? visDraft.mapping.row : [];
            var mc = (visDraft.mapping && Array.isArray(visDraft.mapping.col)) ? visDraft.mapping.col : [];
            var mv = (visDraft.mapping && Array.isArray(visDraft.mapping.values)) ? visDraft.mapping.values : [];
            cpCols = mr.concat(mc).concat(mv).filter(Boolean);
          }
          html += '<div class="sep"></div>';
          html += '<details class="vm-sec"'+detOpen('Column Properties', true)+'>';
          html += '<summary class="lbl" data-vs-anchor="colprops" style="font-weight:700;cursor:pointer">Column Properties</summary>';
          html += '<div style="margin-top:8px">'+vmRenderColumnPropertiesUI(visDraft, ctx, cpCols)+'</div>';
          html += '</details>';
          if(visDraft.type==='table' || visDraft.type==='pivot'){
            html += '<div class="sep"></div>';
            html += '<details class="vm-sec"'+detOpen('Conditional Formatting', true)+'>';
            html += '<summary class="lbl" data-vs-anchor="tablecf" style="font-weight:700;cursor:pointer">Conditional Formatting</summary>';
            html += '<div style="margin-top:8px">'+vmRenderTableCfUI(visDraft, cpCols)+'</div>';
            html += '</details>';
          }
        }

        // --- Statistics / Regression ---
        var visStatsModes = Array.isArray(visDraft.options.statsModes) ? visDraft.options.statsModes.slice() : [];
        if(!visStatsModes.length && visDraft.options.statsMode && visDraft.options.statsMode!=='none') visStatsModes=[String(visDraft.options.statsMode)];
        visDraft.options.statsColors = visDraft.options.statsColors || {};
        html += '<div class="sep"></div>';
        html += '<details class="vm-sec"'+detOpen('Statistics / Regression', false)+'>';
        html += '<summary class="lbl" data-vs-anchor="stats" style="font-weight:700;cursor:pointer">Statistics / Regression</summary>';
        html += '<div class="form" style="margin-top:8px">';
        function vsChk(mode,label){
          return '<input type="checkbox" data-opt-statsmode="'+mode+'" '+(visStatsModes.indexOf(mode)>=0?'checked':'')+'> <span class="note">'+label+'</span>';
        }
        html += '<div class="vm-msel" id="vm-stats-msel">'
              + '<button type="button" class="btn vm-msel-btn" id="vm-stats-btn">Statistics (multi-select)</button>'
              + '<div class="vm-msel-menu" id="vm-stats-menu" style="display:none">'
              + '  <input class="search" id="vm-stats-search" placeholder="Search..." />'
              + '  <label class="vm-msel-item">'+vsChk('mean','Mean')+'</label>'
              + '  <label class="vm-msel-item">'+vsChk('median','Median')+'</label>'
              + '  <label class="vm-msel-item">'+vsChk('ci','CI / Band')+'</label>'
              + '  <label class="vm-msel-item">'+vsChk('regression','Regression')+'</label>'
              + '</div></div>';
        html += '<div class="row" style="gap:8px;flex-wrap:wrap;margin-top:6px">'
              + '<label class="note">Mean <input type="color" data-opt="statsColorMean" value="'+escapeHtml(visDraft.options.statsColors.mean||'#ff0000')+'"></label>'
              + '<label class="note">Median <input type="color" data-opt="statsColorMedian" value="'+escapeHtml(visDraft.options.statsColors.median||'#0000ff')+'"></label>'
              + '<label class="note">CI <input type="color" data-opt="statsColorCi" value="'+escapeHtml(visDraft.options.statsColors.ci||'#008000')+'"></label>'
              + '<label class="note">Regression <input type="color" data-opt="statsColorRegression" value="'+escapeHtml(visDraft.options.statsColors.regression||'#ffa500')+'"></label>'
              + '</div>';
        html += '<div class="row" style="gap:8px;flex-wrap:wrap;margin-top:6px">'
              + '<label class="note"><input type="checkbox" data-opt="regressionShowOutliers" '+(visDraft.options.regressionShowOutliers?'checked':'')+'> Show regression outliers</label>'
              + '<select data-opt="outlierMethod" style="min-width:120px"><option value="kmeans" '+((visDraft.options.outlierMethod||'kmeans')==='kmeans'?'selected':'')+'>KMeans</option><option value="distance" '+(visDraft.options.outlierMethod==='distance'?'selected':'')+'>Distance</option><option value="hierarchical" '+(visDraft.options.outlierMethod==='hierarchical'?'selected':'')+'>Hierarchical</option><option value="std" '+(visDraft.options.outlierMethod==='std'?'selected':'')+'>STD</option><option value="iqr" '+(visDraft.options.outlierMethod==='iqr'?'selected':'')+'>IQR</option><option value="mad" '+(visDraft.options.outlierMethod==='mad'?'selected':'')+'>MAD</option></select>'
              + '<input data-opt="outlierK" type="number" step="0.1" min="0.5" max="5" value="'+escapeHtml(String(visDraft.options.outlierK!=null?visDraft.options.outlierK:2))+'" style="width:80px" />'
              + '<label class="note">CI % <input data-opt="ciLevel" type="number" min="50" max="99.9" step="0.1" value="'+escapeHtml(String(visDraft.options.ciLevel!=null?visDraft.options.ciLevel:95))+'" style="width:74px"></label>'
              + '<label class="note">Outlier color <input type="color" data-opt="statsColorOutlier" value="'+escapeHtml(visDraft.options.statsColors.outlier||'#ef4444')+'"></label>'
              + '</div>';
        if(visDraft.type==='scatter'){
          html += '<div class="row" style="gap:8px;flex-wrap:wrap;margin-top:6px">'
                + '<label class="note">Stats apply <select data-opt="scatterStatsMode"><option value="per-series" '+((visDraft.options.scatterStatsMode||'per-series')==='per-series'?'selected':'')+'>Per series</option><option value="combined" '+((visDraft.options.scatterStatsMode||'per-series')==='combined'?'selected':'')+'>Combined</option></select></label>'
                + '<label class="note">Mean / Median axes <select data-opt="scatterAxisStats"><option value="y" '+((visDraft.options.scatterAxisStats||'y')==='y'?'selected':'')+'>Y only</option><option value="xy" '+((visDraft.options.scatterAxisStats||'y')==='xy'?'selected':'')+'>X + Y</option></select></label>'
                + '<label class="note">Regression axes <select data-opt="scatterRegressionAxes"><option value="y" '+((visDraft.options.scatterRegressionAxes||'y')==='y'?'selected':'')+'>Y on X</option><option value="both" '+((visDraft.options.scatterRegressionAxes||'y')==='both'?'selected':'')+'>Both</option></select></label>'
                + '</div>';
        }
        if(visDraft.type==='regHist'){
          html += '<div class="row" style="gap:10px;flex-wrap:wrap;margin-top:6px">'
                + '<label class="note"><input type="checkbox" data-opt="regressionShowYMean" '+(visDraft.options.regressionShowYMean?'checked':'')+'> Y Mean</label>'
                + '<label class="note"><input type="checkbox" data-opt="regressionShowYMedian" '+(visDraft.options.regressionShowYMedian?'checked':'')+'> Y Median</label>'
                + '</div>';
        }
        html += '<div class="note">Multiple overlays can be shown together (Power BI/Tableau style).</div>';
        html += '</div>';
        html += '</details>';

          // --- Legend & Tooltip ---
          html += '<div class="sep"></div>';
          html += '<details class="vm-sec"'+detOpen('Legend & Tooltip', false)+'>';
          html += '<summary class="lbl" data-vs-anchor="legend" style="font-weight:700;cursor:pointer">Legend & Tooltip</summary>';
          html += '<div class="form" style="margin-top:8px">';
          ...
          // --- Data Labels ---
          html += '<div class="sep"></div>';
          html += '<details class="vm-sec"'+detOpen('Data Labels', false)+'>';
          html += '<summary class="lbl" data-vs-anchor="labels" style="font-weight:700;cursor:pointer">Data Labels</summary>';
          html += '<div class="form" style="margin-top:8px">';
          html += '<div class="row"><label class="note"><input type="checkbox" data-opt="showLabels" '+((visDraft.options.showLabels||visDraft.options.labelsOn)?'checked':'')+'> Show labels</label></div>';
          html += '<div class="row" style="gap:8px;align-items:center">'
            ...
            + '</div>';
            html += '<div class="row" style="gap:8px;align-items:center;flex-wrap:wrap;margin-top:6px">'
          ...
          html += '</details>';
            ...
            // --- Sort & Order ---
            html += '<div class="sep"></div>';
            html += '<details class="vm-sec"'+detOpen('Sort & Order', false)+'>';
            html += '<summary class="lbl" data-vs-anchor="sort" style="font-weight:700;cursor:pointer">Sort & Order</summary>';
            html += '<div class="form" style="margin-top:8px">';
            html += '<div class="row" style="gap:8px;align-items:center">'
        ...
        if(isCartesianType(visDraft.type)){
          html += '<div class="sep"></div>';
          html += '<details class="vm-sec"'+detOpen('Axes / Scale', false)+'>';
          html += '<summary class="lbl" data-vs-anchor="axes" style="font-weight:700;cursor:pointer">Axes / Scale</summary>';
          html += '<div class="form" style="margin-top:8px">';
          ...
          html += '<div class="row" style="gap:8px;align-items:center;flex-wrap:wrap">'
                ...
                + '  <span class="note" style="min-width:72px">Y1 range</span>'
                + '  <input data-opt="yMin" type="number" placeholder="Y1 min" value="'+(Number.isFinite(visDraft.options.yMin)?visDraft.options.yMin:'')+'" style="width:110px" />'
                + '  <input data-opt="yMax" type="number" placeholder="Y1 max" value="'+(Number.isFinite(visDraft.options.yMax)?visDraft.options.yMax:'')+'" style="width:110px" />'
                + '</div>';
          var hasY2 = !!(visDraft.measures && visDraft.measures.y2 && visDraft.measures.y2.colKey);
          var hasY3 = !!(visDraft.measures && visDraft.measures.y3 && visDraft.measures.y3.colKey);
          var hasY4 = !!(visDraft.measures && visDraft.measures.y4 && visDraft.measures.y4.colKey);
          if(hasY2){
            html += '<div class="row" style="gap:8px;align-items:center;flex-wrap:wrap;margin-top:6px">'
                 + '  <span class="note" style="min-width:72px">Y2 range</span>'
                 + '  <input data-opt="y2Min" type="number" placeholder="Y2 min" value="'+(Number.isFinite(visDraft.options.y2Min)?visDraft.options.y2Min:'')+'" style="width:110px" />'
                 + '  <input data-opt="y2Max" type="number" placeholder="Y2 max" value="'+(Number.isFinite(visDraft.options.y2Max)?visDraft.options.y2Max:'')+'" style="width:110px" />'
                 + '</div>';
          }
          if(hasY3){
            html += '<div class="row" style="gap:8px;align-items:center;flex-wrap:wrap;margin-top:6px">'
                 + '  <span class="note" style="min-width:72px">Y3 range</span>'
                 + '  <input data-opt="y3Min" type="number" placeholder="Y3 min" value="'+(Number.isFinite(visDraft.options.y3Min)?visDraft.options.y3Min:'')+'" style="width:110px" />'
                 + '  <input data-opt="y3Max" type="number" placeholder="Y3 max" value="'+(Number.isFinite(visDraft.options.y3Max)?visDraft.options.y3Max:'')+'" style="width:110px" />'
                 + '</div>';
          }
          if(hasY4){
            html += '<div class="row" style="gap:8px;align-items:center;flex-wrap:wrap;margin-top:6px">'
                 + '  <span class="note" style="min-width:72px">Y4 range</span>'
                 + '  <input data-opt="y4Min" type="number" placeholder="Y4 min" value="'+(Number.isFinite(visDraft.options.y4Min)?visDraft.options.y4Min:'')+'" style="width:110px" />'
                 + '  <input data-opt="y4Max" type="number" placeholder="Y4 max" value="'+(Number.isFinite(visDraft.options.y4Max)?visDraft.options.y4Max:'')+'" style="width:110px" />'
                 + '</div>';
          }
        ...
        if(isCartesianType(visDraft.type)){
          html += '<div class="sep"></div>';
          html += '<details class="vm-sec"'+detOpen('Axis Ticks & Formatting', false)+'>';
          html += '<summary class="lbl" data-vs-anchor="ticks" style="font-weight:700;cursor:pointer">Axis Ticks & Formatting</summary>';
          html += '<div class="form" style="margin-top:8px">';
          ...
          html += '<div class="row" style="gap:8px;align-items:center;flex-wrap:wrap">'
                + '  <input data-opt="xTickFormat" placeholder="Manual X (e.g. %Y)" value="'+escapeHtml(String(visDraft.options.xTickFormat||''))+'" style="min-width:240px;flex:1" title="Custom Plotly tickformat string" />'
                + '  <input data-opt="yTickFormat" placeholder="Manual Y (e.g. .2s)" value="'+escapeHtml(String(visDraft.options.yTickFormat||''))+'" style="min-width:240px;flex:1" title="Custom Plotly tickformat string" />'
                + '</div>';
          html += '<div class="row" style="gap:8px;align-items:center;flex-wrap:wrap;margin-top:6px">'
        ...
        if(isCartesianType(visDraft.type)){
          html += '<div class="sep"></div>';
          html += '<details class="vm-sec">';
          html += '<summary class="lbl" data-vs-anchor="targets" style="font-weight:700;cursor:pointer">Target Lines / Bands</summary>';
          html += '<div class="form" style="margin-top:8px">';
            html += '<div class="row" style="gap:8px;align-items:center;flex-wrap:wrap">'
              + '  <select data-opt="targetMode" style="min-width:150px">'
              + '    <option value="none" '+((visDraft.options.targetMode||'none')==='none'?'selected':'')+'>None</option>'
              + '    <option value="manual" '+(visDraft.options.targetMode==='manual'?'selected':'')+'>Manual</option>'
              ...
              + '</div>';
          html += '<div class="row" style="gap:8px;align-items:center;flex-wrap:wrap">'
                + '  <select data-opt="targetStyle" style="min-width:140px">'
                ...
                + '  </select>'
              + '  <select data-opt="targetDash" style="min-width:130px">'
        ...
        if(isCartesianType(visDraft.type)){
          html += '<div class="sep"></div>';
          html += '<details class="vm-sec">';
          html += '<summary class="lbl" data-vs-anchor="refs" style="font-weight:700;cursor:pointer">Reference Lines</summary>';
          html += '<div class="form" style="margin-top:8px">';
          html += '<div class="lbl">Enable</div><div><label class="note"><input data-opt="showRefLines" type="checkbox" '+(visDraft.options.showRefLines?'checked':'')+'> Show reference lines</label></div>';
        ...
        html += '<div class="sep"></div>';
        html += '<details class="vm-sec">';
        html += '<summary class="lbl" data-vs-anchor="palette" style="font-weight:700;cursor:pointer">Palette</summary>';
        html += '<div class="form" style="margin-top:8px">';
        var palPreset = (visDraft.options && visDraft.options.palettePreset) ? String(visDraft.options.palettePreset) : vmPalettePresetNameFor(visDraft.options.palette||'');
        html += '<div class="note">Choose a preset palette or Custom to provide your own colors.</div>';
        html += '<select data-opt="palettePreset" style="min-width:220px">'
              + '<option value="tableau10" '+(palPreset==='tableau10'?'selected':'')+'>Tableau 10</option>'
              + '<option value="powerbi" '+(palPreset==='powerbi'?'selected':'')+'>Power BI</option>'
              + '<option value="oracle" '+(palPreset==='oracle'?'selected':'')+'>Oracle Analytics</option>'
              + '<option value="pastel" '+(palPreset==='pastel'?'selected':'')+'>Pastel</option>'
              + '<option value="vibrant" '+(palPreset==='vibrant'?'selected':'')+'>Vibrant</option>'
              + '<option value="colorblind" '+(palPreset==='colorblind'?'selected':'')+'>Colorblind Safe</option>'
              + '<option value="material" '+(palPreset==='material'?'selected':'')+'>Material</option>'
              + '<option value="set2" '+(palPreset==='set2'?'selected':'')+'>Set 2</option>'
              + '<option value="custom" '+(palPreset==='custom'?'selected':'')+'>Custom</option>'
              + '</select>';
        html += '<div class="note">Custom colors (comma-separated hex). Used only when preset is Custom.</div>';
        ...
        if(isCartesianType(visDraft.type)){
          html += '<div class="sep"></div>';
          html += '<details class="vm-sec"'+detOpen('Axis Title', false)+'>';
          html += '<summary class="lbl" data-vs-anchor="axistitle" style="font-weight:700;cursor:pointer">Axis Title</summary>';
          html += '<div class="form" style="margin-top:8px">';
          ...
          html += '<div class="row" style="gap:12px; align-items:flex-end;">'
                ...
                + '  </div>'
                + ( (visDraft.measures && visDraft.measures.y2 && visDraft.measures.y2.colKey) ? ('  <div><div class="note">Y2 Axis Title</div><input data-opt="y2Title" placeholder="Auto" value="'+escapeHtml(opts.y2Title||'')+'" style="width:140px" /></div>') : '' )
                + '</div>';
        ...
        html += '<div class="sep"></div>';
        html += '<details class="vm-sec"'+detOpen('Modebar Icons', false)+'>';
        html += '<summary class="lbl" data-vs-anchor="modebar" style="font-weight:700;cursor:pointer">Modebar Icons</summary>';
        html += '<div class="form" style="margin-top:8px">';
        ...
        var mbNames = {reset:'Reset / Home', points:'Points / Labels / Styles', yaxis:'Y-axis', targets:'Targets', filter:'Outliers', zoom:'Zoom / Pan', status:'Statistics Table', remove:'Remove Point', download:'Download', fs:'Fullscreen', settings:'Settings'};
        ...
        html += '<div class="sep"></div>';
        html += '<details class="vm-sec"'+detOpen('Conditional Formatting', false)+'>';
        html += '<summary class="lbl" data-vs-anchor="cf" style="font-weight:700;cursor:pointer">Conditional Formatting</summary>';
        html += '<div class="form" style="margin-top:8px">';
        ...
        html += '<details class="vm-sec"'+detOpen('Filter', false)+'>'
             + '<summary class="lbl" data-vs-anchor="filter" style="font-weight:700;cursor:pointer">Filter</summary>'
             + '<div style="margin-top:8px">'
             + renderFilterUI(visDraft, ctx)
             + '</div></details>';
        html += '<div class="sep"></div>';
        html += '<details class="vm-sec"'+detOpen('Outlier Filter', false)+'>';
        html += '<summary class="lbl" data-vs-anchor="outliers" style="font-weight:700;cursor:pointer">Outlier Filter</summary>';
        html += '<div class="form" style="margin-top:8px"><div class="row"><label class="note"><input type="checkbox" data-opt="excludeOutliers" '+(visDraft.options.excludeOutliers?'checked':'')+'> Exclude outliers</label></div>'
             + '<div class="row" style="gap:8px;align-items:center;flex-wrap:wrap">'
             + '  <select data-opt="outlierMethod" style="min-width:160px">'
             + '    <option value="iqr" '+((visDraft.options.outlierMethod||'iqr')==='iqr'?'selected':'')+'>IQR</option>'
             + '    <option value="std" '+(visDraft.options.outlierMethod==='std'?'selected':'')+'>Std Dev</option>'
             + '    <option value="mad" '+(visDraft.options.outlierMethod==='mad'?'selected':'')+'>MAD</option>'
             + '  </select>'
             + '  <input data-opt="outlierK" type="number" step="0.1" min="0.5" max="10" value="'+escapeHtml(String(visDraft.options.outlierK||2))+'" style="width:90px" />'
             + '  <span class="note">Sensitivity</span>'
             + '</div></div></details>';
      } else {
      ...
      visBody.querySelectorAll('[data-act="add-refline"]').forEach(function(btn){
        btn.onclick = function(){
          visDraft.options = visDraft.options || {};
          visDraft.options.showRefLines = true;
      ...
      visBody.querySelectorAll('[data-vs-jump]').forEach(function(btn){
        btn.onclick = function(){
          var key = btn.getAttribute('data-vs-jump');
          var sum = visBody.querySelector('summary[data-vs-anchor="'+key+'"]');
          if(!sum) return;
          var det = sum.closest('details');
          if(det && !det.open) det.open = true;
          try{ sum.scrollIntoView({behavior:'smooth', block:'start'}); }catch(_){ }
        };
      });

      visBody.querySelectorAll('[data-visfmt]').forEach(function(btn){
        btn.onclick = function(){
          visDraft.options = visDraft.options || {};
          var p = String(btn.getAttribute('data-visfmt')||'');
          if(p==='executive'){
            visDraft.titleSize = 16;
            visDraft.options.axisTitleFontSize = 13;
            visDraft.options.xTickAngle = 0;
            visDraft.options.yMode = 'zero';
            visDraft.options.showLabels = false;
            visDraft.options.legendPos = 'bottom';
          }else if(p==='compact'){
            visDraft.titleSize = 13;
            visDraft.options.axisTitleFontSize = 11;
            visDraft.options.xTickAngle = 45;
            visDraft.options.showLabels = false;
            visDraft.options.legendPos = 'right';
            visDraft.options.opacity = 0.75;
          }else if(p==='analysis'){
            visDraft.titleSize = 14;
            visDraft.options.axisTitleFontSize = 12;
            visDraft.options.xTickAngle = 0;
            visDraft.options.showLabels = true;
            visDraft.options.legendPos = 'top';
            visDraft.options.statsMode = visDraft.options.statsMode || 'mean';
            if(!Array.isArray(visDraft.options.statsModes) || !visDraft.options.statsModes.length) visDraft.options.statsModes = ['mean','median'];
          }
          renderVisualSettingsDebounced();
        };
      });
    }
    ...
    var btnVisReset = q('#vm-visual-reset');
    if(btnVisClose) btnVisClose.onclick = closeVisualSettings;
    if(btnVisCancel) btnVisCancel.onclick = closeVisualSettings;
    if(btnVisReset) btnVisReset.onclick = function(){
      if(!visDraft) return;
      var keepId = visDraft.id;
      var keepType = visDraft.type;
      var def = Registry[keepType] && Registry[keepType].defaultVisual ? ensureVisualDefaults(Registry[keepType].defaultVisual()) : ensureVisualDefaults({options:{}, filter:defaultFilter(), transform:defaultTransform('raw'), layout:{}, mapping:{}, measures:{}});
      if(visTab==='mapping'){
        visDraft.mapping = deepClone(def.mapping||{});
        visDraft.measures = deepClone(def.measures||{});
      }else if(visTab==='filter'){
        visDraft.filter = deepClone(def.filter||defaultFilter());
      }else if(visTab==='advanced'){
        visDraft.transform = deepClone(def.transform||defaultTransform('raw'));
        visDraft.layout = deepClone(def.layout||{});
      }else if(visTab==='options'){
        visDraft.options = deepClone(def.options||{});
        visDraft.title = def.title || visDraft.title || '';
        visDraft.showTitle = (typeof def.showTitle==='boolean') ? def.showTitle : true;
        visDraft.titleAlign = def.titleAlign || 'left';
        visDraft.titleSize = def.titleSize || 14;
        visDraft.titleColor = def.titleColor || '#111827';
        visDraft.cardBgColor = def.cardBgColor || '#ffffff';
        visDraft.cardBorderColor = def.cardBorderColor || '#e5e7eb';
        visDraft.cardBorderWidth = (def.cardBorderWidth!=null)?def.cardBorderWidth:1;
        visDraft.cardBorderRadius = (def.cardBorderRadius!=null)?def.cardBorderRadius:10;
      }
      visDraft.id = keepId;
      visDraft.type = keepType;
      renderVisualSettings();
    };
    ...
    if(btnVisApply) btnVisApply.onclick = function(){
      ...
      var setGlobalTitleEl = visBody.querySelector('input[data-core="setGlobalTitleDefaults"]');
      if(setGlobalTitleEl && setGlobalTitleEl.checked){
        STATE.globals = STATE.globals || {};
        STATE.globals.visualTitleDefaults = {
          align: visDraft.titleAlign || 'left',
          size: clamp(parseInt(visDraft.titleSize,10)||14, 10, 28),
          color: visDraft.titleColor || '#111827'
        };
      }
      ...
      measEls.forEach(function(el){
        ...
        if(part==='colorMode') visDraft.measures[key].colorMode = String(el.value||'palette');
        if(part==='color'){
           var clr = String(el.value||'');
           var mode = String((visDraft.measures[key] && visDraft.measures[key].colorMode) || 'palette');
           visDraft.measures[key].color = (mode==='custom' && clr) ? clr : null;
        }
      });
      ...
      optEls.forEach(function(el){
        ...
        if(el.type==='checkbox'){
          ...
          if(k==='yMin' || k==='yMax' || k==='y2Min' || k==='y2Max' || k==='y3Min' || k==='y3Max' || k==='y4Min' || k==='y4Max' || k==='targetMin' || k==='targetMax' || k==='xTickAngle' || k==='yTickAngle' || k==='ciLevel'){
            var n = parseFloat(String(el.value||''));
            visDraft.options[k] = isFinite(n) ? n : null;
            if(k==='targetMin' || k==='targetMax') targetTouched = true;
          ...
          } else if(k==='labelCfVal' || k==='xTickCount' || k==='yTickCount'){
            var nv = parseFloat(String(el.value||''));
            visDraft.options[k] = isFinite(nv) ? nv : null;
          } else {
            visDraft.options[k] = String(el.value||'');
            if(k==='statsMode' && visDraft.options[k]==='none'){
              visDraft.options.statsModes = [];
              visDraft.options.regressionShowOutliers = false;
            }
          }
        }
      });
      ...
      // options (special): per-column agg/dec/date/font
      ['colagg','coldec','coldate','colfont'].forEach(function(kind){
        var sel = '[data-opt-'+kind+']';
        var els = visBody.querySelectorAll(sel);
        if(!els || !els.length) return;
        visDraft.options = visDraft.options || {};
        var tgt = (kind==='colagg') ? 'colAggs' : (kind==='coldec' ? 'colDecimals' : (kind==='coldate' ? 'colDateFmts' : 'colFontPx'));
        var map = {};
        els.forEach(function(inp){
          var key = inp.getAttribute('data-opt-'+kind);
          if(!key) return;
          var raw = String(inp.value||'').trim();
          if(!raw) return;
          if(kind==='coldec' || kind==='colfont'){
            var n = parseInt(raw,10); if(isFinite(n)) map[String(key)] = n;
          }else{
            map[String(key)] = raw;
          }
        });
        visDraft.options[tgt] = map;
      });
      ...
      // stats mode multi-select and color mappings
      var smEls = visBody.querySelectorAll('input[data-opt-statsmode]');
      if(smEls && smEls.length){
        visDraft.options = visDraft.options || {};
        var sm = [];
        smEls.forEach(function(el){ if(el.checked){ var v=String(el.getAttribute('data-opt-statsmode')||''); if(v) sm.push(v); } });
        if(sm.indexOf('regression')<0){ visDraft.options.regressionShowOutliers = false; visDraft.options.showTrend = false; }
        visDraft.options.statsModes = sm;
        visDraft.options.statsMode = sm.length ? sm[0] : 'none';
      }
      if(targetTouched || (visDraft.options && Object.prototype.hasOwnProperty.call(visDraft.options,'targetMode'))){
        var tMinRaw = visDraft.options ? visDraft.options.targetMin : null;
        var tMaxRaw = visDraft.options ? visDraft.options.targetMax : null;
        var tMin = (tMinRaw==null || tMinRaw==='') ? null : Number(tMinRaw);
        var tMax = (tMaxRaw==null || tMaxRaw==='') ? null : Number(tMaxRaw);
        var hasT = (tMin!=null && isFinite(tMin)) || (tMax!=null && isFinite(tMax));
        var hasTargetCols = !!(visDraft.options && (visDraft.options.targetMinCol || visDraft.options.targetMaxCol));
        var tMode = String((visDraft.options && visDraft.options.targetMode) || 'none');
        if(tMode==='none') visDraft.options.showTargets = false;
        else if(tMode==='columns') visDraft.options.showTargets = !!hasTargetCols;
        else visDraft.options.showTargets = !!hasT;
      }
      if(visDraft.options){
        var pp = String(visDraft.options.palettePreset||'custom');
        if(pp && pp!=='custom') visDraft.options.palette = vmPalettePresetRaw(pp);
        visDraft.options.statsColors = visDraft.options.statsColors || {};
        if(visDraft.options.statsColorMean){ visDraft.options.statsColors.mean = visDraft.options.statsColorMean; delete visDraft.options.statsColorMean; }
        if(visDraft.options.statsColorMedian){ visDraft.options.statsColors.median = visDraft.options.statsColorMedian; delete visDraft.options.statsColorMedian; }
        if(visDraft.options.statsColorCi){ visDraft.options.statsColors.ci = visDraft.options.statsColorCi; delete visDraft.options.statsColorCi; }
        if(visDraft.options.statsColorRegression){ visDraft.options.statsColors.regression = visDraft.options.statsColorRegression; delete visDraft.options.statsColorRegression; }
        if(visDraft.options.statsColorOutlier){ visDraft.options.statsColors.outlier = visDraft.options.statsColorOutlier; delete visDraft.options.statsColorOutlier; }
      }
      ...
      if(idx>=0){
        ...
        visDraft.__settingsBaseline = deepClone(visDraft.options || {});
        STATE.visuals[idx] = visDraft;
        saveState();
        var applyVid = visId;
        /* Always repaint grid after Apply so Answers page reflects updates immediately
           (mapping/type/options) without waiting for analysis save/refresh. */
        renderGrid();
        if(applyVid) requestVisualRender(applyVid);
        // Keep settings open after apply for cross-tab adjustments
        visDraft = deepClone(STATE.visuals[idx] || visDraft);
        renderVisualSettings();
      }
    };
    ...
    function initAfterPlotly(){
      ...
      if(!STATE.globals.cardShadow) STATE.globals.cardShadow = 'soft';
      if(!STATE.globals.wrapMax) STATE.globals.wrapMax = 'full';
    ...
    function vmRenderSingle(vid){
      try{
        ...
        if(def && typeof def.render === 'function'){
          ...
          try{
            var gdInt = plotEl.querySelector('.js-plotly-plot');
            if(gdInt) vmAttachInteractionHandlers(gdInt, v.id, v, ctx);
          }catch(_){ }
        }
    ...
    function vmAttachInteractionHandlers(gd, vid, v, ctx){
      if(!gd || !window.Plotly || !v) return;
      if(gd.__vmInteractCb){ try{ gd.removeListener('plotly_click', gd.__vmInteractCb); }catch(_){ } }
      var cb = function(ev){
        try{
          var pt = ev && ev.points && ev.points[0];
          if(!pt) return;
          var vv = vmFindVisual(vid); if(!vv) return;
          var m = vv.mapping||{};
          var xAll = Array.isArray(m.x) ? m.x.filter(Boolean) : (m.x?[m.x]:[]);
          if(vv.options && vv.options.drillEnabled && xAll.length>1 && ev && ev.event && ev.event.shiftKey){
            vv.options.drillLevel = (clamp(parseInt(vv.options.drillLevel||0,10)||0,0,xAll.length-1)+1) % xAll.length;
            saveState();
            vmRenderSingle(vid);
            return;
          }
          if(STATE && STATE.globals && STATE.globals.crossFilterEnabled && vv.options && vv.options.crossFilterEmit!==false){
            var dk = xAll.length ? xAll[clamp(parseInt(vv.options.drillLevel||0,10)||0,0,Math.max(0,xAll.length-1))] : null;
            if(dk){
              STATE.globals.crossFilter = {col:dk, val:pt.x, sourceVid:vid};
              saveState();
              renderGrid();
            }
          }
    ...
    function vmAttachRemoveHandler(gd, vid){
      ...
      var cb = function(ev){
        try{
          ...
          var sKey = (pt.data && pt.data.name) ? String(pt.data.name) : '';
          if(!sKey || sKey==='Data' || sKey==='Normal' || sKey==='Outlier') sKey = 0;
          var key = JSON.stringify([pt.x, pt.y, sKey]);
    ...
    function vmCloseCardMenus(exceptVid){
      try{
    ...
    function vmToggleLegacyCardMenu(vid){
    ...
    function vmResetVisual(vid){
      var v = vmFindVisual(vid);
      if(!v) return;
      var def = (Registry[v.type] && Registry[v.type].defaultVisual) ? Registry[v.type].defaultVisual() : null;
      var base = deepClone((v.__settingsBaseline && typeof v.__settingsBaseline==='object') ? v.__settingsBaseline : ((def && def.options) ? def.options : {}));
      v.options = base || {};
      v.filter = deepClone((def && def.filter) ? def.filter : defaultFilter());

      if(v.options){
        v.options.removedPoints = [];
        if(typeof v.options.showLegend!=='boolean') v.options.showLegend = true;
      }
    ...
    function vmPositionPop(btn){
      ...
      if((!btn || !btn.getBoundingClientRect) && pop.__vid && __vmLastToolbarBtnByVid && __vmLastToolbarBtnByVid[pop.__vid]) btn = __vmLastToolbarBtnByVid[pop.__vid];
      if(!btn || !btn.getBoundingClientRect) return;
      var r = btn.getBoundingClientRect();
      var pw = Math.min(Math.max(pop.offsetWidth||240, 180), window.innerWidth-16);
      var ph = Math.min(Math.max(pop.offsetHeight||200, 120), window.innerHeight-16);
      var left = r.left;
      var top = r.bottom + 6;
      ...
      if(act==='points'){
        vmOpenPointsMenu(vid, vmGetPlotDivByVid(vid));
        return;
      }
      var pop = vmPopEl();
      if(!pop) return;
      ...
      if(act==='points'){
        ...
        html += '<div class="sec"><div class="lbl">Legend</div><div class="row">'
              ...
              + vmChip('leg-auto','Auto', !v.options.legendPos || v.options.legendPos==='auto')
              + vmChip('leg-top','Top', v.options.legendPos==='top')
              + vmChip('leg-bottom','Bottom', v.options.legendPos==='bottom')
              + vmChip('leg-left','Left', v.options.legendPos==='left')
        ...
        try{ pop.classList.add('vlist'); }catch(_){}
        html += vmPopHdr('Visual Actions');
        var map = {reset:'Reset', points:'Points / Labels / Styles', yaxis:'Y-axis', targets:'Targets', filter:'Outliers', zoom:'Zoom / Pan', status:'Statistics', remove:'Remove data point', download:'Download'};
        var iconMap = {reset:'home', points:'points', yaxis:'yaxis', targets:'targets', filter:'filter', zoom:'zoommenu', status:'status', remove:'remove', download:'export'};
        var raw = String((btnEl && btnEl.getAttribute && btnEl.getAttribute('data-icons')) || '').split(',').filter(Boolean);
        html += '<div class="sec"><div class="row" style="display:flex;flex-direction:column;align-items:stretch;gap:0;padding:0;margin:0">';
        raw.forEach(function(k){
          var label = map[k] || k;
          var ik = iconMap[k] || 'more';
          html += '<button class="btn-sm" data-pop-act="tb-'+escapeHtml(k)+'" title="'+escapeHtml(label)+'" style="display:flex;align-items:center;gap:8px;border-radius:0;border-left:0;border-right:0;border-top:0;border-bottom:1px solid var(--vm-border);padding:7px 10px;text-align:left">'
               + '<span style="width:14px;height:14px;display:inline-flex;align-items:center;justify-content:center">'+(vmTbSvgs[ik]||vmTbSvgs.more||'')+'</span>'
    ...
    function vmHandleToolbarClick(btn, vid, act){
      ...
      if(btn && vid) __vmLastToolbarBtnByVid[vid] = btn;
      var gdHost = vmGetPlotDivByVid(vid);
      var gd = gdHost && gdHost.querySelector ? (gdHost.querySelector('.js-plotly-plot') || gdHost) : gdHost;

      if(act==='points'){ vmCloseOverlays(); vmOpenPointsMenu(vid, gd); return true; }
      if(act==='yaxis'){ vmCloseOverlays(); vmOpenYAxisMenu(vid, gd); return true; }
      if(act==='targets'){ vmCloseOverlays(); vmOpenTargetsMenu(vid, gd); return true; }
      if(act==='zoommenu'){ vmCloseOverlays(); vmOpenZoomMenu(vid, gd); return true; }
      if(act==='filter'){ vmCloseOverlays(); vmShowQuickFilterDialog(vid, gd); return true; }
      if(act==='status'){ vmCloseOverlays(); vmToggleStatusTable(vid, gd); return true; }
      if(act==='export'){ vmCloseOverlays(); vmOpenPopover(vid, act, btn); return true; }
      if(act==='more'){ vmCloseOverlays(); vmOpenPopover(vid, 'tbmenu', btn); return true; }
    ...
    document.addEventListener('click', function(e){
      ...
      if(chip && isPopOpen && pop.__vid){
         ...
         if(v){
           ...
           if(idc.indexOf('st-')===0){
             ...
             if(nextStat !== v.options.statsMode){
               v.options.statsMode = nextStat;
               v.options.statsModes = (nextStat && nextStat!=='none') ? [nextStat] : [];
               if(nextStat!=='regression') v.options.regressionShowOutliers = false;
               changed=true; needsRender=true;
             }
           }
      ...
      if(popBtn && isPopOpen && pop.__vid){
        ...
        if(actBtn.indexOf('tb-')===0){
            var k = actBtn.substring(3);
            var amap = {reset:'reset', points:'points', yaxis:'yaxis', targets:'targets', filter:'filter', zoom:'zoommenu', status:'status', remove:'remove', download:'export', settings:'settings', fs:'fs'};
            var tbAct = amap[k] || '';
            if(tbAct){ vmHandleToolbarClick((__vmLastToolbarBtnByVid[vid]||popBtn), vid, tbAct); }
            vmPopClose();
            e.preventDefault(); e.stopPropagation();
            return;
         }
